(* Content-type: application/vnd.wolfram.cdf.text *)

(*** Wolfram CDF File ***)
(* http://www.wolfram.com/cdf *)

(* CreatedBy='Mathematica 12.0' *)

(***************************************************************************)
(*                                                                         *)
(*                                                                         *)
(*  Under the Wolfram FreeCDF terms of use, this file and its content are  *)
(*  bound by the Creative Commons BY-SA Attribution-ShareAlike license.    *)
(*                                                                         *)
(*        For additional information concerning CDF licensing, see:        *)
(*                                                                         *)
(*         www.wolfram.com/cdf/adopting-cdf/licensing-options.html         *)
(*                                                                         *)
(*                                                                         *)
(***************************************************************************)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[      1088,         20]
NotebookDataLength[    102423,       2175]
NotebookOptionsPosition[     96977,       2064]
NotebookOutlinePosition[     98059,       2100]
CellTagsIndexPosition[     97986,       2095]
WindowTitle->Mass Balances in Evaporative Crystallization
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 ButtonBox[
  GraphicsBox[RasterBox[CompressedData["
1:
tu9+yuhi+5YoUaJEiZ559FT2rUSJEm2eLnZtSJQoUaJEiST6/wFtJUc4
    "], {{0, 0}, {1714, 38}}, {0, 255},
    ColorFunction->RGBColor],
   ImageSize->{1714, 38},
   PlotRange->{{0, 1714}, {0, 38}}],
  Alignment->Left,
  BaseStyle->{"Hyperlink", "DemonstrationHeader"},
  ButtonData->{
    URL["http://demonstrations.wolfram.com"], None},
  ButtonNote->"http://demonstrations.wolfram.com"]], "DemonstrationHeader",Exp\
ressionUUID->"0f6c982c-728a-4e7c-b5bb-e38debd5c80d"],

Cell[TextData[StyleBox["Mass Balances in Evaporative Crystallization",
 FontSize->36]], "Subtitle",
 CellChangeTimes->{
  3.7446508010241675`*^9, {3.7446508805616255`*^9, 3.74465088103907*^9}, 
   3.826833538121453*^9},ExpressionUUID->"ce7c456e-1f7e-4573-9ed7-\
2c07ad00f1a9"],

Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`m2sol$$ = 0, $CellContext`m4sol$$ = 
    0, $CellContext`m5sol$$ = 0, $CellContext`mfsol$$ = 
    47.300000000000004`, $CellContext`part1$$ = 1, $CellContext`part2$$ = 
    4, $CellContext`reset$$ = 3, $CellContext`solution$$ = 
    False, $CellContext`xk5sol$$ = 0, $CellContext`xw5sol$$ = 
    0, $CellContext`zksol$$ = 0, $CellContext`zwsol$$ = 0, Typeset`show$$ = 
    True, Typeset`bookmarkList$$ = {}, Typeset`bookmarkMode$$ = "Menu", 
    Typeset`animator$$, Typeset`animvar$$ = 1, Typeset`name$$ = 
    "\"untitled\"", Typeset`specs$$ = {{
      Hold[$CellContext`reset$$], 0}, {{
       Hold[$CellContext`solution$$], False, "solution"}, {True, False}}, {{
       Hold[$CellContext`mfsol$$], 0, 
       Row[{"feed flow rate ", 
         Subscript[
          Style[
           Overscript["m", 
            Style[".", Bold]], Italic], "feed"], " (kg/h)"}]}, 0, 100, 0.1}, {{
       Hold[$CellContext`m2sol$$], 0, 
       Row[{"water vapor flow rate ", 
         Subscript[
          Style[
           Overscript["m", 
            Style[".", Bold]], Italic], 2], " (kg/h)"}]}, 0, 100, 0.1}, {{
       Hold[$CellContext`m4sol$$], 0, 
       Row[{"KCl crystals flow rate ", 
         Subscript[
          Style[
           Overscript["m", 
            Style[".", Bold]], Italic], 4], " (kg/h)"}]}, 0, 100, 0.1}, {{
       Hold[$CellContext`m5sol$$], 0, 
       Row[{"KCl solution flow rate ", 
         Subscript[
          Style[
           Overscript["m", 
            Style[".", Bold]], Italic], 5], " (kg/h)"}]}, 0, 6, 0.01}, {{
       Hold[$CellContext`zksol$$], 0, 
       Row[{"KCl feed compostition ", 
         Subscript[
          Style["x", Italic], "f"]}]}, 0, 1, 0.001}, {{
       Hold[$CellContext`zwsol$$], 0, 
       Row[{"water feed compostition ", 
         Subscript[
          Style["x", Italic], "w"]}]}, 0, 1, 0.001}, {
      Hold[$CellContext`part1$$], 1}, {
      Hold[$CellContext`part2$$], 1}, {
      Hold[
       Grid[{{
          Button["new problem", {
            Increment[$CellContext`reset$$], $CellContext`solution$$ = 
            False, $CellContext`mfsol$$ = 0, $CellContext`m2sol$$ = 
            0, $CellContext`m4sol$$ = 0, $CellContext`m5sol$$ = 
            0, $CellContext`zksol$$ = 0, $CellContext`zwsol$$ = 
            0, $CellContext`xk5sol$$ = 0, $CellContext`xw5sol$$ = 0}], 
          Manipulate`Place[1], 
          Manipulate`Place[2], 
          Spacer[25], 
          
          PaneSelector[{
           1 -> Manipulate`Place[3], 2 -> Manipulate`Place[4], 3 -> 
            Manipulate`Place[5], 4 -> Manipulate`Place[6], 5 -> 
            Manipulate`Place[7], 6 -> Manipulate`Place[8]}, 
           Dynamic[$CellContext`part1$$]], 
          Manipulate`Place[9], 
          Manipulate`Place[10]}}]], Manipulate`Dump`ThisIsNotAControl}, {{
       Hold[$CellContext`mfsol$$], 0, 
       Row[{"feed flow rate ", 
         Subscript[
          Style[
           Overscript["m", 
            Style[".", Bold]], Italic], "feed"], " (kg/h)"}]}, 0, 100, 0.1}, {{
       Hold[$CellContext`m2sol$$], 0, 
       Row[{"water vapor flow rate ", 
         Subscript[
          Style[
           Overscript["m", 
            Style[".", Bold]], Italic], 2], " (kg/h)"}]}, 0, 100, 0.1}, {{
       Hold[$CellContext`m4sol$$], 0, 
       Row[{"KCl crystals flow rate ", 
         Subscript[
          Style[
           Overscript["m", 
            Style[".", Bold]], Italic], 4], " (kg/h)"}]}, 0, 100, 0.1}, {{
       Hold[$CellContext`m5sol$$], 0, 
       Row[{"KCl solution flow rate ", 
         Subscript[
          Style[
           Overscript["m", 
            Style[".", Bold]], Italic], 5], " (kg/h)"}]}, 0, 6, 0.01}, {{
       Hold[$CellContext`xk5sol$$], 0, 
       Row[{"KCl stream 5 composition ", 
         Subscript[
          Style["x", Italic], "5"]}]}, 0, 1, 0.001}, {{
       Hold[$CellContext`xw5sol$$], 0, 
       Row[{"water stream 5 composition ", 
         Subscript[
          Style["x", Italic], "w, 5"]}]}, 0, 1, 0.001}, {
      Hold[
       Grid[{{
          
          PaneSelector[{
           1 -> Manipulate`Place[11], 2 -> Manipulate`Place[12], 3 -> 
            Manipulate`Place[13], 4 -> Manipulate`Place[14], 5 -> 
            Manipulate`Place[15], 6 -> Manipulate`Place[16]}, 
           Dynamic[$CellContext`part2$$]], 
          "solve for the unknown variables (in green)"}}]], 
      Manipulate`Dump`ThisIsNotAControl}}, Typeset`size$$ = {
    600., {147., 153.}}, Typeset`update$$ = 0, Typeset`initDone$$, 
    Typeset`skipInitDone$$ = True, $CellContext`solution$75313$$ = 
    False, $CellContext`mfsol$75314$$ = 0, $CellContext`m2sol$75315$$ = 
    0, $CellContext`m4sol$75316$$ = 0, $CellContext`m5sol$75317$$ = 
    0, $CellContext`zksol$75318$$ = 0, $CellContext`zwsol$75319$$ = 
    0, $CellContext`xk5sol$75320$$ = 0, $CellContext`xw5sol$75321$$ = 0}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     2, StandardForm, 
      "Variables" :> {$CellContext`m2sol$$ = 0, $CellContext`m4sol$$ = 
        0, $CellContext`m5sol$$ = 0, $CellContext`mfsol$$ = 
        0, $CellContext`part1$$ = 1, $CellContext`part2$$ = 
        1, $CellContext`reset$$ = 0, $CellContext`solution$$ = 
        False, $CellContext`xk5sol$$ = 0, $CellContext`xw5sol$$ = 
        0, $CellContext`zksol$$ = 0, $CellContext`zwsol$$ = 0}, 
      "ControllerVariables" :> {
        Hold[$CellContext`solution$$, $CellContext`solution$75313$$, False], 
        Hold[$CellContext`mfsol$$, $CellContext`mfsol$75314$$, 0], 
        Hold[$CellContext`m2sol$$, $CellContext`m2sol$75315$$, 0], 
        Hold[$CellContext`m4sol$$, $CellContext`m4sol$75316$$, 0], 
        Hold[$CellContext`m5sol$$, $CellContext`m5sol$75317$$, 0], 
        Hold[$CellContext`zksol$$, $CellContext`zksol$75318$$, 0], 
        Hold[$CellContext`zwsol$$, $CellContext`zwsol$75319$$, 0], 
        Hold[$CellContext`xk5sol$$, $CellContext`xk5sol$75320$$, 0], 
        Hold[$CellContext`xw5sol$$, $CellContext`xw5sol$75321$$, 0]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> 
      Module[{$CellContext`crystals$, $CellContext`Tevap$, \
$CellContext`Tcrys$, $CellContext`mf$, $CellContext`zk$, $CellContext`xk3$, \
$CellContext`xk5$, $CellContext`xkr$, $CellContext`s3$, $CellContext`s5$, \
$CellContext`mdot1$, $CellContext`mdot2$, $CellContext`mdot3$, \
$CellContext`mdot4$, $CellContext`mdot5$, $CellContext`mdotr$, \
$CellContext`xk1$, $CellContext`unknowns1$, $CellContext`unknowns2$, \
$CellContext`unknowns$, $CellContext`list$, $CellContext`ax$, \
$CellContext`ay$, $CellContext`bx$, $CellContext`by$, $CellContext`colU$, \
$CellContext`sol$, $CellContext`sigFig$}, SetOptions[
          EvaluationNotebook[], StyleDefinitions -> Notebook[{
             Cell[
              StyleData[StyleDefinitions -> "Default.nb"]], 
             Cell[
              StyleData["Graphics"], FontFamily -> "Arial"]}, 
            StyleDefinitions -> "PrivateStylesheetFormatting.nb"]]; 
        SeedRandom[$CellContext`reset$$]; $CellContext`sigFig$[
           Pattern[$CellContext`n, 
            Blank[]]] := 
         If[$CellContext`n > 999.99, 10 Round[$CellContext`n/10], 
           If[$CellContext`n > 99.999, 
            Round[$CellContext`n], 
            If[$CellContext`n > 9.99, 
             NumberForm[$CellContext`n, {3, 1}], 
             If[$CellContext`n > 0.999, 
              NumberForm[$CellContext`n, {3, 2}], 
              NumberForm[$CellContext`n, {4, 3}]]]]]; While[MemberQ[
             
             Positive[{$CellContext`mdot1$, $CellContext`mdot2$, \
$CellContext`mdot3$, $CellContext`mdot4$, $CellContext`mdot5$, \
$CellContext`mdotr$, $CellContext`xk1$}], False] == True; $CellContext`Tevap$ = 
           RandomInteger[{60, 150}]; $CellContext`Tcrys$ = 
           RandomInteger[{25, 50}]; $CellContext`mf$ = 
           RandomInteger[{80, 100}]; $CellContext`s3$ = 
           0.2881 $CellContext`Tevap$ + 28.123; $CellContext`s5$ = 
           0.2881 $CellContext`Tcrys$ + 28.123; $CellContext`xk3$ = 
           Round[$CellContext`s3$/($CellContext`s3$ + 100), 
             0.001]; $CellContext`xk5$ = 
           Round[$CellContext`s5$/($CellContext`s5$ + 100), 
             0.001]; $CellContext`xkr$ = $CellContext`xk5$; $CellContext`zk$ = 
           Round[$CellContext`xk3$ (RandomInteger[{35, 50}]/100.), 
             0.001]; $CellContext`crystals$ = 
           Round[(0.95 $CellContext`mf$) $CellContext`zk$, 
             0.001]; $CellContext`sol$ = Flatten[
             Quiet[
              
              Solve[{$CellContext`mf$ == $CellContext`m2 + $CellContext`m4 + \
$CellContext`m5, $CellContext`zk$ $CellContext`mf$ == $CellContext`crystals$ + \
$CellContext`xk5$ $CellContext`m5, $CellContext`zk$ $CellContext`mf$ + \
$CellContext`xkr$ $CellContext`mr == $CellContext`xk1$ $CellContext`m1, (
                  1 - $CellContext`zk$) $CellContext`mf$ == $CellContext`m2 + \
(1 - $CellContext`xk5$) $CellContext`m5, $CellContext`m5 == $CellContext`m3 - \
$CellContext`m4 - $CellContext`mr, $CellContext`m3 == $CellContext`m1 - \
$CellContext`m2, $CellContext`xk3$ $CellContext`m3 == $CellContext`m4 + \
$CellContext`xk5$ $CellContext`m5 + $CellContext`xkr$ $CellContext`mr, \
$CellContext`mf$ + $CellContext`mr == $CellContext`m1}, {$CellContext`m1, \
$CellContext`m2, $CellContext`m3, $CellContext`m4, $CellContext`m5, \
$CellContext`mr, $CellContext`xk1$}, Reals]]]; $CellContext`mdot1$ = 
           ReplaceAll[$CellContext`m1, $CellContext`sol$]; \
$CellContext`mdot2$ = 
           ReplaceAll[$CellContext`m2, $CellContext`sol$]; \
$CellContext`mdot3$ = 
           ReplaceAll[$CellContext`m3, $CellContext`sol$]; \
$CellContext`mdot4$ = 
           ReplaceAll[$CellContext`m4, $CellContext`sol$]; \
$CellContext`mdot5$ = 
           ReplaceAll[$CellContext`m5, $CellContext`sol$]; \
$CellContext`mdotr$ = 
           ReplaceAll[$CellContext`mr, $CellContext`sol$]; $CellContext`xk1$ = 
           ReplaceAll[$CellContext`xk1$, $CellContext`sol$]; If[
            Or[MemberQ[
               
               Positive[{$CellContext`mdot1$, $CellContext`mdot2$, \
$CellContext`mdot3$, $CellContext`mdot4$, $CellContext`mdot5$, \
$CellContext`mdotr$, $CellContext`xk1$}], False] == True, $CellContext`mdotr$ < 
             20], 
            Increment[$CellContext`reset$$]]]; $CellContext`unknowns1$ = 
         RandomChoice[{$CellContext`MF, $CellContext`M2, $CellContext`M4, \
$CellContext`M5, $CellContext`ZK}]; $CellContext`unknowns2$ = RandomChoice[
           
           Cases[{$CellContext`MF, $CellContext`M2, $CellContext`M4, \
$CellContext`M5, $CellContext`XK5}, 
            
            Except[$CellContext`unknowns1$]]]; $CellContext`unknowns$ = \
{$CellContext`unknowns1$, $CellContext`unknowns2$}; $CellContext`list$[
           Pattern[$CellContext`var$, 
            Blank[]]] := 
         MemberQ[$CellContext`unknowns$, $CellContext`var$]; \
$CellContext`part1$$ = Part[
           
           Position[{$CellContext`MF, $CellContext`M2, $CellContext`M4, \
$CellContext`M5, $CellContext`ZK, $CellContext`ZW}, $CellContext`unknowns1$], 
           1, 1]; $CellContext`part2$$ = (If[
           
           And[$CellContext`part2$$ >= $CellContext`part1$$, \
$CellContext`part1$$ <= 4], # + 1, #]& )[
           Part[
            Position[
             
             Cases[{$CellContext`MF, $CellContext`M2, $CellContext`M4, \
$CellContext`M5, $CellContext`XK5, $CellContext`XW5}, 
              Except[$CellContext`unknowns1$]], $CellContext`unknowns2$], 1, 
            1]]; $CellContext`ax$ = 2.25; $CellContext`ay$ = 
         0.6; $CellContext`bx$ = 1.78; $CellContext`by$ = 
         0.7; $CellContext`colU$ = RGBColor[0, 0.55, 0.1]; Graphics[{{
            Arrowheads[0.02], 
            Arrow[{{0, 0}, {$CellContext`ax$, 0}}], 
            Arrow[{{$CellContext`ax$, 0}, {2 $CellContext`ax$, 0}}], 
            Arrow[{{
              2 $CellContext`ax$ + 0.5 $CellContext`bx$, $CellContext`by$/
               2}, {2 $CellContext`ax$ + 
               0.5 $CellContext`bx$, $CellContext`by$/2 + $CellContext`ay$}}], 
            
            Arrow[{{2 $CellContext`ax$ + $CellContext`bx$, 0}, {
              3 $CellContext`ax$ + $CellContext`bx$, 0}}], 
            
            Arrow[{{3 $CellContext`ax$ + 2 $CellContext`bx$, 0.25}, {
              4 $CellContext`ax$ + 2 $CellContext`bx$, 0.25}}], 
            
            Arrow[{{3 $CellContext`ax$ + 
               1.5 $CellContext`bx$, -($CellContext`by$/2)}, {
              3 $CellContext`ax$ + 
               1.5 $CellContext`bx$, -($CellContext`by$/
                 2 + $CellContext`ay$)}}], 
            
            Arrow[{{3 $CellContext`ax$ + 2 $CellContext`bx$, -0.25}, {
              4 $CellContext`ax$ + 2 $CellContext`bx$, -0.25}}], 
            
            Arrow[{{3 $CellContext`ax$ + 
               1.5 $CellContext`bx$, (-$CellContext`by$)/
                2 - $CellContext`ay$}, {$CellContext`ax$, (-$CellContext`by$)/
                2 - $CellContext`ay$}, {$CellContext`ax$, 0}}]}, {
            EdgeForm[Thick], 
            FaceForm[None], 
            
            Rectangle[{2 $CellContext`ax$, (-$CellContext`by$)/2}, {
             2 $CellContext`ax$ + $CellContext`bx$, $CellContext`by$/2}], 
            
            Rectangle[{
             3 $CellContext`ax$ + $CellContext`bx$, (-$CellContext`by$)/2}, {
             3 $CellContext`ax$ + 2 $CellContext`bx$, $CellContext`by$/2}]}, 
           Text[
            Style["evaporator", 13], {
            2 $CellContext`ax$ + 0.5 $CellContext`bx$, 0}], 
           Text[
            Style[
             Column[{"crystallizer", "and filter"}, Center], 13], {
            3 $CellContext`ax$ + 1.5 $CellContext`bx$, 0}], 
           Text[
            Style["recycle", 13], {
            3 $CellContext`ax$ + $CellContext`bx$, (-$CellContext`by$)/
              2 - $CellContext`ay$}, {0, 1}], 
           Text[
            Style[
             Column[{"fresh feed", 
               Style[
                Row[{
                  Subscript[
                   Style[
                    Overscript["m", 
                    Style[".", Bold]], Italic], "feed"], " = ", 
                  If[
                   $CellContext`list$[$CellContext`MF], $CellContext`mfsol$$, \
$CellContext`mf$], " kg/h"}], 
                If[
                 $CellContext`list$[$CellContext`MF], $CellContext`colU$, 
                 Black]], 
               Column[{
                 Style[
                  Row[{
                    Subscript[
                    Style["x", Italic], "f"], " = ", 
                    $CellContext`sigFig$[
                    If[
                    $CellContext`list$[$CellContext`ZK], \
$CellContext`zksol$$, $CellContext`zk$]]}], 
                  Which[
                   $CellContext`list$[$CellContext`ZK], $CellContext`colU$, 
                   And[
                    $CellContext`list$[$CellContext`ZW], \
$CellContext`solution$$ == False], White, 
                   Or[$CellContext`solution$$ == True, 0 == 0], Black]]}, 
                Center]}, Center, Spacings -> 0.5], 13], {
            0.35 $CellContext`ax$, 0}, {0, 0.4}], 
           Text[
            Style[
             Column[{
               Row[{
                 Subscript[
                  Style[
                   Overscript["m", 
                    Style[".", Bold]], Italic], 1], " = ", 
                 $CellContext`sigFig$[$CellContext`mdot1$], " kg/h"}], 
               Column[{
                 Row[{
                   Subscript[
                    Style["x", Italic], "1"], " = ", 
                   $CellContext`sigFig$[$CellContext`xk1$]}]}, Center]}, 
              Center, Spacings -> 0.6], 13], {1.5 $CellContext`ax$, 0.02}, {
            0, 0.05}], 
           Text[
            Style[
             Column[{"water vapor", 
               Style[
                Row[{
                  Subscript[
                   Style[
                    Overscript["m", 
                    Style[".", Bold]], Italic], 2], " = ", 
                  $CellContext`sigFig$[
                   If[
                    $CellContext`list$[$CellContext`M2], \
$CellContext`m2sol$$, $CellContext`mdot2$]], " kg/h"}], 
                If[
                 $CellContext`list$[$CellContext`M2], $CellContext`colU$, 
                 Black]]}, Center], 13], {
            2 $CellContext`ax$ + 
             1.15 $CellContext`bx$, $CellContext`by$/2 + $CellContext`ay$}, {
            0, 0.05}], 
           Text[
            Style[
             Column[{
               Row[{
                 Subscript[
                  Style[
                   Overscript["m", 
                    Style[".", Bold]], Italic], 3], " = ", 
                 $CellContext`sigFig$[$CellContext`mdot3$], " kg/h"}], 
               Column[{
                 Row[{
                   Subscript[
                    Style["x", Italic], "3"], " = ", 
                   $CellContext`sigFig$[$CellContext`xk3$]}]}, Center]}, 
              Center, Spacings -> 0.6], 13], {
            2.5 $CellContext`ax$ + $CellContext`bx$, 0}, {0, -0.05}], 
           Text[
            Style[
             Column[{"KCl crystals", 
               Style[
                Row[{
                  Subscript[
                   Style[
                    Overscript["m", 
                    Style[".", Bold]], Italic], 4], " = ", 
                  $CellContext`sigFig$[
                   If[
                    $CellContext`list$[$CellContext`M4], \
$CellContext`m4sol$$, $CellContext`mdot4$]], " kg/h"}], 
                If[
                 $CellContext`list$[$CellContext`M4], $CellContext`colU$, 
                 Black]]}, Center], 13], {
            3.5 $CellContext`ax$ + 2 $CellContext`bx$, 0.5}, {0, 0.05}], 
           Text[
            Style[
             Column[{"KCl solution", 
               Style[
                Row[{
                  Subscript[
                   Style[
                    Overscript["m", 
                    Style[".", Bold]], Italic], 5], " = ", 
                  $CellContext`sigFig$[
                   If[
                    $CellContext`list$[$CellContext`M5], \
$CellContext`m5sol$$, $CellContext`mdot5$]], " kg/h"}], 
                If[
                 $CellContext`list$[$CellContext`M5], $CellContext`colU$, 
                 Black]], 
               Column[{
                 Style[
                  Row[{
                    Subscript[
                    Style["x", Italic], "5"], " = ", 
                    $CellContext`sigFig$[
                    If[
                    $CellContext`list$[$CellContext`XK5], \
$CellContext`xk5sol$$, $CellContext`xk5$]]}], 
                  Which[
                   $CellContext`list$[$CellContext`XK5], $CellContext`colU$, 
                   And[
                    $CellContext`list$[$CellContext`XW5], \
$CellContext`solution$$ == False], White, 0 == 0, Black]]}, Center]}, Center, 
              Spacings -> 0.5], 13], {
            3.5 $CellContext`ax$ + 2 $CellContext`bx$, -0.33}, {0, 0.25}], 
           Text[
            Style[
             Column[{
               Row[{
                 Subscript[
                  Style[
                   Overscript["m", 
                    Style[".", Bold]], Italic], "R"], " = ", 
                 $CellContext`sigFig$[$CellContext`mdotr$], " kg/h"}], 
               Column[{
                 Row[{
                   Subscript[
                    Style["x", Italic], "R"], " = ", 
                   $CellContext`sigFig$[$CellContext`xkr$]}]}, Center]}, 
              Center, Spacings -> 0.5], 13], {
            2 $CellContext`ax$ + 
             0.5 $CellContext`bx$, (-$CellContext`by$)/
              2 - $CellContext`ay$}, {0, -0.05}], 
           If[$CellContext`solution$$, 
            Text[
             Style[
              Framed[
               Column[
                ReplaceAll[{
                  If[
                   $CellContext`list$[$CellContext`MF], 
                   Row[{"correct ", 
                    Subscript[
                    Style[
                    Overscript["m", 
                    Style[".", Bold]], Italic], "feed"], " = ", 
                    $CellContext`sigFig$[$CellContext`mf$], " kg/h"}]], 
                  If[
                   $CellContext`list$[$CellContext`ZK], 
                   Row[{"correct ", 
                    Subscript[
                    Style["z", Italic], "k"], " = ", 
                    $CellContext`sigFig$[$CellContext`zk$]}]], 
                  If[
                   $CellContext`list$[$CellContext`ZW], 
                   Row[{"correct ", 
                    Subscript[
                    Style["z", Italic], "w"], " = ", 
                    $CellContext`sigFig$[1 - $CellContext`zk$]}]], 
                  If[
                   $CellContext`list$[$CellContext`M2], 
                   Row[{"correct ", 
                    Subscript[
                    Style[
                    Overscript["m", 
                    Style[".", Bold]], Italic], 2], " = ", 
                    $CellContext`sigFig$[$CellContext`mdot2$], " kg/h"}]], 
                  If[
                   $CellContext`list$[$CellContext`M4], 
                   Row[{"correct ", 
                    Subscript[
                    Style[
                    Overscript["m", 
                    Style[".", Bold]], Italic], 4], " = ", 
                    $CellContext`sigFig$[$CellContext`mdot4$], " kg/h"}]], 
                  If[
                   $CellContext`list$[$CellContext`M5], 
                   Row[{"correct ", 
                    Subscript[
                    Style[
                    Overscript["m", 
                    Style[".", Bold]], Italic], 5], " = ", 
                    $CellContext`sigFig$[$CellContext`mdot5$], " kg/h"}]], 
                  If[
                   $CellContext`list$[$CellContext`XK5], 
                   Row[{"correct ", 
                    Subscript[
                    Style["x", Italic], "k,5"], " = ", 
                    $CellContext`sigFig$[$CellContext`xk5$]}]], 
                  If[
                   $CellContext`list$[$CellContext`XW5], 
                   Row[{"correct ", 
                    Subscript[
                    Style["x", Italic], "w,5"], " = ", 
                    $CellContext`sigFig$[1 - $CellContext`xk5$]}]]}, Null -> 
                 Nothing]]], 17], {0.9 $CellContext`ax$, 0.85}]]}, 
          ImageSize -> {600, 300}, PlotRange -> {{-0.25, 12.5}, {-1, 1.25}}, 
          PlotRangePadding -> {{0, 0}, {0.4, 0.2}}, AspectRatio -> Full]], 
      "Specifications" :> {{$CellContext`reset$$, 0, ControlType -> None, 
         ControlPlacement -> 
         1}, {{$CellContext`solution$$, False, "solution"}, {True, False}, 
         Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         2}, {{$CellContext`mfsol$$, 0, 
          Row[{"feed flow rate ", 
            Subscript[
             Style[
              Overscript["m", 
               Style[".", Bold]], Italic], "feed"], " (kg/h)"}]}, 0, 100, 0.1,
          Appearance -> "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         3}, {{$CellContext`m2sol$$, 0, 
          Row[{"water vapor flow rate ", 
            Subscript[
             Style[
              Overscript["m", 
               Style[".", Bold]], Italic], 2], " (kg/h)"}]}, 0, 100, 0.1, 
         Appearance -> "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         4}, {{$CellContext`m4sol$$, 0, 
          Row[{"KCl crystals flow rate ", 
            Subscript[
             Style[
              Overscript["m", 
               Style[".", Bold]], Italic], 4], " (kg/h)"}]}, 0, 100, 0.1, 
         Appearance -> "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         5}, {{$CellContext`m5sol$$, 0, 
          Row[{"KCl solution flow rate ", 
            Subscript[
             Style[
              Overscript["m", 
               Style[".", Bold]], Italic], 5], " (kg/h)"}]}, 0, 6, 0.01, 
         Appearance -> "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         6}, {{$CellContext`zksol$$, 0, 
          Row[{"KCl feed compostition ", 
            Subscript[
             Style["x", Italic], "f"]}]}, 0, 1, 0.001, Appearance -> 
         "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         7}, {{$CellContext`zwsol$$, 0, 
          Row[{"water feed compostition ", 
            Subscript[
             Style["x", Italic], "w"]}]}, 0, 1, 0.001, Appearance -> 
         "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         8}, {$CellContext`part1$$, 1, ControlType -> None, ControlPlacement -> 
         9}, {$CellContext`part2$$, 1, ControlType -> None, ControlPlacement -> 
         10}, 
        Grid[{{
           Button["new problem", {
             Increment[$CellContext`reset$$], $CellContext`solution$$ = 
             False, $CellContext`mfsol$$ = 0, $CellContext`m2sol$$ = 
             0, $CellContext`m4sol$$ = 0, $CellContext`m5sol$$ = 
             0, $CellContext`zksol$$ = 0, $CellContext`zwsol$$ = 
             0, $CellContext`xk5sol$$ = 0, $CellContext`xw5sol$$ = 0}], 
           Manipulate`Place[1], 
           Manipulate`Place[2], 
           Spacer[25], 
           
           PaneSelector[{
            1 -> Manipulate`Place[3], 2 -> Manipulate`Place[4], 3 -> 
             Manipulate`Place[5], 4 -> Manipulate`Place[6], 5 -> 
             Manipulate`Place[7], 6 -> Manipulate`Place[8]}, 
            Dynamic[$CellContext`part1$$]], 
           Manipulate`Place[9], 
           Manipulate`Place[10]}}], {{$CellContext`mfsol$$, 0, 
          Row[{"feed flow rate ", 
            Subscript[
             Style[
              Overscript["m", 
               Style[".", Bold]], Italic], "feed"], " (kg/h)"}]}, 0, 100, 0.1,
          Appearance -> "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         11}, {{$CellContext`m2sol$$, 0, 
          Row[{"water vapor flow rate ", 
            Subscript[
             Style[
              Overscript["m", 
               Style[".", Bold]], Italic], 2], " (kg/h)"}]}, 0, 100, 0.1, 
         Appearance -> "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         12}, {{$CellContext`m4sol$$, 0, 
          Row[{"KCl crystals flow rate ", 
            Subscript[
             Style[
              Overscript["m", 
               Style[".", Bold]], Italic], 4], " (kg/h)"}]}, 0, 100, 0.1, 
         Appearance -> "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         13}, {{$CellContext`m5sol$$, 0, 
          Row[{"KCl solution flow rate ", 
            Subscript[
             Style[
              Overscript["m", 
               Style[".", Bold]], Italic], 5], " (kg/h)"}]}, 0, 6, 0.01, 
         Appearance -> "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         14}, {{$CellContext`xk5sol$$, 0, 
          Row[{"KCl stream 5 composition ", 
            Subscript[
             Style["x", Italic], "5"]}]}, 0, 1, 0.001, Appearance -> 
         "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 
         15}, {{$CellContext`xw5sol$$, 0, 
          Row[{"water stream 5 composition ", 
            Subscript[
             Style["x", Italic], "w, 5"]}]}, 0, 1, 0.001, Appearance -> 
         "Labeled", ImageSize -> Small, Enabled -> Dynamic[
           If[$CellContext`solution$$, False, True]], ControlPlacement -> 16}, 
        Grid[{{
           
           PaneSelector[{
            1 -> Manipulate`Place[11], 2 -> Manipulate`Place[12], 3 -> 
             Manipulate`Place[13], 4 -> Manipulate`Place[14], 5 -> 
             Manipulate`Place[15], 6 -> Manipulate`Place[16]}, 
            Dynamic[$CellContext`part2$$]], 
           "solve for the unknown variables (in green)"}}]}, "Options" :> {}, 
      "DefaultOptions" :> {ControllerLinking -> True}],
     ImageSizeCache->{651., {207., 213.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    SynchronousInitialization->True,
    UndoTrackedVariables:>{Typeset`show$$, Typeset`bookmarkMode$$},
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Input",
 CellChangeTimes->{{3.7439643785088263`*^9, 3.743964437879319*^9}, 
   3.744650797367858*^9, {3.744650888517812*^9, 3.744650932175304*^9}, 
   3.82683354869499*^9},
 CellID->377157575,ExpressionUUID->"6f2916ec-4147-44b1-b0ce-81232036e730"],

Cell[TextData[{
 StyleBox["Caption", "Subsection"],
 "\n",
 StyleBox["In this Demonstration, you can perform mass balances on an \
evaporative crystallization process. Use the sliders to select the correct \
solution for the variables shown in green, then check the \
\[OpenCurlyDoubleQuote]solution\[CloseCurlyDoubleQuote] box to verify your \
answers. Click the \[OpenCurlyDoubleQuote]new problem\[CloseCurlyDoubleQuote] \
button for a new set of conditions.",
  FontSize->16]
}], "Text",
 CellChangeTimes->{{3.7439644645171447`*^9, 3.7439644936678715`*^9}, 
   3.744650805398138*^9, {3.744650938736251*^9, 3.7446509416639132`*^9}, 
   3.826833556847985*^9, {3.8268337902472057`*^9, 3.826833793156147*^9}},
 CellID->957328473,ExpressionUUID->"abd28a3c-1615-46f9-88d1-0712bde5d8d9"],

Cell[TextData[StyleBox["Details", "Subsection"]], "Text",
 CellChangeTimes->{{3.8268337956212683`*^9, 
  3.826833797570143*^9}},ExpressionUUID->"cd92d376-b5a9-4e2f-afa7-\
e392be73c809"],

Cell[TextData[StyleBox["Overall material balances",
 FontWeight->"Bold"]], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.585441346609769*^9}, 
   3.591366302935073*^9, {3.6817498871054525`*^9, 3.6817498871054525`*^9}},
 CellID->19283394,ExpressionUUID->"383ccd0f-0803-4538-bce2-46a60589664b"],

Cell[TextData[{
 "[1]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox[
     OverscriptBox["m", 
      StyleBox[".",
       FontSize->18]], "feed"], "=", 
    RowBox[{
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "2"], "+", 
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "4"], "+", 
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "5"]}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"9712a0df-2a79-4379-bf04-0b9e4bf6d569"]
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.585441346609769*^9}, 
   3.591366302935073*^9, {3.6817498871054525`*^9, 3.6817499060400877`*^9}, 
   3.681749954249009*^9, {3.6840784150794754`*^9, 3.6840784150794754`*^9}, 
   3.7484591360170937`*^9, 3.824887859835417*^9, {3.824887915488731*^9, 
   3.8248879223953066`*^9}, 3.824888019965206*^9, {3.824888113717883*^9, 
   3.824888153735222*^9}},
 CellID->214726250,ExpressionUUID->"640c58ef-1cd2-465f-a902-00e05998b6fe"],

Cell[TextData[{
 "[2]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     SubscriptBox["x", "f"], 
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "feed"]}], "=", " ", 
    RowBox[{
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "4"], "+", 
     RowBox[{
      SubscriptBox["x", "5"], " ", 
      SubscriptBox[
       OverscriptBox["m", 
        StyleBox[".",
         FontSize->18]], "5"]}]}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"88c361f0-7a88-4ad1-9cf5-0caf3b940b24"]
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.585441346609769*^9}, 
   3.591366302935073*^9, {3.6817498871054525`*^9, 3.6817499537966084`*^9}, 
   3.824887861731149*^9, {3.824887924839764*^9, 3.82488792643102*^9}, 
   3.8248880190173917`*^9, {3.824888156265148*^9, 3.824888162131159*^9}, {
   3.824888293782768*^9, 3.824888332353225*^9}},
 CellID->421537351,ExpressionUUID->"0b7ba6bb-499a-4cea-a072-588fbdbcbc43"],

Cell[TextData[{
 "[3]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"(", 
      RowBox[{"1", "-", 
       SubscriptBox["x", "f"]}], ")"}], 
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "feed"]}], "=", 
    RowBox[{
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "2"], "+", 
     RowBox[{
      RowBox[{"(", 
       RowBox[{"1", "-", 
        SubscriptBox["x", "5"]}], ")"}], " ", 
      SubscriptBox[
       OverscriptBox["m", 
        StyleBox[".",
         FontSize->18]], "5"]}]}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"0b5c8785-c6f7-43dd-8122-7f388295fb54"]
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.585441346609769*^9}, 
   3.591366302935073*^9, {3.6817498871054525`*^9, 3.681749946109778*^9}, {
   3.748459030649766*^9, 3.748459042321328*^9}, {3.8248878628197784`*^9, 
   3.8248878628197784`*^9}, {3.8248879290591946`*^9, 3.824887933548792*^9}, 
   3.824888018470832*^9, {3.824888164105158*^9, 3.824888167299133*^9}, {
   3.8248882928543267`*^9, 3.824888343023822*^9}},
 CellID->206020753,ExpressionUUID->"4b26d3a9-9083-4c9f-aadc-2ae1d4b3553b"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox[
    OverscriptBox["m", 
     StyleBox[".",
      FontSize->18]], "feed"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "e4424e40-9e73-4e19-add1-4ba4d731e947"],
 " = mass flow rate of fresh feed (kg/h)"
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415497533875`*^9}, {
  3.591366495377385*^9, 3.5913665086856937`*^9}, {3.6775043341342874`*^9, 
  3.67750433707021*^9}, {3.6775139474004235`*^9, 3.677513947716282*^9}, {
  3.681662755617515*^9, 3.6816627785120535`*^9}, {3.681740849836585*^9, 
  3.6817408556007977`*^9}, {3.8248881700561132`*^9, 3.8248881700561132`*^9}},
 CellID->32512792,ExpressionUUID->"3e9cef2d-4940-4665-a451-43483d2cb461"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox[
    OverscriptBox["m", 
     StyleBox[".",
      FontSize->18]], "2"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "2d8ae76b-061d-4a7e-a86a-81bfb8a7c2a0"],
 " = mass flow rate of water exiting evaporator (kg/h)"
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
  3.5854996260908947`*^9, 3.585499626095895*^9}, {3.591366520663713*^9, 
  3.591366527788459*^9}, {3.677504327990834*^9, 3.677504331395461*^9}, {
  3.681662758349326*^9, 3.681662784548665*^9}, {3.824888172374316*^9, 
  3.824888172374316*^9}},
 CellID->383785788,ExpressionUUID->"1cce1dd9-68f9-45fe-9f3e-94b517912b44"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox[
    OverscriptBox["m", 
     StyleBox[".",
      FontSize->18]], "4"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "ca29d444-79ac-4459-95fb-09661e421644"],
 " = mass flow rate of solid KCl crystals exiting the crystallizer and filter \
(kg/h)"
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
   3.5854996260908947`*^9, 3.5854996675812674`*^9}, {3.591366536730555*^9, 
   3.591366542559383*^9}, {3.677504139817539*^9, 3.677504139817539*^9}, {
   3.677504200115492*^9, 3.677504213667881*^9}, {3.6775043610623116`*^9, 
   3.6775043653998575`*^9}, {3.677514029840173*^9, 3.6775140332606544`*^9}, {
   3.6816627600809293`*^9, 3.6816627904670763`*^9}, 3.681664408720852*^9, {
   3.8248881746371365`*^9, 3.8248881746371365`*^9}},
 CellID->121866828,ExpressionUUID->"6e2c3d30-0f62-45cb-9e98-eb188e9e00eb"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox[
    OverscriptBox["m", 
     StyleBox[".",
      FontSize->18]], "5"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "938ef50f-d77f-4eee-83b0-b09e90de4bb0"],
 " = mass flow rate of slurry exiting the crystallizer and filter (kg/h)"
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
   3.5854996260908947`*^9, 3.5854996675812674`*^9}, {3.591366536730555*^9, 
   3.591366542559383*^9}, {3.677504139817539*^9, 3.6775041596968193`*^9}, 
   3.677504201467842*^9, {3.677504354180558*^9, 3.677504356768416*^9}, {
   3.681662763096735*^9, 3.681662793121081*^9}, {3.824888176318398*^9, 
   3.824888176318398*^9}},
 CellID->75001124,ExpressionUUID->"854f3ae2-fe7f-4f3d-9572-d391ee7aa052"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox["x", 
    StyleBox["f",
     FontSlant->"Plain"]], TraditionalForm]], "InlineMath",ExpressionUUID->
  "f69a3826-124d-4fb8-8f78-e9d00e5a0f0e"],
 " = mass fraction of water in fresh feed"
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
   3.5854996260908947`*^9, 3.5854997427245655`*^9}, 3.6816627718808265`*^9, {
   3.748459050844042*^9, 3.74845905127343*^9}, {3.824888348497669*^9, 
   3.824888349691058*^9}},
 CellID->707553748,ExpressionUUID->"330a88e8-b235-45a5-9564-ca87c9932d7f"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox["x", "5"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "f8b0e7a9-d83b-48dd-8be8-56381e48fe1b"],
 " = mass fraction of KCl in solution exiting crystallizer and filter."
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
  3.5854996260908947`*^9, 3.5854997791126466`*^9}, {3.5897387474842186`*^9, 
  3.5897387502983794`*^9}, {3.591366562001245*^9, 3.59136656437318*^9}, {
  3.677504291580162*^9, 3.677504291580162*^9}, {3.748459065911674*^9, 
  3.7484590996451674`*^9}, {3.824888358574466*^9, 3.824888363570657*^9}, {
  3.824888459716713*^9, 3.824888460156965*^9}},
 CellID->39713476,ExpressionUUID->"958db5fc-83bb-4716-a500-505bf2c360a2"],

Cell[TextData[StyleBox["Material balance around crystallizer and filter",
 FontWeight->"Bold"]], "Text",
 CellChangeTimes->{{3.6817499560586123`*^9, 3.6817499821994905`*^9}},
 CellID->456590292,ExpressionUUID->"792827c8-01f2-4c1c-8916-2375fe18e2d7"],

Cell[TextData[{
 "[4]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox[
     OverscriptBox["m", 
      StyleBox[".",
       FontSize->18]], "3"], "=", 
    RowBox[{
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "4"], "+", 
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "5"], "+", 
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], 
      StyleBox["R",
       FontSlant->"Plain"]]}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"12cdd1b5-f275-4e78-b2f6-239455933eab"]
}], "Text",
 CellChangeTimes->{{3.6817499560586123`*^9, 3.681750001535141*^9}, {
   3.684078431269308*^9, 3.6840784323301096`*^9}, 3.7484590091773486`*^9, {
   3.824887905244412*^9, 3.824887938094758*^9}, 3.8248880165406957`*^9, {
   3.8248881813451385`*^9, 3.8248881847001495`*^9}},
 CellID->32315510,ExpressionUUID->"e35e71d0-6388-411b-8900-bd77d7d1750b"],

Cell[TextData[{
 "[5]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     SubscriptBox["x", "3"], " ", 
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "3"]}], "=", 
    RowBox[{
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "4"], "+", 
     RowBox[{
      SubscriptBox["x", "5"], " ", 
      SubscriptBox[
       OverscriptBox["m", 
        StyleBox[".",
         FontSize->18]], "5"]}], "+", 
     RowBox[{
      SubscriptBox["x", "R"], " ", 
      SubscriptBox[
       OverscriptBox["m", 
        StyleBox[".",
         FontSize->18]], "R"]}]}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"0bac2dcf-0747-4ba0-97af-d226a644c575"]
}], "Text",
 CellChangeTimes->{{3.6817499560586123`*^9, 3.6817500475680237`*^9}, {
   3.748459015847375*^9, 3.748459017483867*^9}, 3.8248879096956005`*^9, {
   3.8248879398323975`*^9, 3.8248879421203227`*^9}, 3.8248880160285263`*^9, {
   3.82488818616113*^9, 3.8248881903291936`*^9}, {3.8248884645139036`*^9, 
   3.824888471190982*^9}},
 CellID->238337012,ExpressionUUID->"84f10918-a30c-4bc4-9b72-345d041d12fc"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox[
    OverscriptBox["m", 
     StyleBox[".",
      FontSize->18]], "3"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "68e87d79-556e-4559-9aef-8dc11ba42a15"],
 " = mass flow rate of solution exiting evaporator (kg/h)"
}], "Text",
 CellChangeTimes->{{3.6817412067253065`*^9, 3.681741222434366*^9}, {
  3.824888192599126*^9, 3.824888192599126*^9}},
 CellID->95418373,ExpressionUUID->"194a0ac0-ea2a-409d-8aca-b65e752dc962"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox[
    OverscriptBox["m", 
     StyleBox[".",
      FontSize->18]], 
    StyleBox["R",
     FontSlant->"Plain"]], TraditionalForm]], "InlineMath",ExpressionUUID->
  "cc97800c-29d3-4a0f-99ae-4d57f3f501da"],
 " = mass flow rate of recycle (kg/h)"
}], "Text",
 CellChangeTimes->{{3.6817502258260937`*^9, 3.681750229441516*^9}, {
  3.824888196206155*^9, 3.824888196206155*^9}},
 CellID->412767920,ExpressionUUID->"0c1525a2-5f80-432d-8a4c-6ae3da06300c"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox["x", "3"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "ebdba376-e55d-48a5-8705-15c40b90e490"],
 " = mass fraction of KCl in stream exiting evaporator"
}], "Text",
 CellChangeTimes->{{3.681741278895275*^9, 3.681741281235279*^9}, {
  3.8248884748283677`*^9, 3.824888475067008*^9}},
 CellID->500837087,ExpressionUUID->"ee7e91b4-96da-41df-adbc-13bb37995a90"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox["x", "5"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "12641612-1c5c-4785-95ce-b2be041b6d39"],
 " = mass fraction of KCl in solution exiting crystallizer and filter"
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
  3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
  3.585500310600046*^9}, {3.5855003890175314`*^9, 3.5855005039431047`*^9}, {
  3.585582909254074*^9, 3.585582909259074*^9}, {3.589738797771095*^9, 
  3.5897387998972163`*^9}, {3.6779427968265333`*^9, 3.6779427968265333`*^9}, {
  3.6816630808375034`*^9, 3.6816630815863047`*^9}, {3.8248884772789755`*^9, 
  3.8248884774316654`*^9}},
 CellID->10526691,ExpressionUUID->"b1606577-57d7-405d-b946-762d794e881a"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox["x", 
    StyleBox["R",
     FontSlant->"Plain"]], TraditionalForm]], "InlineMath",ExpressionUUID->
  "b0040fb4-ef00-40f2-a298-aed74337cf04"],
 " = mass fraction of KCl in recycle stream"
}], "Text",
 CellChangeTimes->{{3.824888480220202*^9, 3.824888480391014*^9}},
 CellID->66124469,ExpressionUUID->"3af177c8-744a-4927-af29-fa23853c31c3"],

Cell[TextData[StyleBox["Material balances around mixing point",
 FontWeight->"Bold"]], "Text",
 CellChangeTimes->{{3.6817499560586123`*^9, 3.6817500712444735`*^9}},
 CellID->294407155,ExpressionUUID->"098db17e-1566-4861-b0fc-48f0ff4447b7"],

Cell[TextData[{
 "[6]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "feed"], "+", 
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "R"]}], "=", 
    SubscriptBox[
     OverscriptBox["m", 
      StyleBox[".",
       FontSize->18]], "1"]}], TraditionalForm]], "InlineMath",ExpressionUUID->
  "892b5faf-d870-476f-b6d4-d16a52759ce7"]
}], "Text",
 CellChangeTimes->{{3.6817499560586123`*^9, 3.6817500860891*^9}, {
  3.824887960854966*^9, 3.8248879622904434`*^9}, {3.82488799656382*^9, 
  3.824887998182599*^9}, {3.824888199729035*^9, 3.8248882032241383`*^9}},
 CellID->31044092,ExpressionUUID->"2a74ae65-62e8-4d25-b593-88fdd984d290"],

Cell[TextData[{
 "[7]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{
      SubscriptBox["x", "f"], " ", 
      SubscriptBox[
       OverscriptBox["m", 
        StyleBox[".",
         FontSize->18]], "feed"]}], "+", 
     RowBox[{
      SubscriptBox["x", "R"], " ", 
      SubscriptBox[
       OverscriptBox["m", 
        StyleBox[".",
         FontSize->18]], "R"]}]}], "=", 
    RowBox[{
     SubscriptBox["x", "1"], " ", 
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "1"]}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"65639817-be2a-464f-b1f3-05891ecedee6"]
}], "Text",
 CellChangeTimes->{{3.6817499560586123`*^9, 3.681750105900635*^9}, {
   3.8248879648514433`*^9, 3.824887965783105*^9}, 3.824888001540234*^9, {
   3.8248882050421233`*^9, 3.8248882083476405`*^9}, {3.824888483362193*^9, 
   3.824888504331259*^9}},
 CellID->1858370,ExpressionUUID->"b28d5621-2466-4d63-990a-fe8285a07aa0"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox[
    OverscriptBox["m", 
     StyleBox[".",
      FontSize->18]], "1"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "3daf3125-f80b-4df5-b149-95c68775e0a7"],
 " = mass flow rate entering evaporator (kg/h)"
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
   3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
   3.5855001713570814`*^9}, {3.5897387686764307`*^9, 3.589738769402472*^9}, {
   3.59136662447125*^9, 3.591366631626281*^9}, 3.6775044126827354`*^9, {
   3.681662797913289*^9, 3.68166279822729*^9}, {3.6816628565403996`*^9, 
   3.6816628568212*^9}, {3.681741137101552*^9, 3.6817411379479537`*^9}, {
   3.8248882108561287`*^9, 3.8248882108561287`*^9}},
 CellID->63673656,ExpressionUUID->"9655f28c-1915-46a2-a969-8622e17bc647"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox["x", "1"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "f2cd4dd7-aef6-41c9-82ba-787cd3eab2fa"],
 " = mass fraction of KCl in stream entering evaporator"
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
  3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
  3.5855002107503347`*^9}, {3.5897387724716473`*^9, 3.589738773142686*^9}, {
  3.6779427933041835`*^9, 3.6779427933041835`*^9}, {3.681663054152438*^9, 
  3.68166305487204*^9}, {3.681741158668213*^9, 3.6817411673714285`*^9}, {
  3.8248884968878403`*^9, 3.8248884971701736`*^9}},
 CellID->431665790,ExpressionUUID->"a5cadc4a-c4f9-4718-a406-576eca6c18b1"],

Cell[TextData[StyleBox["Material balance around evaporator",
 FontWeight->"Bold"]], "Text",
 CellChangeTimes->{{3.6817499560586123`*^9, 3.681750134562278*^9}},
 CellID->211566143,ExpressionUUID->"7f5fd6a8-fffd-4f4b-92eb-7fb10503f72c"],

Cell[TextData[{
 "[8]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox[
     OverscriptBox["m", 
      StyleBox[".",
       FontSize->18]], "1"], "=", 
    RowBox[{
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "2"], "+", 
     SubscriptBox[
      OverscriptBox["m", 
       StyleBox[".",
        FontSize->18]], "3"]}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"4e0bcd58-83d7-4656-a759-7e15c997d6aa"]
}], "Text",
 CellChangeTimes->{{3.6817499560586123`*^9, 3.681750143581896*^9}, {
  3.824887970929822*^9, 3.8248879951063232`*^9}, {3.824888213691083*^9, 
  3.824888216402113*^9}},
 CellID->170172390,ExpressionUUID->"630c2fab-2320-40c7-b2c7-a3c9571ab639"],

Cell[TextData[StyleBox["Dependence of solubility on temperature",
 FontWeight->"Bold"]], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
   3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
   3.585500310600046*^9}, {3.5855003890175314`*^9, 3.5855005039431047`*^9}, {
   3.585582909254074*^9, 3.5855829619260864`*^9}, {3.585582993539895*^9, 
   3.585582993541895*^9}, 3.59136631790749*^9},
 CellID->86020593,ExpressionUUID->"f6e28541-b02c-421c-92a7-2ed05876f8d1"],

Cell[TextData[{
 "[9]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["s", "3"], "=", 
    RowBox[{
     RowBox[{"0.2881", " ", 
      SubscriptBox["T", "evap"]}], "+", "28.123"}]}], TraditionalForm]], 
  "InlineMath",ExpressionUUID->"8068fe72-44d9-4caa-8ae8-a825b9b66cfc"]
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
  3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
  3.585500310600046*^9}, {3.5855003890175314`*^9, 3.5855005039431047`*^9}, {
  3.585582909254074*^9, 3.5855829619260864`*^9}, {3.585582993539895*^9, 
  3.5855830343912315`*^9}, {3.585583105596304*^9, 3.5855831144128084`*^9}, {
  3.5911255509258137`*^9, 3.591125550926042*^9}, {3.59136684749867*^9, 
  3.591366856726152*^9}, {3.6775140526658397`*^9, 3.67751405288091*^9}, {
  3.681662945109398*^9, 3.6816629484810038`*^9}, {3.8248879757739067`*^9, 
  3.824888004429244*^9}},
 CellID->118180360,ExpressionUUID->"b2447e0c-7868-4388-be99-0667ab455eb2"],

Cell[TextData[{
 "[10]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["x", "3"], "=", 
    StyleBox[
     FractionBox[
      SubscriptBox["s", "3"], 
      RowBox[{
       SubscriptBox["s", "3"], " ", "+", " ", 
       RowBox[{"100", "kg"}]}]],
     FontSize->18]}], TraditionalForm]], "InlineMath",
  FontSize->16,ExpressionUUID->"6e0eff6b-f8c3-42c9-b176-c4e02937dab4"]
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
  3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
  3.585500310600046*^9}, {3.5855003890175314`*^9, 3.5855005039431047`*^9}, {
  3.585582909254074*^9, 3.5855829902697077`*^9}, {3.5855830710633287`*^9, 
  3.585583076488639*^9}, {3.591366861313905*^9, 3.5913668625323133`*^9}, {
  3.681663112610987*^9, 3.6816631146243906`*^9}, {3.824887978300943*^9, 
  3.8248880058522515`*^9}, {3.824888519650364*^9, 3.824888519805727*^9}},
 CellID->815493074,ExpressionUUID->"6241dcb0-c3e4-4945-9fd1-1b4f39ee4fe0"],

Cell[TextData[{
 "[11]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["s", "5"], "=", 
    RowBox[{
     RowBox[{"0.2881", " ", 
      SubscriptBox["T", "crys"]}], "+", "28.123"}]}], TraditionalForm]], 
  "InlineMath",ExpressionUUID->"59a8e19e-61b8-418f-8842-462d6beab923"]
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
  3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
  3.585500310600046*^9}, {3.5855003890175314`*^9, 3.5855005039431047`*^9}, {
  3.585582909254074*^9, 3.5855829902697077`*^9}, {3.5855830710633287`*^9, 
  3.5855831222592573`*^9}, {3.591366873410494*^9, 3.591366881148797*^9}, {
  3.6775140551156173`*^9, 3.6775140553160887`*^9}, {3.681662954824417*^9, 
  3.6816629570250216`*^9}, {3.8248879815208926`*^9, 3.8248880071598406`*^9}},
 CellID->257801552,ExpressionUUID->"3c83c7dc-c947-4c2f-8141-45a146a6f005"],

Cell[TextData[{
 "[12]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["x", "5"], "=", 
    RowBox[{
     SubscriptBox["x", 
      StyleBox["R",
       FontSlant->"Plain"]], "=", 
     StyleBox[
      FractionBox[
       SubscriptBox["s", "5"], 
       RowBox[{
        SubscriptBox["s", "5"], " ", "+", " ", 
        RowBox[{"100", " ", "kg"}]}]],
      FontSize->18]}]}], TraditionalForm]], "InlineMath",
  FontSize->16,ExpressionUUID->"6a6b43cb-e7af-43ac-a6c6-4efb42978ccd"]
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
  3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
  3.585500310600046*^9}, {3.5855003890175314`*^9, 3.5855005039431047`*^9}, {
  3.585582909254074*^9, 3.5855829902697077`*^9}, {3.5855830710633287`*^9, 
  3.585583142956441*^9}, {3.5897388262137213`*^9, 3.5897388367033215`*^9}, {
  3.591366886074332*^9, 3.591366890963217*^9}, {3.6816631210484037`*^9, 
  3.6816631252478113`*^9}, {3.824887984324092*^9, 3.824888008573744*^9}, {
  3.8248885123941936`*^9, 3.8248885143447933`*^9}},
 CellID->479824880,ExpressionUUID->"99db92d2-21b9-4a8a-895b-81aabbebb7f9"],

Cell[TextData[{
 "[13]\t\t",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["s", "3"], " "}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"9c7ade06-219b-4405-9be2-013a49f088d0"],
 "= solubility of KCl in 100 kg ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox[
     StyleBox["H",
      FontSlant->"Plain"], "2"], 
    StyleBox["O",
     FontSlant->"Plain"]}], TraditionalForm]], "InlineMath",ExpressionUUID->
  "d62c156d-9a8f-4be3-b019-d7a35fa1f97d"],
 " "
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
  3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
  3.585500310600046*^9}, {3.5855003890175314`*^9, 3.5855005039431047`*^9}, {
  3.585582909254074*^9, 3.5855829902697077`*^9}, {3.5855830710633287`*^9, 
  3.585583149706827*^9}, {3.5855831900711355`*^9, 3.5855832340956535`*^9}, {
  3.591366917528612*^9, 3.591366920459938*^9}, {3.6816629702430463`*^9, 
  3.6816629702430463`*^9}, {3.82488798728664*^9, 3.8248880094064374`*^9}},
 CellID->106180653,ExpressionUUID->"fc76572a-547d-436a-b4ca-218d9f87787a"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox["T", "evap"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "559a23f2-ecbe-41d8-ae6f-093ebcebb114"],
 " = evaporator operating temperature (\[Degree]C)"
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
  3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
  3.585500310600046*^9}, {3.5855003890175314`*^9, 3.5855005039431047`*^9}, {
  3.585582909254074*^9, 3.5855829902697077`*^9}, {3.5855830710633287`*^9, 
  3.585583149706827*^9}, {3.5855831900711355`*^9, 3.585583314536255*^9}, {
  3.5860124666938887`*^9, 3.5860124827478065`*^9}, {3.591366950746235*^9, 
  3.5913669507565947`*^9}, {3.681662814109519*^9, 3.681662816827924*^9}, {
  3.681664336904132*^9, 3.681664340914339*^9}},
 CellID->12057607,ExpressionUUID->"94bbad7c-7767-4231-a95e-7f1fc3432a2a"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox["s", "5"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "5754bfb6-29e9-4993-b0bd-e0146b1c0221"],
 " = solubility of KCl in 100 kg",
 " ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox[
     StyleBox["H",
      FontSlant->"Plain"], "2"], 
    StyleBox["O",
     FontSlant->"Plain"]}], TraditionalForm]], "InlineMath",ExpressionUUID->
  "b864ace9-c48f-47eb-8293-7fa9930ce5b5"],
 " "
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
   3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
   3.585500310600046*^9}, {3.5855003890175314`*^9, 3.5855005039431047`*^9}, {
   3.585582909254074*^9, 3.5855829902697077`*^9}, {3.5855830710633287`*^9, 
   3.585583149706827*^9}, {3.5855831900711355`*^9, 3.585583339705694*^9}, 
   3.59136696636545*^9},
 CellID->265036256,ExpressionUUID->"e9d78607-e404-4b77-84fb-9f7b52e990ae"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   SubscriptBox["T", "crys"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "71cbf89f-2246-4a5b-a2c2-76dcb9cff596"],
 " = crystallizer and filter operating temperature (\[Degree]C)"
}], "Text",
 CellChangeTimes->{{3.585441327992704*^9, 3.5854415906267257`*^9}, {
   3.5854996260908947`*^9, 3.5855001013230762`*^9}, {3.5855001405093174`*^9, 
   3.585500310600046*^9}, {3.5855003890175314`*^9, 3.5855005039431047`*^9}, {
   3.585582909254074*^9, 3.5855829902697077`*^9}, {3.5855830710633287`*^9, 
   3.585583149706827*^9}, {3.5855831900711355`*^9, 3.5855833653431606`*^9}, {
   3.5860124905582533`*^9, 3.5860124905582533`*^9}, 3.591442411984352*^9, 
   3.681662821093934*^9, {3.681664346530349*^9, 3.681664351468958*^9}},
 CellID->170197164,ExpressionUUID->"51a8cb8e-235b-4074-81d9-61231150d4b9"],

Cell["\<\
View the screencast video at [1] for more practice with material balances on \
a crystallizer.\
\>", "Text",
 CellChangeTimes->{{3.6817416878631163`*^9, 3.681741736408893*^9}},
 CellID->1059073054,ExpressionUUID->"73564935-42a8-44bd-83fd-1b2a472652cd"],

Cell["Reference", "Text",
 CellChangeTimes->{{3.6817416878631163`*^9, 3.6817417388700337`*^9}},
 CellID->44667022,ExpressionUUID->"db1ebe7c-bc02-4ce5-996d-f4a947f802c0"],

Cell[TextData[{
 "(1) ",
 StyleBox["Crystallizer Material Balance with Recycle",
  FontSlant->"Italic"],
 " [Video]. (Oct 29, 2013) ",
 ButtonBox["www.youtube.com/watch?v=Dnfgc4_OFhI",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["https://www.youtube.com/watch?v=Dnfgc4_OFhI"], None},
  ButtonNote->"https://www.youtube.com/watch?v=Dnfgc4_OFhI"],
 "."
}], "Text",
 CellChangeTimes->{{3.6817416878631163`*^9, 3.681741779636366*^9}, {
  3.824888640942142*^9, 3.824888642678871*^9}},
 CellID->70031784,ExpressionUUID->"a0824d67-4ab2-43b4-8cdb-95c4d5818917"],

Cell["PERMANENT CITATION", "Subsubsection",ExpressionUUID->"bb76c461-db45-44c9-a705-39d966c48fa8"],

Cell[TextData[{
 "Contributed by: ",
 ButtonBox["Rachael L. Baumann",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=Rachael+L.+\
Baumann"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=Rachael+L.+Baumann"],
 " and ",
 ButtonBox["Neil Hendren",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/search.html?query=Neil+Hendren"], 
    None},
  ButtonNote->
   "http://demonstrations.wolfram.com/search.html?query=Neil+Hendren"]
}], "Text",
 CellChangeTimes->{{3.59111658133049*^9, 3.5911166117315297`*^9}, {
   3.59111691286364*^9, 3.5911169128696404`*^9}, 3.591366984541127*^9, {
   3.677831903312129*^9, 3.677831903312129*^9}, {3.6787258765230823`*^9, 
   3.6787259253933764`*^9}, {3.681662650352392*^9, 3.6816626537249985`*^9}, {
   3.7484593097713504`*^9, 3.7484593876405077`*^9}},
 FontSize->14,
 CellID->186418100,ExpressionUUID->"325f9bf0-712e-4313-8ec3-20a22a73fa41"],

Cell[TextData[{
 "Additional contributions by: ",
 ButtonBox["Janet deGrazia",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=Janet+deGrazia"]\
, None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=Janet+deGrazia"],
 ", ",
 ButtonBox["John L. Falconer",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=John+L.+\
Falconer"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=John+L.+Falconer"],
 " and ",
 ButtonBox["Nick Bongiardina",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=Nick+\
Bongiardina"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=Nick+Bongiardina"]
}], "Text",
 CellChangeTimes->{{3.6816626388275714`*^9, 3.681662678054243*^9}, {
  3.684078725526662*^9, 3.684078725526662*^9}},
 FontSize->14,
 CellID->191356406,ExpressionUUID->"54382e8d-10dc-4898-ad91-9359fe736efb"],

Cell["\<\
(University of Colorado Boulder, Department of Chemical and Biological \
Engineering)\
\>", "Text",
 FontSize->14,
 CellID->307825922,ExpressionUUID->"1f6ac170-7e9d-4348-9351-edf9eba9bd5a"],

Cell[TextData[{
 "\[Copyright] ",
 StyleBox[ButtonBox["Wolfram Demonstrations Project & Contributors",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/"], None},
  ButtonNote->"http://demonstrations.wolfram.com/"],
  FontColor->GrayLevel[0.6]],
 "\[ThickSpace]\[ThickSpace]\[ThickSpace]|\[ThickSpace]\[ThickSpace]\
\[ThickSpace]",
 StyleBox[ButtonBox["Terms of Use",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/termsofuse.html"], None},
  ButtonNote->"http://demonstrations.wolfram.com/termsofuse.html"],
  FontColor->GrayLevel[0.6]]
}], "Text",
 CellFrame->{{0, 0}, {0, 0.5}},
 CellMargins->{{48, 48}, {20, 50}},
 CellFrameColor->GrayLevel[0.45098],
 FontFamily->"Verdana",
 FontSize->9,
 FontColor->GrayLevel[0.6],
 CellTags->"Copyright",ExpressionUUID->"9856c3ec-2d22-490e-955f-6e7b3197b5ed"],

Cell[BoxData[""], "Input",
 CellChangeTimes->{{3.7456268721904244`*^9, 3.7456268948035517`*^9}, 
   3.7456270851364527`*^9},ExpressionUUID->"9fa92ac2-225b-4c13-b460-\
8e5bbed2aedc"]
},
ScreenStyleEnvironment->"Working",
CellInsertionPointCell->None,
CellGrouping->Manual,
WindowSize->{1210, 980},
WindowMargins->{{Automatic, 311}, {32, Automatic}},
WindowElements->{
 "StatusArea", "MemoryMonitor", "MagnificationPopUp", "VerticalScrollBar", 
  "MenuBar"},
WindowTitle->"Mass Balances in Evaporative Crystallization",
DockedCells->{},
CellContext->Notebook,
FrontEndVersion->"12.0 for Microsoft Windows (64-bit) (April 8, 2019)",
StyleDefinitions->Notebook[{
   Cell[
    StyleData[StyleDefinitions -> "Default.nb"]], 
   Cell[
    StyleData["Graphics"], FontFamily -> "Arial"]}, Visible -> False, 
  FrontEndVersion -> "12.0 for Microsoft Windows (64-bit) (April 8, 2019)", 
  StyleDefinitions -> "PrivateStylesheetFormatting.nb"]
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{
 "Copyright"->{
  Cell[95916, 2034, 873, 23, 91, "Text",ExpressionUUID->"9856c3ec-2d22-490e-955f-6e7b3197b5ed",
   CellTags->"Copyright"]}
 }
*)
(*CellTagsIndex
CellTagsIndex->{
 {"Copyright", 97839, 2089}
 }
*)
(*NotebookFileOutline
Notebook[{
Cell[1546, 34, 34385, 568, 48, "DemonstrationHeader",ExpressionUUID->"0f6c982c-728a-4e7c-b5bb-e38debd5c80d"],
Cell[35934, 604, 276, 5, 67, "Subtitle",ExpressionUUID->"ce7c456e-1f7e-4573-9ed7-2c07ad00f1a9"],
Cell[36213, 611, 29563, 640, 435, "Input",ExpressionUUID->"6f2916ec-4147-44b1-b0ce-81232036e730",
 CellID->377157575],
Cell[65779, 1253, 783, 14, 86, "Text",ExpressionUUID->"abd28a3c-1615-46f9-88d1-0712bde5d8d9",
 CellID->957328473],
Cell[66565, 1269, 185, 3, 40, "Text",ExpressionUUID->"cd92d376-b5a9-4e2f-afa7-e392be73c809"],
Cell[66753, 1274, 298, 4, 34, "Text",ExpressionUUID->"383ccd0f-0803-4538-bce2-46a60589664b",
 CellID->19283394],
Cell[67054, 1280, 1053, 30, 34, "Text",ExpressionUUID->"640c58ef-1cd2-465f-a902-00e05998b6fe",
 CellID->214726250],
Cell[68110, 1312, 1006, 29, 34, "Text",ExpressionUUID->"0b7ba6bb-499a-4cea-a072-588fbdbcbc43",
 CellID->421537351],
Cell[69119, 1343, 1192, 34, 34, "Text",ExpressionUUID->"4b26d3a9-9083-4c9f-aadc-2ae1d4b3553b",
 CellID->206020753],
Cell[70314, 1379, 721, 15, 34, "Text",ExpressionUUID->"3e9cef2d-4940-4665-a451-43483d2cb461",
 CellID->32512792],
Cell[71038, 1396, 676, 15, 34, "Text",ExpressionUUID->"1cce1dd9-68f9-45fe-9f3e-94b517912b44",
 CellID->383785788],
Cell[71717, 1413, 896, 18, 34, "Text",ExpressionUUID->"6e2c3d30-0f62-45cb-9e98-eb188e9e00eb",
 CellID->121866828],
Cell[72616, 1433, 773, 16, 34, "Text",ExpressionUUID->"854f3ae2-fe7f-4f3d-9572-d391ee7aa052",
 CellID->75001124],
Cell[73392, 1451, 575, 13, 34, "Text",ExpressionUUID->"330a88e8-b235-45a5-9564-ca87c9932d7f",
 CellID->707553748],
Cell[73970, 1466, 732, 13, 34, "Text",ExpressionUUID->"958db5fc-83bb-4716-a500-505bf2c360a2",
 CellID->39713476],
Cell[74705, 1481, 249, 3, 34, "Text",ExpressionUUID->"792827c8-01f2-4c1c-8916-2375fe18e2d7",
 CellID->456590292],
Cell[74957, 1486, 973, 30, 34, "Text",ExpressionUUID->"e35e71d0-6388-411b-8900-bd77d7d1750b",
 CellID->32315510],
Cell[75933, 1518, 1149, 35, 34, "Text",ExpressionUUID->"84f10918-a30c-4bc4-9b72-345d041d12fc",
 CellID->238337012],
Cell[77085, 1555, 483, 12, 34, "Text",ExpressionUUID->"194a0ac0-ea2a-409d-8aca-b65e752dc962",
 CellID->95418373],
Cell[77571, 1569, 504, 14, 34, "Text",ExpressionUUID->"0c1525a2-5f80-432d-8a4c-6ae3da06300c",
 CellID->412767920],
Cell[78078, 1585, 420, 9, 34, "Text",ExpressionUUID->"ee7e91b4-96da-41df-adbc-13bb37995a90",
 CellID->500837087],
Cell[78501, 1596, 795, 14, 34, "Text",ExpressionUUID->"b1606577-57d7-405d-b946-762d794e881a",
 CellID->10526691],
Cell[79299, 1612, 397, 10, 34, "Text",ExpressionUUID->"3af177c8-744a-4927-af29-fa23853c31c3",
 CellID->66124469],
Cell[79699, 1624, 239, 3, 34, "Text",ExpressionUUID->"098db17e-1566-4861-b0fc-48f0ff4447b7",
 CellID->294407155],
Cell[79941, 1629, 771, 23, 34, "Text",ExpressionUUID->"2a74ae65-62e8-4d25-b593-88fdd984d290",
 CellID->31044092],
Cell[80715, 1654, 969, 30, 34, "Text",ExpressionUUID->"b28d5621-2466-4d63-990a-fe8285a07aa0",
 CellID->1858370],
Cell[81687, 1686, 853, 17, 34, "Text",ExpressionUUID->"9655f28c-1915-46a2-a969-8622e17bc647",
 CellID->63673656],
Cell[82543, 1705, 728, 13, 34, "Text",ExpressionUUID->"a5cadc4a-c4f9-4718-a406-576eca6c18b1",
 CellID->431665790],
Cell[83274, 1720, 234, 3, 34, "Text",ExpressionUUID->"7f5fd6a8-fffd-4f4b-92eb-7fb10503f72c",
 CellID->211566143],
Cell[83511, 1725, 724, 23, 34, "Text",ExpressionUUID->"630c2fab-2320-40c7-b2c7-a3c9571ab639",
 CellID->170172390],
Cell[84238, 1750, 517, 7, 34, "Text",ExpressionUUID->"f6e28541-b02c-421c-92a7-2ed05876f8d1",
 CellID->86020593],
Cell[84758, 1759, 993, 20, 36, "Text",ExpressionUUID->"b2447e0c-7868-4388-be99-0667ab455eb2",
 CellID->118180360],
Cell[85754, 1781, 991, 22, 43, "Text",ExpressionUUID->"6241dcb0-c3e4-4945-9fd1-1b4f39ee4fe0",
 CellID->815493074],
Cell[86748, 1805, 900, 18, 36, "Text",ExpressionUUID->"3c83c7dc-c947-4c2f-8141-45a146a6f005",
 CellID->257801552],
Cell[87651, 1825, 1152, 27, 43, "Text",ExpressionUUID->"99db92d2-21b9-4a8a-895b-81aabbebb7f9",
 CellID->479824880],
Cell[88806, 1854, 1090, 26, 34, "Text",ExpressionUUID->"fc76572a-547d-436a-b4ca-218d9f87787a",
 CellID->106180653],
Cell[89899, 1882, 872, 15, 36, "Text",ExpressionUUID->"94bbad7c-7767-4231-a95e-7f1fc3432a2a",
 CellID->12057607],
Cell[90774, 1899, 941, 24, 34, "Text",ExpressionUUID->"e9d78607-e404-4b77-84fb-9f7b52e990ae",
 CellID->265036256],
Cell[91718, 1925, 841, 14, 36, "Text",ExpressionUUID->"51a8cb8e-235b-4074-81d9-61231150d4b9",
 CellID->170197164],
Cell[92562, 1941, 262, 5, 34, "Text",ExpressionUUID->"73564935-42a8-44bd-83fd-1b2a472652cd",
 CellID->1059073054],
Cell[92827, 1948, 169, 2, 34, "Text",ExpressionUUID->"db1ebe7c-bc02-4ce5-996d-f4a947f802c0",
 CellID->44667022],
Cell[92999, 1952, 559, 14, 34, "Text",ExpressionUUID->"a0824d67-4ab2-43b4-8cdb-95c4d5818917",
 CellID->70031784],
Cell[93561, 1968, 98, 0, 44, "Subsubsection",ExpressionUUID->"bb76c461-db45-44c9-a705-39d966c48fa8"],
Cell[93662, 1970, 1000, 24, 29, "Text",ExpressionUUID->"325f9bf0-712e-4313-8ec3-20a22a73fa41",
 CellID->186418100],
Cell[94665, 1996, 1046, 29, 33, "Text",ExpressionUUID->"54382e8d-10dc-4898-ad91-9359fe736efb",
 CellID->191356406],
Cell[95714, 2027, 199, 5, 33, "Text",ExpressionUUID->"1f6ac170-7e9d-4348-9351-edf9eba9bd5a",
 CellID->307825922],
Cell[95916, 2034, 873, 23, 91, "Text",ExpressionUUID->"9856c3ec-2d22-490e-955f-6e7b3197b5ed",
 CellTags->"Copyright"],
Cell[96792, 2059, 181, 3, 28, "Input",ExpressionUUID->"9fa92ac2-225b-4c13-b460-8e5bbed2aedc"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature 4wpAUOErXGmT#DKqkFO07JlR *)
