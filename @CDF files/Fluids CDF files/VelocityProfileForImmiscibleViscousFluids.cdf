(* Content-type: application/vnd.wolfram.cdf.text *)

(*** Wolfram CDF File ***)
(* http://www.wolfram.com/cdf *)

(* CreatedBy='Mathematica 12.0' *)

(***************************************************************************)
(*                                                                         *)
(*                                                                         *)
(*  Under the Wolfram FreeCDF terms of use, this file and its content are  *)
(*  bound by the Creative Commons BY-SA Attribution-ShareAlike license.    *)
(*                                                                         *)
(*        For additional information concerning CDF licensing, see:        *)
(*                                                                         *)
(*         www.wolfram.com/cdf/adopting-cdf/licensing-options.html         *)
(*                                                                         *)
(*                                                                         *)
(***************************************************************************)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[      1088,         20]
NotebookDataLength[    107202,       2250]
NotebookOptionsPosition[     77527,       1640]
NotebookOutlinePosition[    103949,       2201]
CellTagsIndexPosition[    103836,       2195]
WindowTitle->Velocity Profile for Immiscible Viscous Fluids
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 ButtonBox[
  GraphicsBox[RasterBox[CompressedData["
1:
    "], {{0, 0}, {1714, 38}}, {0, 255},
    ColorFunction->RGBColor],
   ImageSize->{1714, 38},
   PlotRange->{{0, 1714}, {0, 38}}],
  Alignment->Left,
  BaseStyle->{"Hyperlink", "DemonstrationHeader"},
  ButtonData->{
    URL["http://demonstrations.wolfram.com"], None},
  ButtonNote->"http://demonstrations.wolfram.com"]], "DemonstrationHeader",Exp\
ressionUUID->"8cd2104e-a0fc-4d8a-94b4-d216bf96caf1"],

Cell["Velocity Profile for Immiscible Viscous Fluids", "DemoTitle",ExpressionUUID->"500c3d3e-a74d-4d34-9b2e-021f237458f2"],

Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`h2$$ = 0.33, $CellContext`h3$$ = 
    0.34, $CellContext`npr$$ = 1, $CellContext`\[Mu]2$$ = 
    1, $CellContext`\[Mu]3$$ = 0.5, Typeset`show$$ = True, 
    Typeset`bookmarkList$$ = {}, Typeset`bookmarkMode$$ = "Menu", 
    Typeset`animator$$, Typeset`animvar$$ = 1, Typeset`name$$ = 
    "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`npr$$], 1, ""}, {
      1 -> " velocity distribution ", 2 -> " velocity versus height "}}, {{
       Hold[$CellContext`h3$$], 0.34, 
       Style[
        Row[{
          Subscript[
           Style["h", Italic], 3], " (top)"}], 
        RGBColor[0.5, 0, 0.5]]}, 0.01, 
      Dynamic[0.99 - $CellContext`h2$$], 0.01}, {{
       Hold[$CellContext`h2$$], 0.33, 
       Style[
        Row[{
          Subscript[
           Style["h", Italic], 2], " (middle)"}], 
        RGBColor[0, 0.6, 0]]}, 0.01, 
      Dynamic[0.99 - $CellContext`h3$$], 0.01}, {{
       Hold[$CellContext`\[Mu]3$$], 0.5, 
       Style[
        Row[{
          Subscript[
           Style["\[Mu]", Italic], 3], " (top)"}], 
        RGBColor[0.5, 0, 0.5]]}, 0.1, 10, 0.1}, {{
       Hold[$CellContext`\[Mu]2$$], 1, 
       Style[
        Row[{
          Subscript["\[Mu]", 2], " (middle)"}], 
        RGBColor[0, 0.6, 0]]}, 0.1, 10, 0.1}, {
      Hold[
       Grid[{{
          Manipulate`Place[1], SpanFromLeft}, {
          Style["fluid height", Bold], 
          Manipulate`Place[2], 
          Manipulate`Place[3]}, {
          Style["viscosities", Bold], 
          Manipulate`Place[4], 
          Manipulate`Place[5]}}, Alignment -> Left]], 
      Manipulate`Dump`ThisIsNotAControl}}, Typeset`size$$ = {
    600., {185., 190.}}, Typeset`update$$ = 0, Typeset`initDone$$, 
    Typeset`skipInitDone$$ = True, $CellContext`npr$14778$$ = 
    False, $CellContext`h3$14779$$ = 0, $CellContext`h2$14780$$ = 
    0, $CellContext`\[Mu]3$14781$$ = 0, $CellContext`\[Mu]2$14782$$ = 0}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     2, StandardForm, 
      "Variables" :> {$CellContext`h2$$ = 0.33, $CellContext`h3$$ = 
        0.34, $CellContext`npr$$ = 1, $CellContext`\[Mu]2$$ = 
        1, $CellContext`\[Mu]3$$ = 0.5}, "ControllerVariables" :> {
        Hold[$CellContext`npr$$, $CellContext`npr$14778$$, False], 
        Hold[$CellContext`h3$$, $CellContext`h3$14779$$, 0], 
        Hold[$CellContext`h2$$, $CellContext`h2$14780$$, 0], 
        Hold[$CellContext`\[Mu]3$$, $CellContext`\[Mu]3$14781$$, 0], 
        Hold[$CellContext`\[Mu]2$$, $CellContext`\[Mu]2$14782$$, 0]}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> 
      Module[{$CellContext`color1$, $CellContext`color2$, \
$CellContext`color3$, $CellContext`\[Mu]1$, $CellContext`U$, \
$CellContext`h1$, $CellContext`denominator$, $CellContext`p1$, \
$CellContext`p2$}, $CellContext`color1$ = 
         Opacity[0.6, Blue]; $CellContext`color2$ = 
         Opacity[0.9, Green]; $CellContext`color3$ = 
         Opacity[0.6, Purple]; $CellContext`\[Mu]1$ = 1; $CellContext`U$ = 
         1; $CellContext`h1$ = 
         If[1 - $CellContext`h3$$ - $CellContext`h2$$ > 0.01, 
           1 - $CellContext`h3$$ - $CellContext`h2$$, 
           0.01]; $CellContext`denominator$ = $CellContext`h1$ + \
$CellContext`h2$$ + $CellContext`h3$$ - ($CellContext`h1$ + \
$CellContext`h2$$) (
           1 - $CellContext`\[Mu]3$$/$CellContext`\[Mu]2$$) + \
$CellContext`h1$ ($CellContext`\[Mu]3$$/$CellContext`\[Mu]1$ - $CellContext`\
\[Mu]3$$/$CellContext`\[Mu]2$$); $CellContext`p1$ = Show[
           
           Plot[{0, 1}, {$CellContext`x, 0, 1}, PlotStyle -> None, 
            Filling -> {2 -> $CellContext`h1$ + $CellContext`h2$$}, 
            FillingStyle -> $CellContext`color3$], 
           Plot[$CellContext`h1$ + $CellContext`h2$$, {$CellContext`x, 0, 1}, 
            PlotStyle -> None, PlotRange -> All, Filling -> $CellContext`h1$, 
            FillingStyle -> $CellContext`color2$], 
           
           Plot[$CellContext`h1$, {$CellContext`x, 0, 1}, PlotStyle -> None, 
            PlotRange -> All, Filling -> 0, 
            FillingStyle -> $CellContext`color1$], 
           
           Plot[{1.1, -0.1}, {$CellContext`x, 0, 1}, PlotStyle -> None, 
            Filling -> {1 -> 1, 2 -> 0}, FillingStyle -> GrayLevel[0.4]], 
           
           Plot[$CellContext`x (($CellContext`h3$$ $CellContext`\[Mu]2$$ + \
$CellContext`h2$$ $CellContext`\[Mu]3$$ + ($CellContext`h1$ \
$CellContext`\[Mu]2$$) $CellContext`\[Mu]3$$)/(($CellContext`U$ $CellContext`\
\[Mu]2$$) $CellContext`\[Mu]3$$)), {$CellContext`x, 0, 1}, 
            PlotStyle -> {Thick, Black}, PlotRange -> {0, $CellContext`h1$}], 
           
           Plot[(($CellContext`h3$$ $CellContext`x) $CellContext`\[Mu]2$$ + \
($CellContext`h1$ $CellContext`U$) $CellContext`\[Mu]3$$ + ($CellContext`h2$$ \
$CellContext`x) $CellContext`\[Mu]3$$ - (($CellContext`h1$ $CellContext`U$) \
$CellContext`\[Mu]2$$) $CellContext`\[Mu]3$$ + (($CellContext`h1$ \
$CellContext`x) $CellContext`\[Mu]2$$) \
$CellContext`\[Mu]3$$)/($CellContext`U$ $CellContext`\[Mu]3$$), \
{$CellContext`x, 0, 1}, PlotStyle -> {Thick, Black}, 
            PlotRange -> {$CellContext`h1$, $CellContext`h1$ + \
$CellContext`h2$$}], 
           
           Plot[(1/($CellContext`U$ $CellContext`\[Mu]2$$)) \
(($CellContext`h1$ $CellContext`U$) $CellContext`\[Mu]2$$ + \
($CellContext`h2$$ $CellContext`U$) $CellContext`\[Mu]2$$ + \
($CellContext`h3$$ $CellContext`x) $CellContext`\[Mu]2$$ - ($CellContext`h2$$ \
$CellContext`U$) $CellContext`\[Mu]3$$ + ($CellContext`h2$$ $CellContext`x) \
$CellContext`\[Mu]3$$ - (($CellContext`h1$ $CellContext`U$) \
$CellContext`\[Mu]2$$) $CellContext`\[Mu]3$$ + (($CellContext`h1$ \
$CellContext`x) $CellContext`\[Mu]2$$) $CellContext`\[Mu]3$$), \
{$CellContext`x, 0, 1}, PlotStyle -> {Thick, Black}, 
            PlotRange -> {$CellContext`h1$ + $CellContext`h2$$, \
$CellContext`h1$ + $CellContext`h2$$ + $CellContext`h3$$}], PlotRangeClipping -> 
           False, PlotRangePadding -> {None, 0.1}, Epilog -> {{Thick, 
              Arrowheads[0.035], 
              Apply[{
                If[$CellContext`h1$ > #2, 
                 
                 Arrow[{{
                   0, # $CellContext`h1$}, {((# $CellContext`h1$) \
$CellContext`U$) ($CellContext`\[Mu]3$$/($CellContext`\[Mu]1$ \
$CellContext`denominator$)), # $CellContext`h1$}}]], 
                If[$CellContext`h2$$ > #2, 
                 
                 Arrow[{{
                   0, $CellContext`h1$ + # $CellContext`h2$$}, \
{($CellContext`U$ ($CellContext`h1$ + # $CellContext`h2$$)) ($CellContext`\
\[Mu]3$$/($CellContext`\[Mu]2$$ $CellContext`denominator$)) + \
(($CellContext`h1$ $CellContext`U$) $CellContext`\[Mu]3$$) (($CellContext`\
\[Mu]2$$ - $CellContext`\[Mu]1$)/(($CellContext`\[Mu]1$ \
$CellContext`\[Mu]2$$) $CellContext`denominator$)), $CellContext`h1$ + # \
$CellContext`h2$$}}]], 
                If[# < 1, 
                 If[$CellContext`h3$$ > #2, 
                  
                  Arrow[{{
                    0, $CellContext`h1$ + $CellContext`h2$$ + # \
$CellContext`h3$$}, {$CellContext`U$ - $CellContext`U$ (($CellContext`h1$ + \
$CellContext`h2$$ + $CellContext`h3$$)/$CellContext`denominator$) + \
$CellContext`U$ (($CellContext`h1$ + $CellContext`h2$$ + # \
$CellContext`h3$$)/$CellContext`denominator$), $CellContext`h1$ + \
$CellContext`h2$$ + # $CellContext`h3$$}}]]]}& , {{0.25, 0.15}, {0.5, 0.09}, {
                0.75, 0.1}, {1, 0.03}}, {1}], 
              Arrow[{{0, 1}, {1, 1}}]}, 
             Apply[Text[
               Style[
                Row[{#, " plate"}], 17, White], {0.5, #2}, {0, -1.2}]& , {{
               "stationary", -0.1}, {"moving", 1}}, {1}]}]; $CellContext`p2$ = 
         Show[
           
           Plot[($CellContext`U$ $CellContext`y) \
($CellContext`\[Mu]3$$/($CellContext`\[Mu]1$ $CellContext`denominator$)), \
{$CellContext`y, 0, $CellContext`h1$}, PlotStyle -> {Thick, Black}, Filling -> 
            Axis, FillingStyle -> $CellContext`color1$], 
           
           Plot[($CellContext`U$ $CellContext`y) \
($CellContext`\[Mu]3$$/($CellContext`\[Mu]2$$ $CellContext`denominator$)) + \
($CellContext`h1$ $CellContext`U$) (($CellContext`\[Mu]2$$ \
$CellContext`\[Mu]3$$ - $CellContext`\[Mu]1$ \
$CellContext`\[Mu]3$$)/(($CellContext`\[Mu]1$ $CellContext`\[Mu]2$$) \
$CellContext`denominator$)), {$CellContext`y, $CellContext`h1$, \
$CellContext`h1$ + $CellContext`h2$$}, PlotStyle -> {Thick, Black}, Filling -> 
            0, FillingStyle -> $CellContext`color2$], 
           
           Plot[$CellContext`U$ - ($CellContext`h1$ + $CellContext`h2$$ + \
$CellContext`h3$$) ($CellContext`U$/$CellContext`denominator$) + \
$CellContext`U$ ($CellContext`y/$CellContext`denominator$), {$CellContext`y, \
$CellContext`h1$ + $CellContext`h2$$, $CellContext`h1$ + $CellContext`h2$$ + \
$CellContext`h3$$}, PlotStyle -> {Thick, Black}, Filling -> 0, 
            FillingStyle -> $CellContext`color3$], PlotRangePadding -> None]; 
        Show[
          
          Switch[$CellContext`npr$$, 1, $CellContext`p1$, 
           2, $CellContext`p2$], PlotRange -> {{0, 1}, {0, 1}}, Axes -> False,
           Frame -> True, FrameLabel -> 
          If[$CellContext`npr$$ == 1, N, Reverse][{
            "fluid velocity", "fraction of fluid height"}], 
          LabelStyle -> {17, Black}, ImageSize -> {600, 375}, AspectRatio -> 
          Full, ImagePadding -> {{55, 25}, {50, 25}}]], 
      "Specifications" :> {{{$CellContext`npr$$, 1, ""}, {
         1 -> " velocity distribution ", 2 -> " velocity versus height "}, 
         ControlType -> Setter, ControlPlacement -> 
         1}, {{$CellContext`h3$$, 0.34, 
          Style[
           Row[{
             Subscript[
              Style["h", Italic], 3], " (top)"}], 
           RGBColor[0.5, 0, 0.5]]}, 0.01, 
         Dynamic[0.99 - $CellContext`h2$$], 0.01, Appearance -> "Labeled", 
         ImageSize -> Small, ControlPlacement -> 
         2}, {{$CellContext`h2$$, 0.33, 
          Style[
           Row[{
             Subscript[
              Style["h", Italic], 2], " (middle)"}], 
           RGBColor[0, 0.6, 0]]}, 0.01, 
         Dynamic[0.99 - $CellContext`h3$$], 0.01, Appearance -> "Labeled", 
         ImageSize -> Small, ControlPlacement -> 
         3}, {{$CellContext`\[Mu]3$$, 0.5, 
          Style[
           Row[{
             Subscript[
              Style["\[Mu]", Italic], 3], " (top)"}], 
           RGBColor[0.5, 0, 0.5]]}, 0.1, 10, 0.1, Appearance -> "Labeled", 
         ImageSize -> Small, ControlPlacement -> 
         4}, {{$CellContext`\[Mu]2$$, 1, 
          Style[
           Row[{
             Subscript["\[Mu]", 2], " (middle)"}], 
           RGBColor[0, 0.6, 0]]}, 0.1, 10, 0.1, Appearance -> "Labeled", 
         ImageSize -> Small, ControlPlacement -> 5}, 
        Grid[{{
           Manipulate`Place[1], SpanFromLeft}, {
           Style["fluid height", Bold], 
           Manipulate`Place[2], 
           Manipulate`Place[3]}, {
           Style["viscosities", Bold], 
           Manipulate`Place[4], 
           Manipulate`Place[5]}}, Alignment -> Left]}, "Options" :> {}, 
      "DefaultOptions" :> {ControllerLinking -> True}],
     ImageSizeCache->{647., {251., 258.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    SynchronousInitialization->True,
    UndoTrackedVariables:>{Typeset`show$$, Typeset`bookmarkMode$$},
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output",
 CellID->90055845,ExpressionUUID->"daa06956-2ed8-4a4e-b979-4eb7235a7931"],

Cell["\<\
This Demonstration shows the behavior of three immiscible, incompressible \
fluids in laminar flow. The fluids are layered vertically and subjected to \
steady-state Couette flow; the top plate moves to the right at a constant \
velocity, and the bottom plate is stationary.  At each height, the black \
arrow is proportional to the velocity of the fluid at that height. You can \
change the viscosities and heights of fluids 2 and 3 with sliders. The \
velocity distribution shows the impact of fluid viscosity on the fluid's \
velocity gradient. The velocity versus height plot indicates the relative \
slopes (or velocity gradients) in each fluid. All numbers are dimensionless.\
\>", "ManipulateCaption",ExpressionUUID->"454fad80-aef9-42d1-8d15-\
3ef67cd49cca"],

Cell["THINGS TO TRY", "ManipulateCaption",
 CellFrame->{{0, 0}, {1, 0}},
 CellFrameColor->RGBColor[0.87, 0.87, 0.87],
 FontFamily->"Helvetica",
 FontSize->12,
 FontWeight->"Bold",
 FontColor->RGBColor[0.597406, 0, 0.0527047],
 CellTags->
  "ControlSuggestions",ExpressionUUID->"f66fc162-2766-47a7-8242-e05aa1ecf274"],

Cell[TextData[{
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Resize Images",
     FontFamily->"Verdana"]],ExpressionUUID->
     "219b1aa3-d4ce-4b52-98fb-0f18f36cac33"], True->Cell[TextData[StyleBox[
    "Resize Images",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]],ExpressionUUID->
     "0be52f92-3fb0-44d0-8a3d-393333b6725d"]}, Dynamic[
     CurrentValue["MouseOver"]]],
   "\"Click inside an image to reveal its orange resize frame.\\nDrag any of \
the orange resize handles to resize the image.\"",
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]],ExpressionUUID->
  "9f700e61-6b22-41e7-96f4-99b6aa5ad5f8"],
 StyleBox["\[NonBreakingSpace]\[FilledVerySmallSquare]\[NonBreakingSpace]",
  FontColor->RGBColor[0.928786, 0.43122, 0.104662]],
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Slider Zoom",
     FontFamily->"Verdana"]],ExpressionUUID->
     "7a272eaa-7348-4dbd-a974-3103d4dc0706"], True->Cell[TextData[StyleBox[
    "Slider Zoom",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]],ExpressionUUID->
     "8fd8660b-8045-48db-b4e4-fba22887eba5"]}, Dynamic[
     CurrentValue["MouseOver"]]],
   RowBox[{"\"Hold down the \"", 
     FrameBox[
     "Alt", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], 
     "\" key while moving a slider to make fine adjustments in the slider \
value.\\nHold \"", 
     FrameBox[
     "Ctrl", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" and/or \"", 
     FrameBox[
     "Shift", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" at the same time as \"", 
     FrameBox[
     "Alt", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" to make ever finer adjustments.\""}],
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]],ExpressionUUID->
  "f9481de3-35ee-4d33-a12f-aa5ede95d670"],
 StyleBox["\[NonBreakingSpace]\[FilledVerySmallSquare]\[NonBreakingSpace]",
  FontColor->RGBColor[0.928786, 0.43122, 0.104662]],
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Gamepad Controls",
     FontFamily->"Verdana"]],ExpressionUUID->
     "b93d3294-9f34-4ad4-96a9-f9c4280c9c6c"], True->Cell[TextData[StyleBox[
    "Gamepad Controls",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]],ExpressionUUID->
     "2fcfac88-4096-48a0-a7da-a74ba0d5184f"]}, Dynamic[
     CurrentValue["MouseOver"]]],
   "\"Control this Demonstration with a gamepad or other\\nhuman interface \
device connected to your computer.\"",
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]],ExpressionUUID->
  "8fd1a909-93bc-4bb4-abe6-502df8c02cb2"],
 StyleBox["\[NonBreakingSpace]\[FilledVerySmallSquare]\[NonBreakingSpace]",
  FontColor->RGBColor[0.928786, 0.43122, 0.104662]],
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Automatic Animation",
     FontFamily->"Verdana"]],ExpressionUUID->
     "5eb350b9-242b-4979-a2c7-9eb104cad201"], True->Cell[TextData[StyleBox[
    "Automatic Animation",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]],ExpressionUUID->
     "2c91615e-a297-42b1-bc69-0f12d10577bf"]}, Dynamic[
     CurrentValue["MouseOver"]]],
   RowBox[{"\"Animate a slider in this Demonstration by clicking the\"", 
     AdjustmentBox[
      Cell[
       GraphicsData[
       "CompressedBitmap", 
        "eJzzTSzJSM1NLMlMTlRwL0osyMhMLlZwyy8CCjEzMjAwcIKwAgOI/R/IhBKc\n\
/4EAyGAG0f+nTZsGwgysIJIRKsWKLAXGIHFmEpUgLADxWUAkI24jZs+eTaEt\n\
IG+wQKRmzJgBlYf5lhEA30OqWA=="], "Graphics", ImageSize -> {9, 9}, ImageMargins -> 
       0, CellBaseline -> Baseline], BoxBaselineShift -> 0.1839080459770115, 
      BoxMargins -> {{0., 0.}, {-0.1839080459770115, 0.1839080459770115}}], 
     "\"button\\nnext to the slider, and then clicking the play button that \
appears.\\nAnimate all controls by selecting \"", 
     StyleBox["Autorun", FontWeight -> "Bold"], "\" from the\"", 
     AdjustmentBox[
      Cell[
       GraphicsData[
       "CompressedBitmap", 
        "eJyNULENwyAQfEySIlMwTVJlCGRFsosokeNtqBmDBagoaZjAI1C8/8GUUUC6\n\
57h7cQ8PvU7Pl17nUav7oj/TPH7V7b2QJAUAXBkKmCPRowxICy64bRvGGNF7\n\
X8CctGoDSN4xhIDGGDhzFXwUh3/ClBKrDQPmnGXtI6u0OOd+tZBVUqy1xSaH\n\
UqiK6pPe4XdEdAz6563tx/gejuORGMxJaz8mdpJn7hc="], "Graphics", 
       ImageSize -> {10, 10}, ImageMargins -> 0, CellBaseline -> Baseline], 
      BoxBaselineShift -> 0.1839080459770115, 
      BoxMargins -> {{0., 0.}, {-0.1839080459770115, 0.1839080459770115}}], 
     "\"menu.\""}],
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]],ExpressionUUID->
  "0dfa1b60-7d81-4cfc-a761-39c78d323ee5"]
}], "ManipulateCaption",
 CellMargins->{{Inherited, Inherited}, {0, 0}},
 Deployed->True,
 FontFamily->"Verdana",
 CellTags->
  "ControlSuggestions",ExpressionUUID->"7d599a67-1a3e-4eca-a6e6-5abb8edf5661"],

Cell["DETAILS", "DetailsSection",ExpressionUUID->"6040eb1d-6f27-4ad1-b02c-75c5f16ae1e4"],

Cell[TextData[{
 "Couette flow is fluid flow through parallel plates where one plate is \
stationary and the other is moving at a constant velocity. In this \
Demonstration, the flow is at steady-state and fully developed. The Navier",
 "\[Dash]",
 "Stokes equations in the ",
 Cell[BoxData[
  FormBox["x", TraditionalForm]], "InlineMath",ExpressionUUID->
  "752c3b30-34d5-4f44-ac78-45a2724ffd73"],
 " and ",
 Cell[BoxData[
  FormBox["y", TraditionalForm]], "InlineMath",ExpressionUUID->
  "d311fc81-3557-4610-85d2-08a059611405"],
 " directions are used:"
}], "DetailNotes",
 CellID->33307729,ExpressionUUID->"611e9463-42a1-4bf7-bddb-4cd187fa3f35"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"\[Rho]", " ", 
      SubscriptBox["g", "x"]}], "-", 
     FractionBox[
      RowBox[{"\[PartialD]", "P"}], 
      RowBox[{"\[PartialD]", "x"}]], "+", 
     RowBox[{"\[Mu]", " ", 
      RowBox[{"(", 
       RowBox[{
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["x", "2"]}]], "+", 
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["y", "2"]}]], "+", 
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["z", "2"]}]]}], ")"}]}]}], "=", 
    RowBox[{"\[Rho]", " ", 
     RowBox[{"(", 
      RowBox[{
       FractionBox[
        RowBox[{"\[PartialD]", 
         SubscriptBox["u", "x"]}], 
        RowBox[{"\[PartialD]", "t"}]], "+", 
       RowBox[{
        SubscriptBox["u", "x"], " ", 
        FractionBox[
         RowBox[{"\[PartialD]", 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", "x"}]]}], "+", 
       RowBox[{
        SubscriptBox["u", "y"], " ", 
        FractionBox[
         RowBox[{"\[PartialD]", 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", "y"}]]}], "+", 
       RowBox[{
        SubscriptBox["u", "z"], " ", 
        FractionBox[
         RowBox[{"\[PartialD]", 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", "z"}]]}]}], ")"}]}]}], TraditionalForm]], 
  "InlineMath",ExpressionUUID->"f562cb9c-1946-46c1-9821-f2dd159f2c48"],
 ","
}], "DetailNotes",
 CellID->5312798,ExpressionUUID->"e551005a-9d3f-4a83-8d81-744395eb4fa9"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"\[Rho]", " ", 
      SubscriptBox["g", "y"]}], "-", 
     FractionBox[
      RowBox[{"\[PartialD]", "P"}], 
      RowBox[{"\[PartialD]", "y"}]], "+", 
     RowBox[{"\[Mu]", " ", 
      RowBox[{"(", 
       RowBox[{
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "y"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["x", "2"]}]], "+", 
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "y"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["y", "2"]}]], "+", 
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "y"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["z", "2"]}]]}], ")"}]}]}], "=", 
    RowBox[{"\[Rho]", " ", 
     RowBox[{"(", 
      RowBox[{
       FractionBox[
        RowBox[{"\[PartialD]", 
         SubscriptBox["u", "y"]}], 
        RowBox[{"\[PartialD]", "t"}]], "+", 
       RowBox[{
        SubscriptBox["u", "x"], " ", 
        FractionBox[
         RowBox[{"\[PartialD]", 
          SubscriptBox["u", "y"]}], 
         RowBox[{"\[PartialD]", "x"}]]}], "+", 
       RowBox[{
        SubscriptBox["u", "y"], " ", 
        FractionBox[
         RowBox[{"\[PartialD]", 
          SubscriptBox["u", "y"]}], 
         RowBox[{"\[PartialD]", "y"}]]}], "+", 
       RowBox[{
        SubscriptBox["u", "z"], " ", 
        FractionBox[
         RowBox[{"\[PartialD]", 
          SubscriptBox["u", "y"]}], 
         RowBox[{"\[PartialD]", "z"}]]}]}], ")"}]}]}], TraditionalForm]], 
  "InlineMath",ExpressionUUID->"adfc94eb-d9e5-4e4b-9a2a-d322e1601c30"],
 "."
}], "DetailNotes",
 CellID->93403604,ExpressionUUID->"f976dcec-269f-42a1-a2de-f9519f751a6d"],

Cell[TextData[{
 "Since flow is steady-state and there is no flow in the ",
 Cell[BoxData[
  FormBox["z", TraditionalForm]], "InlineMath",ExpressionUUID->
  "ec28f0ca-edbf-43f6-9924-e49948297973"],
 " direction, these equations simplify to:"
}], "DetailNotes",
 CellID->367809359,ExpressionUUID->"ba6ad532-902c-46da-a005-46ae2b7ab7af"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"\[Rho]", " ", 
      SubscriptBox["g", "x"]}], "-", 
     FractionBox[
      RowBox[{"\[PartialD]", "P"}], 
      RowBox[{"\[PartialD]", "x"}]], "+", 
     RowBox[{"\[Mu]", " ", 
      RowBox[{"(", 
       RowBox[{
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["x", "2"]}]], "+", 
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["y", "2"]}]]}], ")"}]}]}], "=", 
    RowBox[{"\[Rho]", " ", 
     RowBox[{"(", 
      RowBox[{
       RowBox[{
        SubscriptBox["u", "x"], " ", 
        FractionBox[
         RowBox[{"\[PartialD]", 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", "x"}]]}], "+", 
       RowBox[{
        SubscriptBox["u", "y"], " ", 
        FractionBox[
         RowBox[{"\[PartialD]", 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", "y"}]]}]}], ")"}]}]}], TraditionalForm]], 
  "InlineMath",ExpressionUUID->"f83a5a99-c717-46c9-b8c3-293606163578"],
 ","
}], "DetailNotes",
 CellID->110693609,ExpressionUUID->"cb2b7f7a-3f45-4b80-8901-803d98d5c254"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"\[Rho]", " ", 
      SubscriptBox["g", "y"]}], "-", 
     FractionBox[
      RowBox[{"\[PartialD]", "P"}], 
      RowBox[{"\[PartialD]", "y"}]], "+", 
     RowBox[{"\[Mu]", " ", 
      RowBox[{"(", 
       RowBox[{
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "y"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["x", "2"]}]], "+", 
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "y"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["y", "2"]}]]}], ")"}]}]}], "=", 
    RowBox[{"\[Rho]", " ", 
     RowBox[{"(", 
      RowBox[{
       RowBox[{
        SubscriptBox["u", "x"], " ", 
        FractionBox[
         RowBox[{"\[PartialD]", 
          SubscriptBox["u", "y"]}], 
         RowBox[{"\[PartialD]", "x"}]]}], "+", 
       RowBox[{
        SubscriptBox["u", "y"], " ", 
        FractionBox[
         RowBox[{"\[PartialD]", 
          SubscriptBox["u", "y"]}], 
         RowBox[{"\[PartialD]", "y"}]]}]}], ")"}]}]}], TraditionalForm]], 
  "InlineMath",ExpressionUUID->"6caf510f-4bc9-480e-b814-5bfdbbe9f8ed"],
 "."
}], "DetailNotes",
 CellID->98264323,ExpressionUUID->"e1536e12-ca26-4680-a66e-76b198309d86"],

Cell[TextData[{
 "Flow is in the ",
 Cell[BoxData[
  FormBox["x", TraditionalForm]], "InlineMath",ExpressionUUID->
  "fc4746aa-2603-4b11-a78e-a6322617ecc9"],
 " direction, so ",
 Cell[BoxData[
  FormBox[
   SubscriptBox["u", "y"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "cbc02373-21b8-495f-bb5a-3094e5d35c7f"],
 " drops out:"
}], "DetailNotes",
 CellID->382523754,ExpressionUUID->"b0f7d523-86d9-4b8c-88a2-04241f076d91"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"-", 
      FractionBox[
       RowBox[{"\[PartialD]", "P"}], 
       RowBox[{"\[PartialD]", "x"}]]}], "+", 
     RowBox[{"\[Mu]", " ", 
      RowBox[{"(", 
       RowBox[{
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["x", "2"]}]], "+", 
        FractionBox[
         RowBox[{
          SuperscriptBox["\[PartialD]", "2"], 
          SubscriptBox["u", "x"]}], 
         RowBox[{"\[PartialD]", 
          SuperscriptBox["y", "2"]}]]}], ")"}]}]}], "=", 
    RowBox[{"\[Rho]", " ", 
     SubscriptBox["u", "x"], " ", 
     FractionBox[
      RowBox[{"\[PartialD]", 
       SubscriptBox["u", "x"]}], 
      RowBox[{"\[PartialD]", "x"}]]}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"0fc1c43b-99e5-4426-afef-914deb71709a"],
 ","
}], "DetailNotes",
 CellID->835182146,ExpressionUUID->"430aff84-5d4a-488e-a565-d255bfad6c02"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{
     RowBox[{"\[Rho]", " ", 
      SubscriptBox["g", "y"]}], "-", 
     FractionBox[
      RowBox[{"\[PartialD]", "P"}], 
      RowBox[{"\[PartialD]", "y"}]]}], "=", "0"}], TraditionalForm]], 
  "InlineMath",ExpressionUUID->"db3a98b7-786f-4603-9f5f-5713737a2f3a"],
 "."
}], "DetailNotes",
 CellID->93162791,ExpressionUUID->"60d9f15a-d126-48e9-a472-e828ab3f107b"],

Cell[TextData[{
 "Since the flow is fully developed the velocity in the direction of flow \
does not change. It is assumed that the pressure is hydrostatic so pressure \
does not vary in the ",
 Cell[BoxData[
  FormBox["x", TraditionalForm]], "InlineMath",ExpressionUUID->
  "57d1101c-4915-4c35-b862-6116f6ebcd87"],
 " direction:"
}], "DetailNotes",
 CellID->115099086,ExpressionUUID->"8856d656-5fba-4d6f-b628-ec8b702570d5"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"\[Mu]", " ", 
     FractionBox[
      RowBox[{
       SuperscriptBox["\[PartialD]", "2"], 
       SubscriptBox["u", "x"]}], 
      RowBox[{"\[PartialD]", 
       SuperscriptBox["y", "2"]}]]}], "=", "0"}], TraditionalForm]], 
  "InlineMath",ExpressionUUID->"ec1667f3-d0c1-4a91-9224-b979f5b4479e"],
 ","
}], "DetailNotes",
 CellID->1552285,ExpressionUUID->"6e399ffd-b10b-4258-ac33-ba1ca043d3c0"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    RowBox[{"\[Rho]", " ", 
     SubscriptBox["g", "y"]}], "=", 
    FractionBox[
     RowBox[{"\[PartialD]", "P"}], 
     RowBox[{"\[PartialD]", "y"}]]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"8525b0ce-68b1-4acf-a24d-b1e6918f5703"],
 "."
}], "DetailNotes",
 CellID->739653595,ExpressionUUID->"8c8f1dc7-3bd0-49b7-8662-126db28b87fc"],

Cell[TextData[{
 "For the boundary conditions ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["u", "x"], "=", "0"}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"c7f96832-3a27-4dfc-a6b9-928a44ee5489"],
 " at ",
 Cell[BoxData[
  FormBox[
   RowBox[{"y", "=", "0"}], TraditionalForm]], "InlineMath",ExpressionUUID->
  "581cfbba-abfa-4925-bf7b-a35d53cd806d"],
 " and ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["u", "x"], "=", "U"}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"bfc7e551-69df-465a-af0b-5db8a4308a29"],
 " at ",
 Cell[BoxData[
  FormBox[
   RowBox[{"y", "=", "H"}], TraditionalForm]], "InlineMath",ExpressionUUID->
  "e17cab28-7ce0-4c54-8a9c-9725e2f49629"],
 ", the simplified Navier\[Dash]Stokes equation is:"
}], "DetailNotes",
 CellID->100146628,ExpressionUUID->"a02fe1c6-268f-4dda-b568-2d7d21734fa1"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["u", "x"], "=", 
    RowBox[{
     FractionBox["U", "H"], " ", "y"}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"5d243a9a-f175-47a2-a877-5865bd838d5e"],
 ","
}], "DetailNotes",
 CellID->328857538,ExpressionUUID->"162a9a27-3bb8-4f03-ae32-1334c559b16a"],

Cell[TextData[{
 "where ",
 Cell[BoxData[
  FormBox["\[Rho]", TraditionalForm]], "InlineMath",ExpressionUUID->
  "3792d336-13da-4305-9885-6eb16b3aa031"],
 " is density, ",
 Cell[BoxData[
  FormBox["g", TraditionalForm]], "InlineMath",ExpressionUUID->
  "d0a9c35f-a4c2-4832-a857-1396bad414f5"],
 " is gravity, ",
 Cell[BoxData[
  FormBox["P", TraditionalForm]], "InlineMath",ExpressionUUID->
  "e5a0cd4f-0f1f-4210-873e-c56ce111fdb5"],
 " is pressure, ",
 Cell[BoxData[
  FormBox["\[Mu]", TraditionalForm]], "InlineMath",ExpressionUUID->
  "af4f6f9b-364a-4633-996b-406beee9dcce"],
 " is viscosity, ",
 Cell[BoxData[
  FormBox["u", TraditionalForm]], "InlineMath",ExpressionUUID->
  "30b1d231-4bab-46ec-aa70-d8b113fdcd4a"],
 " is fluid velocity, ",
 Cell[BoxData[
  FormBox["U", TraditionalForm]], "InlineMath",ExpressionUUID->
  "40b1fd51-b819-4486-abba-6b6d466fd89d"],
 " is plate velocity, ",
 Cell[BoxData[
  FormBox["t", TraditionalForm]], "InlineMath",ExpressionUUID->
  "009d27bc-c8ee-4e9f-b91e-94bbf82104f5"],
 " is time and ",
 Cell[BoxData[
  FormBox["H", TraditionalForm]], "InlineMath",ExpressionUUID->
  "4e3a95b1-bd41-485a-9730-8e2bb22f43db"],
 " is the distance between plates."
}], "DetailNotes",
 CellID->366102625,ExpressionUUID->"284592e6-e363-4f75-b153-b53f686b682b"],

Cell["Reference", "DetailNotes",
 CellID->80080962,ExpressionUUID->"1b825366-b419-44d1-935c-7000526bb405"],

Cell[TextData[{
 "[1] B. R. Munson, T. H. Okiishi, and W. W. Huebsch, ",
 StyleBox["Fundamentals of Fluid Mechanics",
  FontSlant->"Italic"],
 ", 6th ed., Hoboken, NJ: John Wiley & Sons, 2010."
}], "DetailNotes",
 CellID->62244006,ExpressionUUID->"ddf0922d-eda1-4f5f-b23f-f46c90b468a8"],

Cell["RELATED LINKS", "RelatedLinksSection",ExpressionUUID->"b0ed89ae-8549-492a-9ecc-ab36b41fdd15"],

Cell["fluid dynamics", "RelatedLinks",
 CellID->51395686,ExpressionUUID->"9e41317a-e3d9-4df8-9499-3d607ed1f722"],

Cell["fluids", "RelatedLinks",
 CellID->101510803,ExpressionUUID->"14dd1e91-db07-4866-9e1a-37580cb650eb"],

Cell["chemical engineering", "RelatedLinks",
 CellID->600023165,ExpressionUUID->"9564f0ca-b1a1-4962-8601-ee42538ec62b"],

Cell["shear stress", "RelatedLinks",
 CellID->108149988,ExpressionUUID->"56dc8adc-aab3-4c21-b50a-863d96bbcd36"],

Cell["Couette flow", "RelatedLinks",
 CellID->263701942,ExpressionUUID->"f45d1397-7323-4411-8b96-41c22c2f31d3"],

Cell["flow between plates", "RelatedLinks",
 CellID->42667138,ExpressionUUID->"8b905ac3-6046-4a08-ad0e-305585642d39"],

Cell[BoxData[
 ButtonBox[
  PaneSelectorBox[{False->Cell[BoxData[
    GraphicsBox[RasterBox[CompressedData["
1:
      "], {{0, 0}, {194, 22}}, {0, 255},
      ColorFunction->RGBColor],
     ImageSize->{194, 22},
     PlotRange->{{0, 194}, {0, 22}}]],ExpressionUUID->
    "9076b5b1-c9e9-4ec8-9ac2-e535086e6c85"], True->Cell[BoxData[
    GraphicsBox[RasterBox[CompressedData["
1:
zfE/BGJAMMgG8SChPwC1foQX
      "], {{0, 0}, {194, 22}}, {0, 255},
      ColorFunction->RGBColor],
     ImageSize->{194, 22},
     PlotRange->{{0, 194}, {0, 22}}]],ExpressionUUID->
    "df11a649-73e9-46f0-ba41-159dac45e6d7"]}, Dynamic[
    CurrentValue["MouseOver"]]],
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/versions/source.jsp?id=\
VelocityProfileForImmiscibleViscousFluids"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/\
VelocityProfileForImmiscibleViscousFluids/\
VelocityProfileForImmiscibleViscousFluids-source.nb"]], \
"DemoSourceNotebookSection",
 CellFrame->{{0, 0}, {1, 1}},
 ShowCellBracket->False,
 CellMargins->{{48, 48}, {28, 28}},
 CellGroupingRules->{"SectionGrouping", 25},
 CellFrameMargins->{{48, 48}, {6, 8}},
 CellFrameColor->RGBColor[
  0.87, 0.87, 0.87],ExpressionUUID->"b5a0bb34-a43d-4604-946f-4f867c76a0de"],

Cell["PERMANENT CITATION", "CitationSection",ExpressionUUID->"f0d561bf-3925-4e17-b27d-f5654133530f"],

Cell[TextData[{
 "\[NonBreakingSpace]",
 ButtonBox["Duncan Chadly",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=Duncan+Chadly"],
     None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=Duncan+Chadly"],
 ", ",
 ButtonBox["Michael Wrobel",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=Michael+Wrobel"]\
, None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=Michael+Wrobel"],
 ", ",
 ButtonBox["Terra Salamida",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=Terra+Salamida"]\
, None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=Terra+Salamida"],
 " and",
 ButtonBox[" ",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=Terra+Salamida"]\
, None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=Terra+Salamida"],
 ButtonBox["Rachael L. Baumann",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/author.html?author=Rachael+L.+\
Baumann"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/author.html?author=Rachael+L.+Baumann"]
}], "Author",
 FontColor->GrayLevel[0.6],
 CellID->351548106,ExpressionUUID->"b5e0a01b-5025-40a7-aeb5-3e6597e0448d"],

Cell[TextData[{
 "\"",
 ButtonBox["Velocity Profile for Immiscible Viscous Fluids",
  BaseStyle->"SiteLink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/\
VelocityProfileForImmiscibleViscousFluids/"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/\
VelocityProfileForImmiscibleViscousFluids/"],
 "\"",
 "\[ParagraphSeparator]\[NonBreakingSpace]",
 ButtonBox["http://demonstrations.wolfram.com/\
VelocityProfileForImmiscibleViscousFluids/",
  BaseStyle->"SiteLink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/\
VelocityProfileForImmiscibleViscousFluids/"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/\
VelocityProfileForImmiscibleViscousFluids/"],
 "\[ParagraphSeparator]\[NonBreakingSpace]",
 ButtonBox["Wolfram Demonstrations Project",
  BaseStyle->"SiteLink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/"], None},
  ButtonNote->"http://demonstrations.wolfram.com/"],
 "\[ParagraphSeparator]\[NonBreakingSpace]",
 "Published: ",
 "December 13, 2013"
}], "Citations",ExpressionUUID->"b3e6671e-e605-491c-b2eb-e0a2ead54ff6"],

Cell[TextData[{
 "\[Copyright] ",
 StyleBox[ButtonBox["Wolfram Demonstrations Project & Contributors",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/"], None},
  ButtonNote->"http://demonstrations.wolfram.com/"],
  FontColor->GrayLevel[0.6]],
 "\[ThickSpace]\[ThickSpace]\[ThickSpace]|\[ThickSpace]\[ThickSpace]\
\[ThickSpace]",
 StyleBox[ButtonBox["Terms of Use",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/termsofuse.html"], None},
  ButtonNote->"http://demonstrations.wolfram.com/termsofuse.html"],
  FontColor->GrayLevel[0.6]]
}], "Text",
 CellFrame->{{0, 0}, {0, 0.5}},
 CellMargins->{{48, 48}, {20, 50}},
 CellFrameColor->GrayLevel[0.45098],
 FontFamily->"Verdana",
 FontSize->9,
 FontColor->GrayLevel[0.6],
 CellTags->"Copyright",ExpressionUUID->"38413eed-314c-4596-a49c-344b8d386e3f"]
},
Editable->False,
Saveable->False,
ScreenStyleEnvironment->"Working",
CellInsertionPointCell->None,
CellGrouping->Manual,
WindowSize->{750, 650},
WindowMargins->{{0, Automatic}, {Automatic, 0}},
WindowElements->{
 "StatusArea", "MemoryMonitor", "MagnificationPopUp", "VerticalScrollBar", 
  "MenuBar"},
WindowTitle->"Velocity Profile for Immiscible Viscous Fluids",
DockedCells->{},
CellContext->Notebook,
FrontEndVersion->"12.0 for Microsoft Windows (64-bit) (September 19, 2019)",
StyleDefinitions->Notebook[{
   Cell[
    CellGroupData[{
      Cell[
      "Demonstration Styles", "Title", 
       CellChangeTimes -> {
        3.3509184553711*^9, {3.36928902713192*^9, 3.36928902738193*^9}, {
         3.3754479092466917`*^9, 3.3754479095123196`*^9}, {
         3.375558447161495*^9, 3.375558447395873*^9}, {3.37572892702972*^9, 
         3.375728927639103*^9}}], 
      Cell[
       StyleData[StyleDefinitions -> "Default.nb"]], 
      Cell[
       CellGroupData[{
         Cell[
         "Style Environment Names", "Section", 
          CellChangeTimes -> {{3.369277974278112*^9, 3.369277974396138*^9}}], 
         Cell[
          StyleData[All, "Working"], ShowCellBracket -> False]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Notebook Options", "Section", 
          CellChangeTimes -> {{3.374865264950812*^9, 3.374865265419568*^9}}], 
         Cell[
         "  The options defined for the style below will be used at the \
Notebook level.  ", "Text"], 
         Cell[
          StyleData["Notebook"], Editable -> True, 
          PageHeaders -> {{None, None, None}, {None, None, None}}, 
          PageFooters -> {{None, None, None}, {None, None, None}}, 
          PageHeaderLines -> {False, False}, 
          PageFooterLines -> {False, False}, 
          PrintingOptions -> {
           "FacingPages" -> False, "FirstPageFooter" -> False, 
            "RestPagesFooter" -> False}, CreateCellID -> True, 
          CellFrameLabelMargins -> 6, DefaultNewInlineCellStyle -> 
          "InlineMath", DefaultInlineFormatType -> 
          "DefaultTextInlineFormatType", ShowStringCharacters -> True, 
          CacheGraphics -> False, StyleMenuListing -> None, 
          DemonstrationSite`Private`TrackCellChangeTimes -> False]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Input/Output", "Section", 
          CellChangeTimes -> {{3.3756313297791014`*^9, 
           3.3756313299509783`*^9}}], 
         Cell[
         "The cells in this section define styles used for input and output \
to the kernel.  Be careful when modifying, renaming, or removing these \
styles, because the front end associates special meanings with these style \
names.    ", "Text"], 
         Cell[
          StyleData["Input"], CellMargins -> {{48, 4}, {6, 4}}], 
         Cell[
          StyleData["Output"], CellMargins -> {{48, 4}, {6, 4}}], 
         Cell[
          StyleData["DemonstrationHeader"], Deletable -> False, 
          CellFrame -> {{0, 0}, {0, 0}}, ShowCellBracket -> False, 
          CellMargins -> {{0, 0}, {30, 0}}, 
          CellGroupingRules -> {"SectionGrouping", 20}, 
          CellHorizontalScrolling -> True, 
          CellFrameMargins -> {{0, 0}, {0, 0}}, CellFrameColor -> 
          RGBColor[0, 0, 0], StyleMenuListing -> None, Background -> 
          RGBColor[0, 0, 0]], 
         Cell[
          StyleData["ShowSource"], CellFrame -> {{0, 0}, {0, 1}}, 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 28}}, 
          CellFrameMargins -> {{48, 48}, {6, 8}}, CellFrameColor -> 
          RGBColor[0.87, 0.87, 0.87], StyleMenuListing -> None, FontFamily -> 
          "Helvetica", FontSize -> 10, FontWeight -> "Bold", FontSlant -> 
          "Plain", FontColor -> RGBColor[1, 0.42, 0]]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Basic Styles", "Section", 
          CellChangeTimes -> {{3.34971724802035*^9, 3.34971724966638*^9}, {
           3.35091840608065*^9, 3.35091840781999*^9}, {3.35091845122987*^9, 
           3.35091845356607*^9}, {3.35686681885432*^9, 3.35686681945788*^9}, {
           3.375657418186455*^9, 3.375657418452083*^9}}], 
         Cell[
          StyleData["Hyperlink"], StyleMenuListing -> None, FontColor -> 
          GrayLevel[0]], 
         Cell[
          StyleData["SiteLink"], StyleMenuListing -> None, 
          ButtonStyleMenuListing -> Automatic, FontColor -> 
          GrayLevel[0.45098], 
          ButtonBoxOptions -> {
           Active -> True, Appearance -> {Automatic, None}, 
            ButtonFunction :> (FrontEndExecute[{
               NotebookLocate[#2]}]& ), ButtonNote -> ButtonData}], 
         Cell[
          StyleData["Link"], FontColor -> GrayLevel[0.45098]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoNotes"], CellFrame -> True, 
             CellMargins -> {{0, 0}, {0, 0}}, 
             CellFrameMargins -> {{48, 48}, {4, 4}}, CellFrameColor -> 
             GrayLevel[0.99], StyleMenuListing -> None, FontFamily -> 
             "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.45098], 
             DemonstrationSite`Private`ReturnCreatesNewCell -> True], 
            Cell[
             StyleData["DemoNotes", "Printout"], 
             CellMargins -> {{24, 0}, {0, 10}}, FontSize -> 9]}, Open]], 
         Cell[
          StyleData["SnapshotsSection"], CellFrame -> {{0, 0}, {0, 2}}, 
          ShowCellBracket -> False, ShowGroupOpener -> True, 
          CellMargins -> {{48, 48}, {10, 30}}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], DefaultNewCellStyle -> 
          "SnapshotCaption", StyleMenuListing -> None, FontFamily -> 
          "Verdana", FontSize -> 12, FontColor -> GrayLevel[0.45098], 
          PrivateCellOptions -> {"DefaultCellGroupOpen" -> False}], 
         Cell[
          StyleData[
          "SnapshotCaption", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["SnapshotOutput"], ShowCellBracket -> False, 
             CellMargins -> {{48, 10}, {5, 7}}, Evaluatable -> True, 
             CellGroupingRules -> "InputGrouping", PageBreakWithin -> False, 
             GroupPageBreakWithin -> False, DefaultFormatType -> 
             DefaultInputFormatType, ShowAutoStyles -> True, 
             "TwoByteSyntaxCharacterAutoReplacement" -> True, 
             HyphenationOptions -> {
              "HyphenationCharacter" -> "\[Continuation]"}, 
             AutoItalicWords -> {}, LanguageCategory -> "Mathematica", 
             FormatType -> InputForm, NumberMarks -> True, 
             LinebreakAdjustments -> {0.85, 2, 10, 0, 1}, CounterIncrements -> 
             "Input", MenuSortingValue -> 1500, MenuCommandKey -> "9", 
             DemonstrationSite`Private`StripStyleOnPaste -> True], 
            Cell[
             StyleData["SnapshotOuput", "Printout"], 
             CellMargins -> {{39, 0}, {4, 6}}, 
             LinebreakAdjustments -> {0.85, 2, 10, 1, 1}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoTitle"], Deletable -> False, ShowCellBracket -> 
             False, CellMargins -> {{48, 48}, {22, 10}}, 
             CellGroupingRules -> {"SectionGrouping", 20}, StyleMenuListing -> 
             None, FontFamily -> "Verdana", FontSize -> 20, FontWeight -> 
             "Bold", FontColor -> RGBColor[0.597406, 0, 0.0527047], 
             Background -> GrayLevel[1]], 
            Cell[
             StyleData["DemoName", "Printout"], 
             CellMargins -> {{24, 8}, {8, 27}}, 
             HyphenationOptions -> {"HyphenationCharacter" -> "-"}, FontSize -> 
             16]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DetailsSection"], CellFrame -> {{0, 0}, {1, 0}}, 
             ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 28}}, 
             CellGroupingRules -> {"SectionGrouping", 25}, 
             CellFrameMargins -> {{48, 48}, {6, 8}}, CellFrameColor -> 
             RGBColor[0.87, 0.87, 0.87], StyleMenuListing -> None, FontFamily -> 
             "Helvetica", FontSize -> 12, FontWeight -> "Bold", FontColor -> 
             RGBColor[0.597406, 0, 0.0527047]], 
            Cell[
             StyleData["DetailsSection", "Printout"], 
             CellMargins -> {{12, 0}, {0, 16}}, PageBreakBelow -> False, 
             FontSize -> 12]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoSection"], CellFrame -> {{0, 0}, {1, 0}}, 
             ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 28}}, 
             CellGroupingRules -> {"SectionGrouping", 30}, 
             CellFrameMargins -> {{48, 48}, {6, 8}}, CellFrameColor -> 
             RGBColor[0.87, 0.87, 0.87], StyleMenuListing -> None, FontFamily -> 
             "Helvetica", FontSize -> 12, FontWeight -> "Bold", FontColor -> 
             RGBColor[0.597406, 0, 0.0527047]], 
            Cell[
             StyleData["DemoSection", "Printout"], 
             CellMargins -> {{12, 0}, {0, 16}}, PageBreakBelow -> False, 
             FontSize -> 12]}, Open]], 
         Cell[
          StyleData["ManipulateSection"], CellFrame -> {{0, 0}, {0, 2}}, 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {10, 30}}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], StyleMenuListing -> None, 
          FontFamily -> "Verdana", FontSize -> 12], 
         Cell[
          StyleData["ManipulateCaptionSection"], 
          CellFrame -> {{0, 0}, {0, 2}}, ShowCellBracket -> False, 
          CellMargins -> {{48, 48}, {10, 30}}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], DefaultNewCellStyle -> 
          "ManipulateCaption", StyleMenuListing -> None, FontFamily -> 
          "Verdana", FontSize -> 12, FontColor -> GrayLevel[0.45098]], 
         Cell[
          StyleData["ManipulateCaption"], ShowCellBracket -> False, 
          CellMargins -> {{48, 48}, {10, 16}}, StyleMenuListing -> None, 
          FontFamily -> "Verdana", FontSize -> 12, FontColor -> GrayLevel[0], 
          DemonstrationSite`Private`ReturnCreatesNewCell -> True], 
         Cell[
          StyleData[
          "SeeAlsoSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, DefaultNewCellStyle -> "SeeAlso"], 
         Cell[
          StyleData["SeeAlso", StyleDefinitions -> StyleData["DemoNotes"]], 
          CellDingbat -> 
          Cell["\[FilledSmallSquare]", FontColor -> 
            RGBColor[0.928786, 0.43122, 0.104662]], ShowCellBracket -> False, 
          FontColor -> GrayLevel[0.45098]], 
         Cell[
          StyleData[
          "RelatedLinksSection", StyleDefinitions -> 
           StyleData["DemoSection"]], ShowCellBracket -> False, 
          DefaultNewCellStyle -> "RelatedLinks"], 
         Cell[
          StyleData[
          "RelatedLinks", StyleDefinitions -> StyleData["DemoNotes"], 
           FontColor -> RGBColor[0.928786, 0.43122, 0.104662]], 
          ShowCellBracket -> False, FontColor -> GrayLevel[0.45098]], 
         Cell[
          StyleData[
          "CategoriesSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, DefaultNewCellStyle -> "Categories"], 
         Cell[
          StyleData["Categories", StyleDefinitions -> StyleData["DemoNotes"]],
           ShowCellBracket -> False], 
         Cell[
          StyleData[
          "AuthorSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {4, 18}}, 
          CellElementSpacings -> {"CellMinHeight" -> 3}, 
          CellFrameMargins -> {{48, 48}, {6, 3}}, DefaultNewCellStyle -> 
          "Author", FontSize -> 1, FontColor -> GrayLevel[1]], 
         Cell[
          StyleData[
          "Author", StyleDefinitions -> StyleData["DemoNotes"], FontColor -> 
           GrayLevel[0.64]], ShowCellBracket -> False], 
         Cell[
          StyleData[
          "DetailNotes", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False, FontColor -> GrayLevel[0]], 
         Cell[
          StyleData[
          "CitationSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 14}}, 
          DefaultNewCellStyle -> "Categories"], 
         Cell[
          StyleData["Citations", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False, ParagraphSpacing -> {0, 6}], 
         Cell[
          StyleData[
          "RevisionSection", StyleDefinitions -> StyleData["DemoSection"]], 
          DefaultNewCellStyle -> "RevisionNotes"], 
         Cell[
          StyleData[
          "RevisionNotes", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Specific Styles", "Section", 
          CellChangeTimes -> {{3.34971724802035*^9, 3.34971724966638*^9}, {
           3.35091840608065*^9, 3.35091840781999*^9}, {3.35091845122987*^9, 
           3.35091845356607*^9}, {3.36230868322317*^9, 3.36230868335672*^9}, {
           3.36928857618576*^9, 3.36928857640452*^9}, {3.3737586217185173`*^9,
            3.373758622077897*^9}}], 
         Cell[
          StyleData["InitializationSection"], CellFrame -> {{0, 0}, {0, 2}}, 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {10, 30}}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], StyleMenuListing -> None, 
          FontFamily -> "Verdana", FontSize -> 12, FontColor -> 
          GrayLevel[0.45098]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["AnchorBar"], ShowCellBracket -> False, 
             CellMargins -> {{48, 44}, {3, 6}}, StyleMenuListing -> None, 
             FontFamily -> "Verdana", FontSize -> 9, FontColor -> 
             GrayLevel[0.5]], 
            Cell[
             StyleData["AnchorBar", "Presentation"], FontSize -> 18], 
            Cell[
             StyleData["AnchorBar", "SlideShow"], StyleMenuListing -> None], 
            Cell[
             StyleData["AnchorBar", "Printout"], FontSize -> 9]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["AnchorLink"], StyleMenuListing -> None, 
             ButtonStyleMenuListing -> Automatic, FontColor -> 
             RGBColor[0.5, 0.5, 0.5], 
             ButtonBoxOptions -> {
              Active -> True, ButtonFunction :> (FrontEndExecute[{
                  FrontEnd`NotebookLocate[#2]}]& ), ButtonNote -> 
               ButtonData}], 
            Cell[
             StyleData["AnchorLink", "Printout"], 
             FontVariations -> {"Underline" -> False}, FontColor -> 
             GrayLevel[0]]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["GamePadStatus"], ShowCellBracket -> False, 
             CellMargins -> {{48, 48}, {5, 5}}, StyleMenuListing -> None, 
             FontFamily -> "Verdana", FontSize -> 10], 
            Cell[
             StyleData["GamePadStatus", "Printout"], 
             CellMargins -> {{24, 0}, {0, 10}}, FontSize -> 9]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoInstruction"], CellMargins -> {{48, 48}, {5, 5}}, 
             CellFrameLabelMargins -> 2, MenuSortingValue -> 800, 
             MenuCommandKey -> "8", StyleMenuListing -> None, FontFamily -> 
             "Verdana", FontSize -> 11, Background -> RGBColor[1, 0.85, 0.5], 
             DemonstrationSite`Private`ReturnCreatesNewCell -> True], 
            Cell[
             StyleData["DemoInstruction", "Printout"], 
             CellMargins -> {{24, 0}, {0, 10}}, Hyphenation -> True, 
             HyphenationOptions -> {"HyphenationCharacter" -> "-"}, 
             LineSpacing -> {1., 2, 2.}, FontSize -> 9]}, Open]], 
         Cell[
          StyleData[
          "ImplementationSection", StyleDefinitions -> 
           StyleData["DemoSection"]], Deletable -> True, DefaultNewCellStyle -> 
          "ImplementationNotes"], 
         Cell[
          StyleData[
          "ImplementationNotes", StyleDefinitions -> StyleData["DemoNotes"]]], 
         Cell[
          StyleData[
          "StatusSection", StyleDefinitions -> StyleData["DemoSection"]], 
          DefaultNewCellStyle -> "StatusNotes"], 
         Cell[
          StyleData[
          "StatusNotes", StyleDefinitions -> StyleData["DemoNotes"]]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["SectionGloss"], StyleMenuListing -> None, FontSize -> 
             0.85 Inherited, FontWeight -> "Plain", FontColor -> 
             GrayLevel[0.6]], 
            Cell[
             StyleData["SectionGloss", "Printout"]]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["InlineFormula"], 
             "TwoByteSyntaxCharacterAutoReplacement" -> True, 
             HyphenationOptions -> {
              "HyphenationCharacter" -> "\[Continuation]"}, LanguageCategory -> 
             "Formula", AutoSpacing -> True, ScriptLevel -> 1, 
             AutoMultiplicationSymbol -> False, SingleLetterItalics -> False, 
             SpanMaxSize -> 1, StyleMenuListing -> None, FontFamily -> 
             "Courier", FontSize -> 1.05 Inherited, 
             ButtonBoxOptions -> {Appearance -> {Automatic, None}}, 
             FractionBoxOptions -> {BaseStyle -> {SpanMaxSize -> Automatic}}, 
             GridBoxOptions -> {
              GridBoxItemSize -> {
                "Columns" -> {{Automatic}}, "ColumnsIndexed" -> {}, 
                 "Rows" -> {{1.}}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData["InlineFormula", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["InlineOutput"], CellHorizontalScrolling -> True, 
             "TwoByteSyntaxCharacterAutoReplacement" -> True, 
             HyphenationOptions -> {
              "HyphenationCharacter" -> "\[Continuation]"}, LanguageCategory -> 
             None, AutoMultiplicationSymbol -> False, StyleMenuListing -> 
             None, FontFamily -> "Courier", FontSize -> 1.05 Inherited], 
            Cell[
             StyleData["InlineOutput", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["InlineMath"], DefaultFormatType -> 
             "DefaultTextFormatType", DefaultInlineFormatType -> 
             "TraditionalForm", LanguageCategory -> "Formula", AutoSpacing -> 
             True, ScriptLevel -> 1, AutoMultiplicationSymbol -> False, 
             SingleLetterItalics -> True, SpanMaxSize -> DirectedInfinity[1], 
             StyleMenuListing -> None, FontFamily -> "Times", FontSize -> 
             1.05 Inherited, 
             ButtonBoxOptions -> {Appearance -> {Automatic, None}}, 
             GridBoxOptions -> {
              GridBoxItemSize -> {
                "Columns" -> {{Automatic}}, "ColumnsIndexed" -> {}, 
                 "Rows" -> {{1.}}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData["InlineMath", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["TableBase"], CellMargins -> {{48, 48}, {4, 4}}, 
             SpanMaxSize -> 1, StyleMenuListing -> None, FontFamily -> 
             "Courier", FontSize -> 11, 
             ButtonBoxOptions -> {Appearance -> {Automatic, None}}, 
             GridBoxOptions -> {
              GridBoxAlignment -> {
                "Columns" -> {{Left}}, "ColumnsIndexed" -> {}, 
                 "Rows" -> {{Baseline}}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData["TableBase", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}, FontSize -> 9]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData[
             "1ColumnTableMod", StyleDefinitions -> StyleData["TableBase"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.04], {
                    Scaled[0.966]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], 
                   Offset[0.126], {
                    Offset[0.77]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.4]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData[
             "1ColumnTableMod", "Printout", StyleDefinitions -> 
              StyleData["TableBase", "Printout"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.078], {
                    Scaled[0.922]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], {
                    Offset[0.56]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.56]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData[
             "2ColumnTableMod", StyleDefinitions -> StyleData["TableBase"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.05], 
                   Scaled[0.41], {
                    Scaled[0.565]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], 
                   Offset[0.14], {
                    Offset[0.77]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.4]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData[
             "2ColumnTableMod", "Printout", StyleDefinitions -> 
              StyleData["TableBase", "Printout"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.079], 
                   Scaled[0.363], {
                    Scaled[0.558]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], {
                    Offset[0.56]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.56]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData[
             "3ColumnTableMod", StyleDefinitions -> StyleData["TableBase"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.04], 
                   Scaled[0.266], 
                   Scaled[0.26], {
                    Scaled[0.44]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}}, 
                 "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], 
                   Offset[0.14], {
                    Offset[0.77]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.4]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData[
             "3ColumnTableMod", "Printout", StyleDefinitions -> 
              StyleData["TableBase", "Printout"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.08], 
                   Scaled[0.25], 
                   Scaled[0.25], {
                    Scaled[0.42]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}}, 
                 "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], {
                    Offset[0.56]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.56]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["TableText"], Deletable -> False, StyleMenuListing -> 
             None, FontFamily -> "Verdana", FontSize -> 0.952 Inherited], 
            Cell[
             StyleData["TableText", "Printout"], 
             CellMargins -> {{24, 0}, {0, 8}}, Hyphenation -> True, 
             HyphenationOptions -> {"HyphenationCharacter" -> "-"}, 
             LineSpacing -> {1., 2, 2.}]}, Open]], 
         Cell[
          StyleData["Continuation"], FontColor -> GrayLevel[1]]}, Closed]]}, 
     Open]]}, Visible -> False, FrontEndVersion -> 
  "12.0 for Microsoft Windows (64-bit) (September 19, 2019)", 
  StyleDefinitions -> "Default.nb"]
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{
 "ControlSuggestions"->{
  Cell[48841, 863, 316, 8, 70, "ManipulateCaption",ExpressionUUID->"f66fc162-2766-47a7-8242-e05aa1ecf274",
   CellTags->"ControlSuggestions"],
  Cell[49160, 873, 5233, 112, 70, "ManipulateCaption",ExpressionUUID->"7d599a67-1a3e-4eca-a6e6-5abb8edf5661",
   CellTags->"ControlSuggestions"]},
 "Copyright"->{
  Cell[76650, 1615, 873, 23, 70, "Text",ExpressionUUID->"38413eed-314c-4596-a49c-344b8d386e3f",
   CellTags->"Copyright"]}
 }
*)
(*CellTagsIndex
CellTagsIndex->{
 {"ControlSuggestions", 103383, 2184},
 {"Copyright", 103689, 2189}
 }
*)
(*NotebookFileOutline
Notebook[{
Cell[1548, 34, 34446, 569, 70, "DemonstrationHeader",ExpressionUUID->"8cd2104e-a0fc-4d8a-94b4-d216bf96caf1"],
Cell[35997, 605, 122, 0, 70, "DemoTitle",ExpressionUUID->"500c3d3e-a74d-4d34-9b2e-021f237458f2"],
Cell[36122, 607, 11938, 241, 70, "Output",ExpressionUUID->"daa06956-2ed8-4a4e-b979-4eb7235a7931",
 CellID->90055845],
Cell[48063, 850, 775, 11, 70, "ManipulateCaption",ExpressionUUID->"454fad80-aef9-42d1-8d15-3ef67cd49cca"],
Cell[48841, 863, 316, 8, 70, "ManipulateCaption",ExpressionUUID->"f66fc162-2766-47a7-8242-e05aa1ecf274",
 CellTags->"ControlSuggestions"],
Cell[49160, 873, 5233, 112, 70, "ManipulateCaption",ExpressionUUID->"7d599a67-1a3e-4eca-a6e6-5abb8edf5661",
 CellTags->"ControlSuggestions"],
Cell[54396, 987, 88, 0, 70, "DetailsSection",ExpressionUUID->"6040eb1d-6f27-4ad1-b02c-75c5f16ae1e4"],
Cell[54487, 989, 648, 15, 70, "DetailNotes",ExpressionUUID->"611e9463-42a1-4bf7-bddb-4cd187fa3f35",
 CellID->33307729],
Cell[55138, 1006, 1881, 59, 70, "DetailNotes",ExpressionUUID->"e551005a-9d3f-4a83-8d81-744395eb4fa9",
 CellID->5312798],
Cell[57022, 1067, 1882, 59, 70, "DetailNotes",ExpressionUUID->"f976dcec-269f-42a1-a2de-f9519f751a6d",
 CellID->93403604],
Cell[58907, 1128, 335, 7, 70, "DetailNotes",ExpressionUUID->"ba6ad532-902c-46da-a005-46ae2b7ab7af",
 CellID->367809359],
Cell[59245, 1137, 1356, 43, 70, "DetailNotes",ExpressionUUID->"cb2b7f7a-3f45-4b80-8901-803d98d5c254",
 CellID->110693609],
Cell[60604, 1182, 1355, 43, 70, "DetailNotes",ExpressionUUID->"e1536e12-ca26-4680-a66e-76b198309d86",
 CellID->98264323],
Cell[61962, 1227, 431, 12, 70, "DetailNotes",ExpressionUUID->"b0f7d523-86d9-4b8c-88a2-04241f076d91",
 CellID->382523754],
Cell[62396, 1241, 1044, 33, 70, "DetailNotes",ExpressionUUID->"430aff84-5d4a-488e-a565-d255bfad6c02",
 CellID->835182146],
Cell[63443, 1276, 429, 13, 70, "DetailNotes",ExpressionUUID->"60d9f15a-d126-48e9-a472-e828ab3f107b",
 CellID->93162791],
Cell[63875, 1291, 424, 9, 70, "DetailNotes",ExpressionUUID->"8856d656-5fba-4d6f-b628-ec8b702570d5",
 CellID->115099086],
Cell[64302, 1302, 460, 14, 70, "DetailNotes",ExpressionUUID->"6e399ffd-b10b-4258-ac33-ba1ca043d3c0",
 CellID->1552285],
Cell[64765, 1318, 400, 12, 70, "DetailNotes",ExpressionUUID->"8c8f1dc7-3bd0-49b7-8662-126db28b87fc",
 CellID->739653595],
Cell[65168, 1332, 855, 25, 70, "DetailNotes",ExpressionUUID->"a02fe1c6-268f-4dda-b568-2d7d21734fa1",
 CellID->100146628],
Cell[66026, 1359, 333, 10, 70, "DetailNotes",ExpressionUUID->"162a9a27-3bb8-4f03-ae32-1334c559b16a",
 CellID->328857538],
Cell[66362, 1371, 1284, 35, 70, "DetailNotes",ExpressionUUID->"284592e6-e363-4f75-b153-b53f686b682b",
 CellID->366102625],
Cell[67649, 1408, 106, 1, 70, "DetailNotes",ExpressionUUID->"1b825366-b419-44d1-935c-7000526bb405",
 CellID->80080962],
Cell[67758, 1411, 286, 6, 70, "DetailNotes",ExpressionUUID->"ddf0922d-eda1-4f5f-b23f-f46c90b468a8",
 CellID->62244006],
Cell[68047, 1419, 99, 0, 70, "RelatedLinksSection",ExpressionUUID->"b0ed89ae-8549-492a-9ecc-ab36b41fdd15"],
Cell[68149, 1421, 112, 1, 70, "RelatedLinks",ExpressionUUID->"9e41317a-e3d9-4df8-9499-3d607ed1f722",
 CellID->51395686],
Cell[68264, 1424, 105, 1, 70, "RelatedLinks",ExpressionUUID->"14dd1e91-db07-4866-9e1a-37580cb650eb",
 CellID->101510803],
Cell[68372, 1427, 119, 1, 70, "RelatedLinks",ExpressionUUID->"9564f0ca-b1a1-4962-8601-ee42538ec62b",
 CellID->600023165],
Cell[68494, 1430, 111, 1, 70, "RelatedLinks",ExpressionUUID->"56dc8adc-aab3-4c21-b50a-863d96bbcd36",
 CellID->108149988],
Cell[68608, 1433, 111, 1, 70, "RelatedLinks",ExpressionUUID->"f45d1397-7323-4411-8b96-41c22c2f31d3",
 CellID->263701942],
Cell[68722, 1436, 117, 1, 70, "RelatedLinks",ExpressionUUID->"8b905ac3-6046-4a08-ad0e-305585642d39",
 CellID->42667138],
Cell[68842, 1439, 5169, 96, 70, "DemoSourceNotebookSection",ExpressionUUID->"b5a0bb34-a43d-4604-946f-4f867c76a0de",
 CellGroupingRules->{"SectionGrouping", 25}],
Cell[74014, 1537, 100, 0, 70, "CitationSection",ExpressionUUID->"f0d561bf-3925-4e17-b27d-f5654133530f"],
Cell[74117, 1539, 1427, 42, 70, "Author",ExpressionUUID->"b5e0a01b-5025-40a7-aeb5-3e6597e0448d",
 CellID->351548106],
Cell[75547, 1583, 1100, 30, 70, "Citations",ExpressionUUID->"b3e6671e-e605-491c-b2eb-e0a2ead54ff6"],
Cell[76650, 1615, 873, 23, 70, "Text",ExpressionUUID->"38413eed-314c-4596-a49c-344b8d386e3f",
 CellTags->"Copyright"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature 2RajKRXHqSweXu7e6T91OPxk *)
