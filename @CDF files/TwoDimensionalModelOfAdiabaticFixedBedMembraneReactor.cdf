(* Content-type: application/vnd.wolfram.cdf.text *)

(*** Wolfram CDF File ***)
(* http://www.wolfram.com/cdf *)

(* CreatedBy='Mathematica 12.0' *)

(***************************************************************************)
(*                                                                         *)
(*                                                                         *)
(*  Under the Wolfram FreeCDF terms of use, this file and its content are  *)
(*  bound by the Creative Commons BY-SA Attribution-ShareAlike license.    *)
(*                                                                         *)
(*        For additional information concerning CDF licensing, see:        *)
(*                                                                         *)
(*         www.wolfram.com/cdf/adopting-cdf/licensing-options.html         *)
(*                                                                         *)
(*                                                                         *)
(***************************************************************************)

(*CacheID: 234*)
(* Internal cache information:
NotebookFileLineBreakTest
NotebookFileLineBreakTest
NotebookDataPosition[      1088,         20]
NotebookDataLength[    108126,       2230]
NotebookOptionsPosition[     79180,       1630]
NotebookOutlinePosition[    105790,       2195]
CellTagsIndexPosition[    105649,       2188]
WindowTitle->Two-Dimensional Model of Adiabatic Fixed-Bed Membrane Reactor
WindowFrame->Normal*)

(* Beginning of Notebook Content *)
Notebook[{
Cell[BoxData[
 ButtonBox[
  GraphicsBox[RasterBox[CompressedData["
1:
    "], {{0, 0}, {1714, 38}}, {0, 255},
    ColorFunction->RGBColor],
   ImageSize->{1714, 38},
   PlotRange->{{0, 1714}, {0, 38}}],
  Alignment->Left,
  BaseStyle->{"Hyperlink", "DemonstrationHeader"},
  ButtonData->{
    URL["http://demonstrations.wolfram.com"], None},
  ButtonNote->"http://demonstrations.wolfram.com"]], "DemonstrationHeader",Exp\
ressionUUID->"c8622e65-97ca-4173-959d-4347ec69b0b6"],

Cell["\<\
Two-Dimensional Model of Adiabatic Fixed-Bed Membrane Reactor\
\>", "DemoTitle",ExpressionUUID->"9f848c06-d2cf-40b1-bc2b-6452142f3bca"],

Cell[BoxData[
 TagBox[
  StyleBox[
   DynamicModuleBox[{$CellContext`Ea$$ = 3000., $CellContext`g$$ = 
    4, $CellContext`umax$$ = 0.2, $CellContext`Z$$ = 
    20, $CellContext`\[ScriptCapitalD]$$ = 
    0.0001, $CellContext`\[CapitalDelta]H$$ = 
    30000., $CellContext`\[Lambda]$$ = 0.001, Typeset`show$$ = True, 
    Typeset`bookmarkList$$ = {}, Typeset`bookmarkMode$$ = "Menu", 
    Typeset`animator$$, Typeset`animvar$$ = 1, Typeset`name$$ = 
    "\"untitled\"", Typeset`specs$$ = {{{
       Hold[$CellContext`g$$], 4, ""}, {
      1 -> "axial concentration", 2 -> "radial concentration", 3 -> 
       "axial temperature", 4 -> "radial temperature", 5 -> 
       "% \!\(\*SubscriptBox[\(H\), \(2\)]\) recovery"}}, {{
       Hold[$CellContext`\[CapitalDelta]H$$], 30000., "enthalpy (kJ/mol)"}, 
      10000., 50000., 5000.}, {{
       Hold[$CellContext`\[Lambda]$$], 0.001, "thermal conductivity (W/m K)"},
       0.0001, 0.01}, {{
       Hold[$CellContext`umax$$], 0.2, "fluid velocity (m/s)"}, 0.1, 0.5, 
      0.01}, {{
       Hold[$CellContext`Ea$$], 3000., "activation energy (kJ/mol)"}, 2000., 
      3500., 500.}, {{
       Hold[$CellContext`Z$$], 20, "distance along axis (% L)"}, 2, 100., 
      2}, {{
       Hold[$CellContext`\[ScriptCapitalD]$$], 0.0001, 
       "diffusivity (\!\(\*SuperscriptBox[\(m\), \(2\)]\)/s)"}, 1.*^-6, 
      0.0001, 1.*^-6}, {
      Hold[
       Grid[{{
          Manipulate`Place[1], 
          Manipulate`Place[2]}, {
          Manipulate`Place[3], 
          Manipulate`Place[4]}, {
          Manipulate`Place[5], 
          Manipulate`Place[6]}}, Alignment -> Left]], 
      Manipulate`Dump`ThisIsNotAControl}}, Typeset`size$$ = {
    554., {198., 202.}}, Typeset`update$$ = 0, Typeset`initDone$$, 
    Typeset`skipInitDone$$ = True}, 
    DynamicBox[Manipulate`ManipulateBoxes[
     2, StandardForm, 
      "Variables" :> {$CellContext`Ea$$ = 3000., $CellContext`g$$ = 
        4, $CellContext`umax$$ = 0.2, $CellContext`Z$$ = 
        20, $CellContext`\[ScriptCapitalD]$$ = 
        0.0001, $CellContext`\[CapitalDelta]H$$ = 
        30000., $CellContext`\[Lambda]$$ = 0.001}, 
      "ControllerVariables" :> {}, 
      "OtherVariables" :> {
       Typeset`show$$, Typeset`bookmarkList$$, Typeset`bookmarkMode$$, 
        Typeset`animator$$, Typeset`animvar$$, Typeset`name$$, 
        Typeset`specs$$, Typeset`size$$, Typeset`update$$, Typeset`initDone$$,
         Typeset`skipInitDone$$}, "Body" :> 
      Module[{$CellContext`\[Epsilon]$ = 10.^(-6), $CellContext`L$ = 
         1, $CellContext`cAo$ = 1., $CellContext`kperm$ = 
         1, $CellContext`\[Rho]$ = 0.26, $CellContext`Cp$ = 
         1000., $CellContext`To$ = 700., $CellContext`Rg$ = 
         1.98, $CellContext`A$ = 
         6.5, $CellContext`R$, $CellContext`u$, $CellContext`Keq$, \
$CellContext`rxn$, $CellContext`CA$, $CellContext`CB$, $CellContext`CH2$, \
$CellContext`Temp$, $CellContext`den$, $CellContext`fA$, $CellContext`fB$, \
$CellContext`fH2$, $CellContext`fT$, $CellContext`axialC$, \
$CellContext`radialC$, $CellContext`axialT$, $CellContext`radialT$, \
$CellContext`axialH2$}, $CellContext`R$ = 
         0.05 $CellContext`L$; $CellContext`u$ = $CellContext`umax$$ (
           1 - ($CellContext`r/($CellContext`R$ + \
$CellContext`\[Epsilon]$))^2); $CellContext`Keq$ = 
         10^18 Exp[(-$CellContext`\[CapitalDelta]H$$)/($CellContext`Rg$ \
$CellContext`T[$CellContext`z, $CellContext`r])]; $CellContext`rxn$ = \
($CellContext`A$ 
           Exp[(-$CellContext`Ea$$)/($CellContext`Rg$ \
$CellContext`T[$CellContext`z, $CellContext`r])]) \
($CellContext`cA[$CellContext`z, $CellContext`r] - \
$CellContext`cB[$CellContext`z, $CellContext`r] \
($CellContext`cH2[$CellContext`z, $CellContext`r]/$CellContext`Keq$)); \
{$CellContext`CA$, $CellContext`CB$, $CellContext`CH2$, $CellContext`Temp$} = 
         Quiet[
           NDSolveValue[{$CellContext`u$ D[
                $CellContext`cA[$CellContext`z, $CellContext`r], \
$CellContext`z] == ($CellContext`\[ScriptCapitalD]$$ (1/$CellContext`r)) 
               D[$CellContext`r D[
                   $CellContext`cA[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r] - $CellContext`rxn$, $CellContext`cA[
              0, $CellContext`r] == $CellContext`cAo$, 
             ReplaceAll[0 == D[
                $CellContext`cA[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r -> -$CellContext`R$ - \
$CellContext`\[Epsilon]$], 
             ReplaceAll[0 == D[
                $CellContext`cA[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r -> $CellContext`R$], $CellContext`u$ D[
                $CellContext`cB[$CellContext`z, $CellContext`r], \
$CellContext`z] == ($CellContext`\[ScriptCapitalD]$$ (1/$CellContext`r)) 
               D[$CellContext`r D[
                   $CellContext`cB[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r] + $CellContext`rxn$, $CellContext`cB[
              0, $CellContext`r] == 0, 
             ReplaceAll[0 == D[
                $CellContext`cB[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r -> -$CellContext`R$ - \
$CellContext`\[Epsilon]$], 
             ReplaceAll[0 == D[
                $CellContext`cB[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r -> $CellContext`R$], $CellContext`u$ D[
                $CellContext`cH2[$CellContext`z, $CellContext`r], \
$CellContext`z] == ($CellContext`\[ScriptCapitalD]$$ (1/$CellContext`r)) 
               D[$CellContext`r D[
                   $CellContext`cH2[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r] + $CellContext`rxn$, $CellContext`cH2[
              0, $CellContext`r] == 0, 
             
             ReplaceAll[$CellContext`kperm$ $CellContext`cH2[$CellContext`z, \
$CellContext`r] == $CellContext`\[ScriptCapitalD]$$ D[
                 $CellContext`cH2[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r -> -$CellContext`R$ - \
$CellContext`\[Epsilon]$], 
             
             ReplaceAll[$CellContext`kperm$ $CellContext`cH2[$CellContext`z, \
$CellContext`r] == $CellContext`\[ScriptCapitalD]$$ D[
                 $CellContext`cH2[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r -> $CellContext`R$], (($CellContext`u$ \
$CellContext`\[Rho]$) $CellContext`Cp$) D[
                $CellContext`T[$CellContext`z, $CellContext`r], \
$CellContext`z] == ($CellContext`\[Lambda]$$ (1/$CellContext`r)) 
               D[$CellContext`r D[
                   $CellContext`T[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r] + $CellContext`rxn$ (-$CellContext`\
\[CapitalDelta]H$$), $CellContext`T[0, $CellContext`r] == $CellContext`To$, 
             ReplaceAll[0 == D[
                $CellContext`T[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r -> -$CellContext`R$ - \
$CellContext`\[Epsilon]$], 
             ReplaceAll[0 == D[
                $CellContext`T[$CellContext`z, $CellContext`r], \
$CellContext`r], $CellContext`r -> $CellContext`R$]}, {$CellContext`cA, \
$CellContext`cB, $CellContext`cH2, $CellContext`T}, {$CellContext`z, 
             0, $CellContext`L$}, {$CellContext`r, -$CellContext`R$ - \
$CellContext`\[Epsilon]$, $CellContext`R$}]]; $CellContext`den$ = Quiet[
           
           NIntegrate[$CellContext`u$, {$CellContext`r, 0, $CellContext`R$}, 
            Method -> {Automatic, "SymbolicProcessing" -> 0}, AccuracyGoal -> 
            1]]; $CellContext`fA$[
           Pattern[$CellContext`z$, 
            Blank[]]] := Quiet[
           
           NIntegrate[$CellContext`u$ ($CellContext`CA$[$CellContext`z$, \
$CellContext`r]/$CellContext`den$), {$CellContext`r, 0, $CellContext`R$}, 
            Method -> {Automatic, "SymbolicProcessing" -> 0}, AccuracyGoal -> 
            1]]; $CellContext`fB$[
           Pattern[$CellContext`z$, 
            Blank[]]] := Quiet[
           
           NIntegrate[$CellContext`u$ ($CellContext`CB$[$CellContext`z$, \
$CellContext`r]/$CellContext`den$), {$CellContext`r, 0, $CellContext`R$}, 
            Method -> {Automatic, "SymbolicProcessing" -> 0}, AccuracyGoal -> 
            1]]; $CellContext`fH2$[
           Pattern[$CellContext`z$, 
            Blank[]]] := Quiet[
           
           NIntegrate[$CellContext`u$ ($CellContext`CH2$[$CellContext`z$, \
$CellContext`r]/$CellContext`den$), {$CellContext`r, 0, $CellContext`R$}, 
            Method -> {Automatic, "SymbolicProcessing" -> 0}, AccuracyGoal -> 
            1]]; $CellContext`fT$[
           Pattern[$CellContext`z$, 
            Blank[]]] := Quiet[
           
           NIntegrate[$CellContext`u$ ($CellContext`Temp$[$CellContext`z$, \
$CellContext`r]/$CellContext`den$), {$CellContext`r, 0, $CellContext`R$}, 
            Method -> {Automatic, "SymbolicProcessing" -> 0}, AccuracyGoal -> 
            1]]; $CellContext`axialC$ = Plot[{
            $CellContext`fA$[$CellContext`z], 
            $CellContext`fB$[$CellContext`z], 
            $CellContext`fH2$[$CellContext`z]}, {$CellContext`z, 
            0, $CellContext`L$}, PerformanceGoal -> "Quality", 
           PlotRange -> {{0., $CellContext`L$}, {0., $CellContext`cAo$}}, 
           Frame -> True, 
           PlotStyle -> {{Thick, Black}, {Thick, Red}, {Thick, Blue}, {
             Thick, Red, Dashed}}, LabelStyle -> {16, Black}, FrameTicksStyle -> 
           Directive[Black, 16], FrameLabel -> {
             Row[{"reactor length, ", 
               Style["z/L", Italic]}], "average concentration"}, 
           ImageSize -> {600, 400}, Epilog -> Text[
             Row[{
               Style["A", Black, 18], 
               Style[" \[Equilibrium] ", Black, 18], 
               Style["B", Red, 18], 
               Style[" + ", Black, 18], 
               Subscript[
                Style["H", Blue, 18], 
                Style["2", Blue]]}], {0.5 $CellContext`L$, 0.9}, Background -> 
             White]]; $CellContext`radialC$ = Plot[{
            $CellContext`CA$[$CellContext`Z$$/100., $CellContext`r], 
            $CellContext`CB$[$CellContext`Z$$/100., $CellContext`r], 
            $CellContext`CH2$[$CellContext`Z$$/
             100., $CellContext`r]}, {$CellContext`r, -$CellContext`R$, \
$CellContext`R$}, PerformanceGoal -> "Quality", 
           PlotRange -> {{-$CellContext`R$, $CellContext`R$}, {0., 1.}}, 
           Frame -> True, 
           PlotStyle -> {{Thick, Black}, {Thick, Red}, {Thick, Blue}}, 
           LabelStyle -> {16, Black}, FrameTicksStyle -> Directive[Black, 16],
            FrameLabel -> {
             Row[{"reactor radius, ", 
               Style["r/L", Italic]}], "concentration"}, 
           ImageSize -> {600, 400}, Epilog -> Text[
             Row[{
               Style["A", Black, 18], 
               Style[" \[Equilibrium] ", Black, 18], 
               Style["B", Red, 18], 
               Style[" + ", Black, 18], 
               Subscript[
                Style["H", Blue, 18], 
                Style["2", Blue]]}], {0., 0.9}, Background -> 
             White]]; $CellContext`axialH2$ = 
         Plot[100 (($CellContext`fB$[$CellContext`z] - \
$CellContext`fH2$[$CellContext`z])/$CellContext`cAo$), {$CellContext`z, 
            0, $CellContext`L$}, PerformanceGoal -> "Quality", 
           PlotRange -> {{0, $CellContext`L$}, {0., 100.}}, 
           PlotStyle -> {Thick, Blue}, LabelStyle -> {16, Black}, 
           FrameTicksStyle -> Directive[Black, 16], FrameLabel -> {
             Row[{"reactor length, ", 
               Style["z/L", Italic]}], 
             Row[{"% ", 
               Subscript["H", "2"], " permeate recovered"}]}, Frame -> True, 
           ImageSize -> {600, 400}]; $CellContext`radialT$ = ContourPlot[
           $CellContext`Temp$[$CellContext`z, $CellContext`r], \
{$CellContext`z, 
            0, $CellContext`L$}, {$CellContext`r, -$CellContext`R$, \
$CellContext`R$}, Contours -> 20, AspectRatio -> 1/GoldenRatio, 
           PerformanceGoal -> "Quality", ColorFunction -> "TemperatureMap", 
           LabelStyle -> {16, Black}, FrameTicksStyle -> Directive[Black, 16],
            FrameLabel -> {
             Row[{"axis, ", 
               Style["z/L", Italic]}], 
             Row[{"radius, ", 
               Style["r/L", Italic]}]}, 
           PlotRange -> {{
             0, $CellContext`L$}, {-$CellContext`R$, $CellContext`R$}}, 
           PlotLegends -> Automatic, 
           ImageSize -> {0.87 600, 400}]; $CellContext`axialT$ = Plot[
           $CellContext`fT$[$CellContext`z], {$CellContext`z, 
            0, $CellContext`L$}, PerformanceGoal -> "Quality", PlotStyle -> 
           Thickness[0.0075], ColorFunction -> "TemperatureMap", 
           LabelStyle -> {16, Black}, FrameTicksStyle -> Directive[Black, 16],
            Frame -> True, FrameLabel -> {
             Row[{"axis, ", 
               Style["z/L", Italic]}], "average temperature, K"}, 
           PlotRange -> {{0., $CellContext`L$}, {All, $CellContext`To$}}, 
           ImageSize -> {600, 400}]; 
        Which[$CellContext`g$$ == 1, $CellContext`axialC$, $CellContext`g$$ == 
          2, $CellContext`radialC$, $CellContext`g$$ == 
          3, $CellContext`axialT$, $CellContext`g$$ == 
          4, $CellContext`radialT$, $CellContext`g$$ == 
          5, $CellContext`axialH2$]], 
      "Specifications" :> {{{$CellContext`g$$, 4, ""}, {
         1 -> "axial concentration", 2 -> "radial concentration", 3 -> 
          "axial temperature", 4 -> "radial temperature", 5 -> 
          "% \!\(\*SubscriptBox[\(H\), \(2\)]\) recovery"}, ControlType -> 
         PopupMenu}, {{$CellContext`\[CapitalDelta]H$$, 30000., 
          "enthalpy (kJ/mol)"}, 10000., 50000., 5000., ImageSize -> Tiny, 
         Appearance -> "Labeled", ControlPlacement -> 
         1}, {{$CellContext`\[Lambda]$$, 0.001, 
          "thermal conductivity (W/m K)"}, 0.0001, 0.01, ImageSize -> Tiny, 
         Appearance -> "Labeled", ControlPlacement -> 
         2}, {{$CellContext`umax$$, 0.2, "fluid velocity (m/s)"}, 0.1, 0.5, 
         0.01, ImageSize -> Tiny, Appearance -> "Labeled", ControlPlacement -> 
         3}, {{$CellContext`Ea$$, 3000., "activation energy (kJ/mol)"}, 2000.,
          3500., 500., ImageSize -> Tiny, Appearance -> "Labeled", 
         ControlPlacement -> 
         4}, {{$CellContext`Z$$, 20, "distance along axis (% L)"}, 2, 100., 2,
          ImageSize -> Tiny, Appearance -> "Labeled", Enabled -> 
         Dynamic[$CellContext`g$$ == 2], ControlPlacement -> 
         5}, {{$CellContext`\[ScriptCapitalD]$$, 0.0001, 
          "diffusivity (\!\(\*SuperscriptBox[\(m\), \(2\)]\)/s)"}, 1.*^-6, 
         0.0001, 1.*^-6, ImageSize -> Tiny, Appearance -> "Labeled", 
         ControlPlacement -> 6}, 
        Grid[{{
           Manipulate`Place[1], 
           Manipulate`Place[2]}, {
           Manipulate`Place[3], 
           Manipulate`Place[4]}, {
           Manipulate`Place[5], 
           Manipulate`Place[6]}}, Alignment -> Left]}, 
      "Options" :> {
       SynchronousUpdating -> False, 
        TrackedSymbols :> {$CellContext`g$$, $CellContext`\[CapitalDelta]H$$, \
$CellContext`\[Lambda]$$, $CellContext`Ea$$, $CellContext`Z$$, $CellContext`\
\[ScriptCapitalD]$$, $CellContext`umax$$}, ControlPlacement -> Top, Alignment -> 
        Left}, "DefaultOptions" :> {ControllerLinking -> True}],
     ImageSizeCache->{601., {274., 281.}},
     SingleEvaluation->True],
    Deinitialization:>None,
    DynamicModuleValues:>{},
    SynchronousInitialization->True,
    UndoTrackedVariables:>{Typeset`show$$, Typeset`bookmarkMode$$},
    UnsavedVariables:>{Typeset`initDone$$},
    UntrackedVariables:>{Typeset`size$$}], "Manipulate",
   Deployed->True,
   StripOnInput->False],
  Manipulate`InterpretManipulate[1]]], "Output",
 CellID->345238038,ExpressionUUID->"ebb911a3-74f5-4062-8e6c-6c9f6e00b246"],

Cell[TextData[{
 "Membrane catalytic reactors can increase rates of conversion when a \
reaction is thermodynamically unfavorable. This Demonstration evaluates a \
dehydrogenation reaction in a two-dimensional tubular adiabatic fixed bed \
catalytic membrane reactor. The reversible endothermic reaction, ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    StyleBox["A",
     FontSlant->"Plain"], "\[Equilibrium]", 
    RowBox[{
     StyleBox["B",
      FontSlant->"Plain"], "+", 
     SubscriptBox[
      StyleBox["H",
       FontSlant->"Plain"], "2"]}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"0024eb46-0871-4e2f-887f-0a5c11a9b350"],
 ", takes place in the tube side as hydrogen permeates through the membrane \
to the shell compartment, the membrane being permeable only to ",
 Cell[BoxData[
  FormBox[
   SubscriptBox[
    StyleBox["H",
     FontSlant->"Plain"], "2"], TraditionalForm]], "InlineMath",
  ExpressionUUID->"f1e48261-c815-45f8-9a6b-04de1406f13f"],
 "."
}], "ManipulateCaption",
 CellID->1013067035,ExpressionUUID->"d12b4d50-1327-4509-af26-13b6ed21f5fc"],

Cell["THINGS TO TRY", "ManipulateCaption",
 CellFrame->{{0, 0}, {1, 0}},
 CellFrameColor->RGBColor[0.87, 0.87, 0.87],
 FontFamily->"Helvetica",
 FontSize->12,
 FontWeight->"Bold",
 FontColor->RGBColor[0.597406, 0, 0.0527047],
 CellTags->
  "ControlSuggestions",ExpressionUUID->"e2f090a0-4d95-4d2e-b4ea-0f62707244a6"],

Cell[TextData[{
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Resize Images",
     FontFamily->"Verdana"]],ExpressionUUID->
     "9d93996c-b5b2-4c25-aed9-496972ae02d9"], True->Cell[TextData[StyleBox[
    "Resize Images",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]],ExpressionUUID->
     "bc74c067-990a-41a2-bf55-69370264a754"]}, Dynamic[
     CurrentValue["MouseOver"]]],
   "\"Click inside an image to reveal its orange resize frame.\\nDrag any of \
the orange resize handles to resize the image.\"",
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]],ExpressionUUID->
  "7401b652-b6fa-4a68-91b1-bf5e884076e0"],
 StyleBox["\[NonBreakingSpace]\[FilledVerySmallSquare]\[NonBreakingSpace]",
  FontColor->RGBColor[0.928786, 0.43122, 0.104662]],
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Rotate and Zoom in 3D",
     FontFamily->"Verdana"]],ExpressionUUID->
     "00d85219-c09e-49a8-8b8d-4f2e2ba42746"], True->Cell[TextData[StyleBox[
    "Rotate and Zoom in 3D",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]],ExpressionUUID->
     "4bb3d637-213f-4033-8bb5-6e0eca7b7c81"]}, Dynamic[
     CurrentValue["MouseOver"]]],
   RowBox[{
    "\"Drag a 3D graphic to rotate it. Starting the drag near the center \
tumbles\\nthe graphic; starting near a corner turns it parallel to the plane \
of the screen.\\nHold down \"", 
     FrameBox[
     "Ctrl", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" (or \"", 
     FrameBox[
     "Cmd", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" on Mac) and drag up and down to zoom.\""}],
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]],ExpressionUUID->
  "e44a99a1-30e9-4ada-a3fd-412526da49cc"],
 StyleBox["\[NonBreakingSpace]\[FilledVerySmallSquare]\[NonBreakingSpace]",
  FontColor->RGBColor[0.928786, 0.43122, 0.104662]],
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Slider Zoom",
     FontFamily->"Verdana"]],ExpressionUUID->
     "2b99c1ca-54ab-43c9-acb6-fd24979bf911"], True->Cell[TextData[StyleBox[
    "Slider Zoom",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]],ExpressionUUID->
     "6f5e85c1-cbe8-49a9-a055-76048882e853"]}, Dynamic[
     CurrentValue["MouseOver"]]],
   RowBox[{"\"Hold down the \"", 
     FrameBox[
     "Alt", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], 
     "\" key while moving a slider to make fine adjustments in the slider \
value.\\nHold \"", 
     FrameBox[
     "Ctrl", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" and/or \"", 
     FrameBox[
     "Shift", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" at the same time as \"", 
     FrameBox[
     "Alt", Background -> GrayLevel[0.9], FrameMargins -> 2, FrameStyle -> 
      GrayLevel[0.9]], "\" to make ever finer adjustments.\""}],
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]],ExpressionUUID->
  "9a46fc27-3cb3-48e6-a478-efb6129e8acd"],
 StyleBox["\[NonBreakingSpace]\[FilledVerySmallSquare]\[NonBreakingSpace]",
  FontColor->RGBColor[0.928786, 0.43122, 0.104662]],
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Gamepad Controls",
     FontFamily->"Verdana"]],ExpressionUUID->
     "f21a7e0e-5289-4083-b502-6cd4a0baff17"], True->Cell[TextData[StyleBox[
    "Gamepad Controls",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]],ExpressionUUID->
     "d151996b-27b8-4409-b7d5-5b1da05c1f05"]}, Dynamic[
     CurrentValue["MouseOver"]]],
   "\"Control this Demonstration with a gamepad or other\\nhuman interface \
device connected to your computer.\"",
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]],ExpressionUUID->
  "6259610d-faa4-4f0d-a03a-64433bfbb9c8"],
 StyleBox["\[NonBreakingSpace]\[FilledVerySmallSquare]\[NonBreakingSpace]",
  FontColor->RGBColor[0.928786, 0.43122, 0.104662]],
 Cell[BoxData[
  TooltipBox[
   PaneSelectorBox[{False->Cell[TextData[StyleBox["Automatic Animation",
     FontFamily->"Verdana"]],ExpressionUUID->
     "3d97d213-7bc2-4c9c-8721-b2ee89052d8b"], True->Cell[TextData[StyleBox[
    "Automatic Animation",
     FontFamily->"Verdana",
     FontColor->GrayLevel[0.5]]],ExpressionUUID->
     "4f1ff61b-e1f8-4b00-bfc4-403ec51baf27"]}, Dynamic[
     CurrentValue["MouseOver"]]],
   RowBox[{"\"Animate a slider in this Demonstration by clicking the\"", 
     AdjustmentBox[
      Cell[
       GraphicsData[
       "CompressedBitmap", 
        "eJzzTSzJSM1NLMlMTlRwL0osyMhMLlZwyy8CCjEzMjAwcIKwAgOI/R/IhBKc\n\
/4EAyGAG0f+nTZsGwgysIJIRKsWKLAXGIHFmEpUgLADxWUAkI24jZs+eTaEt\n\
IG+wQKRmzJgBlYf5lhEA30OqWA=="], "Graphics", ImageSize -> {9, 9}, ImageMargins -> 
       0, CellBaseline -> Baseline], BoxBaselineShift -> 0.1839080459770115, 
      BoxMargins -> {{0., 0.}, {-0.1839080459770115, 0.1839080459770115}}], 
     "\"button\\nnext to the slider, and then clicking the play button that \
appears.\\nAnimate all controls by selecting \"", 
     StyleBox["Autorun", FontWeight -> "Bold"], "\" from the\"", 
     AdjustmentBox[
      Cell[
       GraphicsData[
       "CompressedBitmap", 
        "eJyNULENwyAQfEySIlMwTVJlCGRFsosokeNtqBmDBagoaZjAI1C8/8GUUUC6\n\
57h7cQ8PvU7Pl17nUav7oj/TPH7V7b2QJAUAXBkKmCPRowxICy64bRvGGNF7\n\
X8CctGoDSN4xhIDGGDhzFXwUh3/ClBKrDQPmnGXtI6u0OOd+tZBVUqy1xSaH\n\
UqiK6pPe4XdEdAz6563tx/gejuORGMxJaz8mdpJn7hc="], "Graphics", 
       ImageSize -> {10, 10}, ImageMargins -> 0, CellBaseline -> Baseline], 
      BoxBaselineShift -> 0.1839080459770115, 
      BoxMargins -> {{0., 0.}, {-0.1839080459770115, 0.1839080459770115}}], 
     "\"menu.\""}],
   TooltipStyle->{
    FontFamily -> "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.35], 
     Background -> GrayLevel[0.98]}]],ExpressionUUID->
  "d12670cd-a403-46a1-9f63-392b6d2e5d3f"]
}], "ManipulateCaption",
 CellMargins->{{Inherited, Inherited}, {0, 0}},
 Deployed->True,
 FontFamily->"Verdana",
 CellTags->
  "ControlSuggestions",ExpressionUUID->"01a3432c-0af8-4354-a3e7-d4a39d8d587c"],

Cell["DETAILS", "DetailsSection",
 CellTags->"Details",ExpressionUUID->"fc196182-4ed0-44c8-864c-6ef82eb7e8b2"],

Cell["Steady-state mass and energy balances in this reactor are:", \
"DetailNotes",
 CellID->1520704587,ExpressionUUID->"cc49f8a3-f096-4abf-987d-cdcf3c69c2ad"],

Cell[TextData[{
 "tube side ",
 Cell[BoxData[
  FormBox[
   RowBox[{"0", "<", "r", "<", "R"}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"50f4f7f6-4bc8-4448-996e-9f58ce711ffd"],
 ","
}], "DetailNotes",
 CellID->619226281,ExpressionUUID->"0b72ba76-5bfa-49ba-a627-6928d3d50d9f"],

Cell[TextData[Cell[BoxData[
 FormBox[
  RowBox[{" ", 
   RowBox[{
    RowBox[{
     RowBox[{
      SubscriptBox["u", "max"], "(", 
      RowBox[{"1", "-", 
       SuperscriptBox[
        RowBox[{"(", 
         FractionBox["r", "R"], ")"}], "2"]}], ")"}], 
     FractionBox[
      RowBox[{"\[PartialD]", 
       SubscriptBox["c", "i"]}], 
      RowBox[{"\[PartialD]", "z"}]]}], "=", " ", 
    RowBox[{
     RowBox[{
      FractionBox["\[ScriptCapitalD]", "r"], 
      RowBox[{
       FractionBox["\[PartialD]", 
        RowBox[{"\[PartialD]", "r"}]], 
       RowBox[{"(", 
        RowBox[{"r", 
         FractionBox[
          RowBox[{"\[PartialD]", 
           SubscriptBox["c", "i"]}], 
          RowBox[{"\[PartialD]", "r"}]]}], ")"}]}]}], "+", "rxn"}]}]}], 
  TraditionalForm]], \
"InlineMath",ExpressionUUID->"6cce5463-8929-42f1-af38-7b72cd7af210"]], \
"DetailNotes",
 CellID->1196967707,ExpressionUUID->"d8a03a53-3ac3-472c-86b9-3358770ae748"],

Cell["and", "DetailNotes",
 CellID->1164058886,ExpressionUUID->"8ed17abe-965e-446d-92cb-2112eb5f6666"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{" ", 
    RowBox[{
     RowBox[{
      SubscriptBox["u", "max"], " ", "\[Rho]", " ", 
      RowBox[{
       SubscriptBox["C", "p"], "(", 
       RowBox[{"1", "-", 
        SuperscriptBox[
         RowBox[{"(", 
          FractionBox["r", "R"], ")"}], "2"]}], ")"}], 
      FractionBox[
       RowBox[{"\[PartialD]", "T"}], 
       RowBox[{"\[PartialD]", "z"}]]}], "=", " ", 
     RowBox[{
      RowBox[{
       FractionBox["\[Lambda]", "r"], 
       RowBox[{
        FractionBox["\[PartialD]", 
         RowBox[{"\[PartialD]", "r"}]], 
        RowBox[{"(", 
         RowBox[{"r", 
          FractionBox[
           RowBox[{"\[PartialD]", "T"}], 
           RowBox[{"\[PartialD]", "r"}]]}], ")"}]}]}], "+", 
      RowBox[{"rxn", "(", 
       RowBox[{
        RowBox[{"-", "\[CapitalDelta]"}], "\[VeryThinSpace]", "H"}], 
       ")"}]}]}]}], TraditionalForm]], "InlineMath",ExpressionUUID->
  "6b8bd01c-22cb-4cf8-bed5-995a71a53c14"],
 ","
}], "DetailNotes",
 CellID->218044408,ExpressionUUID->"bbdb6f50-b7a2-4442-83c0-c5dc644e4582"],

Cell[TextData[{
 "where ",
 Cell[BoxData[
  FormBox["z", TraditionalForm]], "InlineMath",ExpressionUUID->
  "0e1a3a6f-d55a-44f6-8073-9a2cc4954283"],
 " and ",
 Cell[BoxData[
  FormBox["r", TraditionalForm]], "InlineMath",ExpressionUUID->
  "25985d9c-ebea-4d34-8bf7-949d38c2427f"],
 " are the axial and radial coordinates; ",
 Cell[BoxData[
  FormBox["i", TraditionalForm]], "InlineMath",ExpressionUUID->
  "772e2cd1-a614-45a6-a211-31d1ee264ff6"],
 " represents the reactant ",
 Cell[BoxData[
  FormBox[
   StyleBox["A",
    FontSlant->"Plain"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "ead83a92-09e6-4b6e-8966-f71952fa8810"],
 ", ",
 Cell[BoxData[
  FormBox[
   StyleBox["B",
    FontSlant->"Plain"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "7034ee4b-2d34-4189-8d28-d87b9f25e628"],
 " and ",
 Cell[BoxData[
  FormBox[
   SubscriptBox[
    StyleBox["H",
     FontSlant->"Plain"], "2"], TraditionalForm]], "InlineMath",
  ExpressionUUID->"5cebd984-2733-4a61-a324-8c91bce802dd"],
 ", the products; ",
 Cell[BoxData[
  FormBox["T", TraditionalForm]], "InlineMath",ExpressionUUID->
  "da9187fc-9731-4c6b-91c0-75fe21841872"],
 " is the temperature; ",
 Cell[BoxData[
  FormBox[
   SubscriptBox["u", "max"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "a5b56330-ef90-4148-b2c5-c2cbebc3075b"],
 ", ",
 Cell[BoxData[
  FormBox["\[Rho]", TraditionalForm]], "InlineMath",ExpressionUUID->
  "da165eb3-a7a2-4663-ab13-2b986976f7d9"],
 " and ",
 Cell[BoxData[
  FormBox[
   SubscriptBox["C", "p"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "87c69754-23fb-40a8-8d2e-88e24cc91b30"],
 " are maximum fluid velocity, fluid density and fluid heat capacity; ",
 Cell[BoxData[
  FormBox["\[ScriptCapitalD]", TraditionalForm]], "InlineMath",ExpressionUUID->
  "8aa65ec9-4011-4b15-8bb4-af18b8e0a88a"],
 ", ",
 Cell[BoxData[
  FormBox["\[Lambda]", TraditionalForm]], "InlineMath",ExpressionUUID->
  "eab64512-dafd-4b83-b1af-934363b28fb0"],
 " and",
 Cell[BoxData[
  FormBox[
   RowBox[{" ", 
    RowBox[{"\[CapitalDelta]", "\[VeryThinSpace]", "H"}]}], TraditionalForm]],
   "InlineMath",ExpressionUUID->"e709698b-b834-4f5f-9c04-6d7303e7bd10"],
 " are the diffusion coefficient, thermal conductivity and enthalpy of \
reaction; and ",
 Cell[BoxData[
  FormBox[
   RowBox[{"rxn", "=", 
    RowBox[{"k", "(", 
     RowBox[{
      SubscriptBox["c", 
       StyleBox["A",
        FontSlant->"Plain"]], "-", " ", 
      FractionBox[
       RowBox[{
        SubscriptBox["c", "B"], " ", 
        SubscriptBox["c", 
         SubscriptBox["H", "2"]]}], 
       SubscriptBox["K", "eq"]]}], ")"}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"9139628c-873e-4db2-8f79-2b17ef67097b"],
 " is the rate of dehydrogenation, with"
}], "DetailNotes",
 CellID->1065263474,ExpressionUUID->"f5a681bc-6f12-4ca1-ad7f-dea074308e3d"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{"k", "=", 
    RowBox[{"A", " ", 
     SuperscriptBox["e", 
      RowBox[{
       RowBox[{
        RowBox[{"-", 
         SubscriptBox["E", 
          StyleBox["A",
           FontSlant->"Plain"]]}], "/", 
        SubscriptBox["R", 
         StyleBox["g",
          FontSlant->"Plain"]]}], "T"}]]}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"cad3a539-c987-4037-b547-9d6a7171f6fa"],
 " "
}], "DetailNotes",
 CellID->1970551463,ExpressionUUID->"ee0e1515-4ae9-40c3-a852-5d7f7aa9fa25"],

Cell["and", "DetailNotes",
 CellID->1563297351,ExpressionUUID->"b0432eef-5063-4599-abd5-feb4ee50d4cc"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["K", "eq"], "=", 
    RowBox[{"K", " ", 
     SuperscriptBox["e", 
      RowBox[{
       RowBox[{"-", "\[CapitalDelta]"}], "\[VeryThinSpace]", 
       RowBox[{"H", "/", 
        SubscriptBox["R", 
         StyleBox["g",
          FontSlant->"Plain"]]}], " ", "T"}]]}]}], TraditionalForm]], 
  "InlineMath",ExpressionUUID->"5d4d152c-ef46-4d22-9ed1-af5e6c0d6168"],
 ","
}], "DetailNotes",
 CellID->1281036049,ExpressionUUID->"3de17c52-78b4-4a6c-a216-1ba28b5565d0"],

Cell[TextData[{
 "where ",
 Cell[BoxData[
  FormBox["A", TraditionalForm]], "InlineMath",
  InitializationCell->True,ExpressionUUID->
  "0151a8b2-86e0-48ee-9ac8-bee4fb9c919a"],
 " and ",
 Cell[BoxData[
  FormBox["K", TraditionalForm]], "InlineMath",ExpressionUUID->
  "42e7814c-2fe9-4bde-ae9f-8d6485a00191"],
 " are pre-exponential factors; ",
 Cell[BoxData[
  FormBox[
   SubscriptBox["E", 
    StyleBox["A",
     FontSlant->"Plain"]], TraditionalForm]], "InlineMath",ExpressionUUID->
  "efc84787-6237-4020-a574-ddfa75399e61"],
 " is the activation energy; and ",
 Cell[BoxData[
  FormBox[
   SubscriptBox["R", 
    StyleBox["g",
     FontSlant->"Plain"]], TraditionalForm]], "InlineMath",ExpressionUUID->
  "a31cccf2-21af-4811-99c2-05011317f9da"],
 " is the universal gas constant."
}], "DetailNotes",
 CellID->496076751,ExpressionUUID->"6a041c04-071a-4a57-8af1-c64806969113"],

Cell["The boundary values are as follows:", "DetailNotes",
 CellID->1127956628,ExpressionUUID->"f014f6e7-c281-45d7-8cc3-d491e31285a7"],

Cell[TextData[{
 "At ",
 Cell[BoxData[
  FormBox[
   RowBox[{"z", "=", "0"}], TraditionalForm]], "InlineMath",ExpressionUUID->
  "d1e6e88c-ce20-4cea-a60f-f281f01fb803"],
 ":"
}], "DetailNotes",
 CellID->2133810852,ExpressionUUID->"6543a726-e098-43aa-866b-5c42b87f02a8"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["c", 
     StyleBox["A",
      FontSlant->"Plain"]], "=", 
    SubscriptBox["c", "o"]}], TraditionalForm]], "InlineMath",ExpressionUUID->
  "9c0d4380-ffcf-4fd9-828e-9ac6123fab23"],
 ", ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    SubscriptBox["c", 
     StyleBox["B",
      FontSlant->"Plain"]], "=", 
    RowBox[{
     SubscriptBox["c", 
      SubscriptBox[
       StyleBox["H",
        FontSlant->"Plain"], "2"]], "=", "0"}]}], TraditionalForm]], 
  "InlineMath",ExpressionUUID->"a326ee21-532b-490f-a467-8ce8272db77e"],
 " and ",
 Cell[BoxData[
  FormBox[
   RowBox[{"T", "=", 
    SubscriptBox["T", "0"]}], TraditionalForm]], "InlineMath",ExpressionUUID->
  "5cacee44-c496-490f-a190-adede27cc38e"],
 ", where ",
 Cell[BoxData[
  FormBox[
   SubscriptBox["c", "0"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "da25ecb7-6f76-4ab0-99e7-d9bf028c1e7a"],
 " and ",
 Cell[BoxData[
  FormBox[
   SubscriptBox["T", "0"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "d8baf77f-6aa9-42e1-9fbf-8abe5f5b7a94"],
 " are the initial values of the concentration and temperature."
}], "DetailNotes",
 CellID->2057646787,ExpressionUUID->"0b721a69-c1f9-480e-ae48-0bae62e5605f"],

Cell[TextData[{
 "At ",
 Cell[BoxData[
  FormBox[
   RowBox[{"r", "=", "R"}], TraditionalForm]], "InlineMath",ExpressionUUID->
  "b3c60a96-d99d-486b-9118-2cec829594a3"],
 ", and ",
 Cell[BoxData[
  FormBox[
   RowBox[{"r", "=", 
    RowBox[{"-", "R"}]}], TraditionalForm]], "InlineMath",ExpressionUUID->
  "de963401-aa2c-4ee6-94cc-904958de82d9"],
 ":"
}], "DetailNotes",
 CellID->227898012,ExpressionUUID->"f7205b88-055e-415e-912d-9e6aa774839e"],

Cell[TextData[{
 Cell[BoxData[
  FormBox[
   RowBox[{
    FractionBox[
     RowBox[{"\[PartialD]", " ", 
      SubscriptBox["c", 
       SubscriptBox[
        StyleBox["H",
         FontSlant->"Plain"], "2"]]}], 
     RowBox[{"\[PartialD]", "r"}]], "=", 
    RowBox[{
     SubscriptBox["k", "perm"], 
     SubscriptBox["c", 
      SubscriptBox[
       StyleBox["H",
        FontSlant->"Plain"], "2"]]}]}], TraditionalForm]], "InlineMath",
  ExpressionUUID->"1896b87f-3c5b-46c6-85b4-de7bf4f74348"],
 " and ",
 Cell[BoxData[
  FormBox[
   RowBox[{
    FractionBox[
     RowBox[{"\[PartialD]", 
      SubscriptBox["c", 
       StyleBox["A",
        FontSlant->"Plain"]]}], 
     RowBox[{"\[PartialD]", "r"}]], "=", 
    RowBox[{
     FractionBox[
      RowBox[{"\[PartialD]", 
       SubscriptBox["c", 
        StyleBox["B",
         FontSlant->"Plain"]]}], 
      RowBox[{"\[PartialD]", "r"}]], "=", 
     RowBox[{
      FractionBox[
       RowBox[{"\[PartialD]", "T"}], 
       RowBox[{"\[PartialD]", "r"}]], "=", "0"}]}]}], TraditionalForm]], 
  "InlineMath",ExpressionUUID->"21454940-119b-4e96-a25b-42bddc35458d"],
 "; here, ",
 Cell[BoxData[
  FormBox[
   SubscriptBox["k", "perm"], TraditionalForm]], "InlineMath",ExpressionUUID->
  "a6ad08de-350f-4517-a870-c58266cfab3a"],
 " is the hydrogen permeation coefficient."
}], "DetailNotes",
 CellID->595758509,ExpressionUUID->"f2e868fd-c5b7-4251-bf95-93c10acfe24d"],

Cell[TextData[{
 "These equations are solved with the built-in Mathematica function ",
 StyleBox["NDSolve", "MR"],
 ". You can vary the fluid thermodynamic properties, the fluid velocity and \
the distance along the reactor axis to determine their effect on the \
distribution of temperature and concentrations in different parts of the \
reactor."
}], "DetailNotes",
 CellID->1413681601,ExpressionUUID->"9d0a85ae-7b45-454e-8366-a06e201c84f1"],

Cell[BoxData[
 ButtonBox[
  PaneSelectorBox[{False->Cell[BoxData[
    GraphicsBox[RasterBox[CompressedData["
1:
      "], {{0, 0}, {194, 22}}, {0, 255},
      ColorFunction->RGBColor],
     ImageSize->{194, 22},
     PlotRange->{{0, 194}, {0, 22}}]],ExpressionUUID->
    "d519f243-8bb8-4a93-a7e1-acb50b765344"], True->Cell[BoxData[
    GraphicsBox[RasterBox[CompressedData["
1:
zfE/BGJAMMgG8SChPwC1foQX
      "], {{0, 0}, {194, 22}}, {0, 255},
      ColorFunction->RGBColor],
     ImageSize->{194, 22},
     PlotRange->{{0, 194}, {0, 22}}]],ExpressionUUID->
    "547b408c-8fc4-4ade-bd68-566cc398e001"]}, Dynamic[
    CurrentValue["MouseOver"]]],
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/versions/source.jsp?id=\
TwoDimensionalModelOfAdiabaticFixedBedMembraneReactor"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/\
TwoDimensionalModelOfAdiabaticFixedBedMembraneReactor/\
TwoDimensionalModelOfAdiabaticFixedBedMembraneReactor-source.nb"]], \
"DemoSourceNotebookSection",
 CellFrame->{{0, 0}, {1, 1}},
 ShowCellBracket->False,
 CellMargins->{{48, 48}, {28, 28}},
 CellGroupingRules->{"SectionGrouping", 25},
 CellFrameMargins->{{48, 48}, {6, 8}},
 CellFrameColor->RGBColor[
  0.87, 0.87, 0.87],ExpressionUUID->"072f9686-4e32-4ac5-9f38-1fdc85cf6d75"],

Cell["PERMANENT CITATION", "CitationSection",ExpressionUUID->"dfabcf30-ee28-4ff0-980d-43d540c344de"],

Cell[TextData[{
 "\[NonBreakingSpace]",
 ButtonBox["Clay Gruesbeck",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["https://demonstrations.wolfram.com/author.html?author=Clay+\
Gruesbeck"], None},
  ButtonNote->
   "https://demonstrations.wolfram.com/author.html?author=Clay+Gruesbeck"]
}], "Author",
 FontColor->GrayLevel[
  0.6],ExpressionUUID->"f315556b-2173-4665-9e59-791e285c66bb"],

Cell[TextData[{
 "\"",
 ButtonBox["Two-Dimensional Model of Adiabatic Fixed-Bed Membrane Reactor",
  BaseStyle->"SiteLink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/\
TwoDimensionalModelOfAdiabaticFixedBedMembraneReactor/"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/\
TwoDimensionalModelOfAdiabaticFixedBedMembraneReactor/"],
 "\"",
 "\[ParagraphSeparator]\[NonBreakingSpace]",
 ButtonBox["http://demonstrations.wolfram.com/\
TwoDimensionalModelOfAdiabaticFixedBedMembraneReactor/",
  BaseStyle->"SiteLink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/\
TwoDimensionalModelOfAdiabaticFixedBedMembraneReactor/"], None},
  ButtonNote->
   "http://demonstrations.wolfram.com/\
TwoDimensionalModelOfAdiabaticFixedBedMembraneReactor/"],
 "\[ParagraphSeparator]\[NonBreakingSpace]",
 ButtonBox["Wolfram Demonstrations Project",
  BaseStyle->"SiteLink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/"], None},
  ButtonNote->"http://demonstrations.wolfram.com/"],
 "\[ParagraphSeparator]\[NonBreakingSpace]",
 "Published: ",
 "January 27, 2022"
}], "Citations",ExpressionUUID->"5f2ee03c-51f8-4bbd-83f4-1582d06b7bb9"],

Cell[TextData[{
 "\[Copyright] ",
 StyleBox[ButtonBox["Wolfram Demonstrations Project & Contributors",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/"], None},
  ButtonNote->"http://demonstrations.wolfram.com/"],
  FontColor->GrayLevel[0.6]],
 "\[ThickSpace]\[ThickSpace]\[ThickSpace]|\[ThickSpace]\[ThickSpace]\
\[ThickSpace]",
 StyleBox[ButtonBox["Terms of Use",
  BaseStyle->"Hyperlink",
  ButtonData->{
    URL["http://demonstrations.wolfram.com/termsofuse.html"], None},
  ButtonNote->"http://demonstrations.wolfram.com/termsofuse.html"],
  FontColor->GrayLevel[0.6]]
}], "Text",
 CellFrame->{{0, 0}, {0, 0.5}},
 CellMargins->{{48, 48}, {20, 50}},
 CellFrameColor->GrayLevel[0.45098],
 FontFamily->"Verdana",
 FontSize->9,
 FontColor->GrayLevel[0.6],
 CellTags->"Copyright",ExpressionUUID->"b1075779-476e-4f0d-9afd-94ad1b43d7dc"]
},
Editable->False,
Saveable->False,
ScreenStyleEnvironment->"Working",
CellInsertionPointCell->None,
CellGrouping->Manual,
WindowSize->{750, 650},
WindowMargins->{{0, Automatic}, {Automatic, 0}},
WindowElements->{
 "StatusArea", "MemoryMonitor", "MagnificationPopUp", "VerticalScrollBar", 
  "MenuBar"},
WindowTitle->"Two-Dimensional Model of Adiabatic Fixed-Bed Membrane Reactor",
DockedCells->{},
CellContext->Notebook,
FrontEndVersion->"12.0 for Microsoft Windows (64-bit) (September 19, 2019)",
StyleDefinitions->Notebook[{
   Cell[
    CellGroupData[{
      Cell[
      "Demonstration Styles", "Title", 
       CellChangeTimes -> {
        3.3509184553711*^9, {3.36928902713192*^9, 3.36928902738193*^9}, {
         3.3754479092466917`*^9, 3.3754479095123196`*^9}, {
         3.375558447161495*^9, 3.375558447395873*^9}, {3.37572892702972*^9, 
         3.375728927639103*^9}}], 
      Cell[
       StyleData[StyleDefinitions -> "Default.nb"]], 
      Cell[
       CellGroupData[{
         Cell[
         "Style Environment Names", "Section", 
          CellChangeTimes -> {{3.369277974278112*^9, 3.369277974396138*^9}}], 
         Cell[
          StyleData[All, "Working"], ShowCellBracket -> False]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Notebook Options", "Section", 
          CellChangeTimes -> {{3.374865264950812*^9, 3.374865265419568*^9}}], 
         Cell[
         "  The options defined for the style below will be used at the \
Notebook level.  ", "Text"], 
         Cell[
          StyleData["Notebook"], Editable -> True, 
          PageHeaders -> {{None, None, None}, {None, None, None}}, 
          PageFooters -> {{None, None, None}, {None, None, None}}, 
          PageHeaderLines -> {False, False}, 
          PageFooterLines -> {False, False}, 
          PrintingOptions -> {
           "FacingPages" -> False, "FirstPageFooter" -> False, 
            "RestPagesFooter" -> False}, CreateCellID -> True, 
          CellFrameLabelMargins -> 6, DefaultNewInlineCellStyle -> 
          "InlineMath", DefaultInlineFormatType -> 
          "DefaultTextInlineFormatType", ShowStringCharacters -> True, 
          CacheGraphics -> False, StyleMenuListing -> None, 
          DemonstrationSite`Private`TrackCellChangeTimes -> False]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Input/Output", "Section", 
          CellChangeTimes -> {{3.3756313297791014`*^9, 
           3.3756313299509783`*^9}}], 
         Cell[
         "The cells in this section define styles used for input and output \
to the kernel.  Be careful when modifying, renaming, or removing these \
styles, because the front end associates special meanings with these style \
names.    ", "Text"], 
         Cell[
          StyleData["Input"], CellMargins -> {{48, 4}, {6, 4}}], 
         Cell[
          StyleData["Output"], CellMargins -> {{48, 4}, {6, 4}}], 
         Cell[
          StyleData["DemonstrationHeader"], Deletable -> False, 
          CellFrame -> {{0, 0}, {0, 0}}, ShowCellBracket -> False, 
          CellMargins -> {{0, 0}, {30, 0}}, 
          CellGroupingRules -> {"SectionGrouping", 20}, 
          CellHorizontalScrolling -> True, 
          CellFrameMargins -> {{0, 0}, {0, 0}}, CellFrameColor -> 
          RGBColor[0, 0, 0], StyleMenuListing -> None, Background -> 
          RGBColor[0, 0, 0]], 
         Cell[
          StyleData["ShowSource"], CellFrame -> {{0, 0}, {0, 1}}, 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 28}}, 
          CellFrameMargins -> {{48, 48}, {6, 8}}, CellFrameColor -> 
          RGBColor[0.87, 0.87, 0.87], StyleMenuListing -> None, FontFamily -> 
          "Helvetica", FontSize -> 10, FontWeight -> "Bold", FontSlant -> 
          "Plain", FontColor -> RGBColor[1, 0.42, 0]]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Basic Styles", "Section", 
          CellChangeTimes -> {{3.34971724802035*^9, 3.34971724966638*^9}, {
           3.35091840608065*^9, 3.35091840781999*^9}, {3.35091845122987*^9, 
           3.35091845356607*^9}, {3.35686681885432*^9, 3.35686681945788*^9}, {
           3.375657418186455*^9, 3.375657418452083*^9}}], 
         Cell[
          StyleData["Hyperlink"], StyleMenuListing -> None, FontColor -> 
          GrayLevel[0]], 
         Cell[
          StyleData["SiteLink"], StyleMenuListing -> None, 
          ButtonStyleMenuListing -> Automatic, FontColor -> 
          GrayLevel[0.45098], 
          ButtonBoxOptions -> {
           Active -> True, Appearance -> {Automatic, None}, 
            ButtonFunction :> (FrontEndExecute[{
               NotebookLocate[#2]}]& ), ButtonNote -> ButtonData}], 
         Cell[
          StyleData["Link"], FontColor -> GrayLevel[0.45098]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoNotes"], CellFrame -> True, 
             CellMargins -> {{0, 0}, {0, 0}}, 
             CellFrameMargins -> {{48, 48}, {4, 4}}, CellFrameColor -> 
             GrayLevel[0.99], StyleMenuListing -> None, FontFamily -> 
             "Verdana", FontSize -> 10, FontColor -> GrayLevel[0.45098], 
             DemonstrationSite`Private`ReturnCreatesNewCell -> True], 
            Cell[
             StyleData["DemoNotes", "Printout"], 
             CellMargins -> {{24, 0}, {0, 10}}, FontSize -> 9]}, Open]], 
         Cell[
          StyleData["SnapshotsSection"], CellFrame -> {{0, 0}, {0, 2}}, 
          ShowCellBracket -> False, ShowGroupOpener -> True, 
          CellMargins -> {{48, 48}, {10, 30}}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], DefaultNewCellStyle -> 
          "SnapshotCaption", StyleMenuListing -> None, FontFamily -> 
          "Verdana", FontSize -> 12, FontColor -> GrayLevel[0.45098], 
          PrivateCellOptions -> {"DefaultCellGroupOpen" -> False}], 
         Cell[
          StyleData[
          "SnapshotCaption", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["SnapshotOutput"], ShowCellBracket -> False, 
             CellMargins -> {{48, 10}, {5, 7}}, Evaluatable -> True, 
             CellGroupingRules -> "InputGrouping", PageBreakWithin -> False, 
             GroupPageBreakWithin -> False, DefaultFormatType -> 
             DefaultInputFormatType, ShowAutoStyles -> True, 
             "TwoByteSyntaxCharacterAutoReplacement" -> True, 
             HyphenationOptions -> {
              "HyphenationCharacter" -> "\[Continuation]"}, 
             AutoItalicWords -> {}, LanguageCategory -> "Mathematica", 
             FormatType -> InputForm, NumberMarks -> True, 
             LinebreakAdjustments -> {0.85, 2, 10, 0, 1}, CounterIncrements -> 
             "Input", MenuSortingValue -> 1500, MenuCommandKey -> "9", 
             DemonstrationSite`Private`StripStyleOnPaste -> True], 
            Cell[
             StyleData["SnapshotOuput", "Printout"], 
             CellMargins -> {{39, 0}, {4, 6}}, 
             LinebreakAdjustments -> {0.85, 2, 10, 1, 1}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoTitle"], Deletable -> False, ShowCellBracket -> 
             False, CellMargins -> {{48, 48}, {22, 10}}, 
             CellGroupingRules -> {"SectionGrouping", 20}, StyleMenuListing -> 
             None, FontFamily -> "Verdana", FontSize -> 20, FontWeight -> 
             "Bold", FontColor -> RGBColor[0.597406, 0, 0.0527047], 
             Background -> GrayLevel[1]], 
            Cell[
             StyleData["DemoName", "Printout"], 
             CellMargins -> {{24, 8}, {8, 27}}, 
             HyphenationOptions -> {"HyphenationCharacter" -> "-"}, FontSize -> 
             16]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DetailsSection"], CellFrame -> {{0, 0}, {1, 0}}, 
             ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 28}}, 
             CellGroupingRules -> {"SectionGrouping", 25}, 
             CellFrameMargins -> {{48, 48}, {6, 8}}, CellFrameColor -> 
             RGBColor[0.87, 0.87, 0.87], StyleMenuListing -> None, FontFamily -> 
             "Helvetica", FontSize -> 12, FontWeight -> "Bold", FontColor -> 
             RGBColor[0.597406, 0, 0.0527047]], 
            Cell[
             StyleData["DetailsSection", "Printout"], 
             CellMargins -> {{12, 0}, {0, 16}}, PageBreakBelow -> False, 
             FontSize -> 12]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoSection"], CellFrame -> {{0, 0}, {1, 0}}, 
             ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 28}}, 
             CellGroupingRules -> {"SectionGrouping", 30}, 
             CellFrameMargins -> {{48, 48}, {6, 8}}, CellFrameColor -> 
             RGBColor[0.87, 0.87, 0.87], StyleMenuListing -> None, FontFamily -> 
             "Helvetica", FontSize -> 12, FontWeight -> "Bold", FontColor -> 
             RGBColor[0.597406, 0, 0.0527047]], 
            Cell[
             StyleData["DemoSection", "Printout"], 
             CellMargins -> {{12, 0}, {0, 16}}, PageBreakBelow -> False, 
             FontSize -> 12]}, Open]], 
         Cell[
          StyleData["ManipulateSection"], CellFrame -> {{0, 0}, {0, 2}}, 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {10, 30}}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], StyleMenuListing -> None, 
          FontFamily -> "Verdana", FontSize -> 12], 
         Cell[
          StyleData["ManipulateCaptionSection"], 
          CellFrame -> {{0, 0}, {0, 2}}, ShowCellBracket -> False, 
          CellMargins -> {{48, 48}, {10, 30}}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], DefaultNewCellStyle -> 
          "ManipulateCaption", StyleMenuListing -> None, FontFamily -> 
          "Verdana", FontSize -> 12, FontColor -> GrayLevel[0.45098]], 
         Cell[
          StyleData["ManipulateCaption"], ShowCellBracket -> False, 
          CellMargins -> {{48, 48}, {10, 16}}, StyleMenuListing -> None, 
          FontFamily -> "Verdana", FontSize -> 12, FontColor -> GrayLevel[0], 
          DemonstrationSite`Private`ReturnCreatesNewCell -> True], 
         Cell[
          StyleData[
          "SeeAlsoSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, DefaultNewCellStyle -> "SeeAlso"], 
         Cell[
          StyleData["SeeAlso", StyleDefinitions -> StyleData["DemoNotes"]], 
          CellDingbat -> 
          Cell["\[FilledSmallSquare]", FontColor -> 
            RGBColor[0.928786, 0.43122, 0.104662]], ShowCellBracket -> False, 
          FontColor -> GrayLevel[0.45098]], 
         Cell[
          StyleData[
          "RelatedLinksSection", StyleDefinitions -> 
           StyleData["DemoSection"]], ShowCellBracket -> False, 
          DefaultNewCellStyle -> "RelatedLinks"], 
         Cell[
          StyleData[
          "RelatedLinks", StyleDefinitions -> StyleData["DemoNotes"], 
           FontColor -> RGBColor[0.928786, 0.43122, 0.104662]], 
          ShowCellBracket -> False, FontColor -> GrayLevel[0.45098]], 
         Cell[
          StyleData[
          "CategoriesSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, DefaultNewCellStyle -> "Categories"], 
         Cell[
          StyleData["Categories", StyleDefinitions -> StyleData["DemoNotes"]],
           ShowCellBracket -> False], 
         Cell[
          StyleData[
          "AuthorSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {4, 18}}, 
          CellElementSpacings -> {"CellMinHeight" -> 3}, 
          CellFrameMargins -> {{48, 48}, {6, 3}}, DefaultNewCellStyle -> 
          "Author", FontSize -> 1, FontColor -> GrayLevel[1]], 
         Cell[
          StyleData[
          "Author", StyleDefinitions -> StyleData["DemoNotes"], FontColor -> 
           GrayLevel[0.64]], ShowCellBracket -> False], 
         Cell[
          StyleData[
          "DetailNotes", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False, FontColor -> GrayLevel[0]], 
         Cell[
          StyleData[
          "CitationSection", StyleDefinitions -> StyleData["DemoSection"]], 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {8, 14}}, 
          DefaultNewCellStyle -> "Categories"], 
         Cell[
          StyleData["Citations", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False, ParagraphSpacing -> {0, 6}], 
         Cell[
          StyleData[
          "RevisionSection", StyleDefinitions -> StyleData["DemoSection"]], 
          DefaultNewCellStyle -> "RevisionNotes"], 
         Cell[
          StyleData[
          "RevisionNotes", StyleDefinitions -> StyleData["DemoNotes"]], 
          ShowCellBracket -> False]}, Closed]], 
      Cell[
       CellGroupData[{
         Cell[
         "Specific Styles", "Section", 
          CellChangeTimes -> {{3.34971724802035*^9, 3.34971724966638*^9}, {
           3.35091840608065*^9, 3.35091840781999*^9}, {3.35091845122987*^9, 
           3.35091845356607*^9}, {3.36230868322317*^9, 3.36230868335672*^9}, {
           3.36928857618576*^9, 3.36928857640452*^9}, {3.3737586217185173`*^9,
            3.373758622077897*^9}}], 
         Cell[
          StyleData["InitializationSection"], CellFrame -> {{0, 0}, {0, 2}}, 
          ShowCellBracket -> False, CellMargins -> {{48, 48}, {10, 30}}, 
          CellGroupingRules -> {"SectionGrouping", 30}, 
          CellFrameMargins -> {{8, 8}, {8, 2}}, CellFrameColor -> 
          RGBColor[0.870588, 0.521569, 0.121569], StyleMenuListing -> None, 
          FontFamily -> "Verdana", FontSize -> 12, FontColor -> 
          GrayLevel[0.45098]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["AnchorBar"], ShowCellBracket -> False, 
             CellMargins -> {{48, 44}, {3, 6}}, StyleMenuListing -> None, 
             FontFamily -> "Verdana", FontSize -> 9, FontColor -> 
             GrayLevel[0.5]], 
            Cell[
             StyleData["AnchorBar", "Presentation"], FontSize -> 18], 
            Cell[
             StyleData["AnchorBar", "SlideShow"], StyleMenuListing -> None], 
            Cell[
             StyleData["AnchorBar", "Printout"], FontSize -> 9]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["AnchorLink"], StyleMenuListing -> None, 
             ButtonStyleMenuListing -> Automatic, FontColor -> 
             RGBColor[0.5, 0.5, 0.5], 
             ButtonBoxOptions -> {
              Active -> True, ButtonFunction :> (FrontEndExecute[{
                  FrontEnd`NotebookLocate[#2]}]& ), ButtonNote -> 
               ButtonData}], 
            Cell[
             StyleData["AnchorLink", "Printout"], 
             FontVariations -> {"Underline" -> False}, FontColor -> 
             GrayLevel[0]]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["GamePadStatus"], ShowCellBracket -> False, 
             CellMargins -> {{48, 48}, {5, 5}}, StyleMenuListing -> None, 
             FontFamily -> "Verdana", FontSize -> 10], 
            Cell[
             StyleData["GamePadStatus", "Printout"], 
             CellMargins -> {{24, 0}, {0, 10}}, FontSize -> 9]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["DemoInstruction"], CellMargins -> {{48, 48}, {5, 5}}, 
             CellFrameLabelMargins -> 2, MenuSortingValue -> 800, 
             MenuCommandKey -> "8", StyleMenuListing -> None, FontFamily -> 
             "Verdana", FontSize -> 11, Background -> RGBColor[1, 0.85, 0.5], 
             DemonstrationSite`Private`ReturnCreatesNewCell -> True], 
            Cell[
             StyleData["DemoInstruction", "Printout"], 
             CellMargins -> {{24, 0}, {0, 10}}, Hyphenation -> True, 
             HyphenationOptions -> {"HyphenationCharacter" -> "-"}, 
             LineSpacing -> {1., 2, 2.}, FontSize -> 9]}, Open]], 
         Cell[
          StyleData[
          "ImplementationSection", StyleDefinitions -> 
           StyleData["DemoSection"]], Deletable -> True, DefaultNewCellStyle -> 
          "ImplementationNotes"], 
         Cell[
          StyleData[
          "ImplementationNotes", StyleDefinitions -> StyleData["DemoNotes"]]], 
         Cell[
          StyleData[
          "StatusSection", StyleDefinitions -> StyleData["DemoSection"]], 
          DefaultNewCellStyle -> "StatusNotes"], 
         Cell[
          StyleData[
          "StatusNotes", StyleDefinitions -> StyleData["DemoNotes"]]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["SectionGloss"], StyleMenuListing -> None, FontSize -> 
             0.85 Inherited, FontWeight -> "Plain", FontColor -> 
             GrayLevel[0.6]], 
            Cell[
             StyleData["SectionGloss", "Printout"]]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["InlineFormula"], 
             "TwoByteSyntaxCharacterAutoReplacement" -> True, 
             HyphenationOptions -> {
              "HyphenationCharacter" -> "\[Continuation]"}, LanguageCategory -> 
             "Formula", AutoSpacing -> True, ScriptLevel -> 1, 
             AutoMultiplicationSymbol -> False, SingleLetterItalics -> False, 
             SpanMaxSize -> 1, StyleMenuListing -> None, FontFamily -> 
             "Courier", FontSize -> 1.05 Inherited, 
             ButtonBoxOptions -> {Appearance -> {Automatic, None}}, 
             FractionBoxOptions -> {BaseStyle -> {SpanMaxSize -> Automatic}}, 
             GridBoxOptions -> {
              GridBoxItemSize -> {
                "Columns" -> {{Automatic}}, "ColumnsIndexed" -> {}, 
                 "Rows" -> {{1.}}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData["InlineFormula", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["InlineOutput"], CellHorizontalScrolling -> True, 
             "TwoByteSyntaxCharacterAutoReplacement" -> True, 
             HyphenationOptions -> {
              "HyphenationCharacter" -> "\[Continuation]"}, LanguageCategory -> 
             None, AutoMultiplicationSymbol -> False, StyleMenuListing -> 
             None, FontFamily -> "Courier", FontSize -> 1.05 Inherited], 
            Cell[
             StyleData["InlineOutput", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["InlineMath"], DefaultFormatType -> 
             "DefaultTextFormatType", DefaultInlineFormatType -> 
             "TraditionalForm", LanguageCategory -> "Formula", AutoSpacing -> 
             True, ScriptLevel -> 1, AutoMultiplicationSymbol -> False, 
             SingleLetterItalics -> True, SpanMaxSize -> DirectedInfinity[1], 
             StyleMenuListing -> None, FontFamily -> "Times", FontSize -> 
             1.05 Inherited, 
             ButtonBoxOptions -> {Appearance -> {Automatic, None}}, 
             GridBoxOptions -> {
              GridBoxItemSize -> {
                "Columns" -> {{Automatic}}, "ColumnsIndexed" -> {}, 
                 "Rows" -> {{1.}}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData["InlineMath", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["TableBase"], CellMargins -> {{48, 48}, {4, 4}}, 
             SpanMaxSize -> 1, StyleMenuListing -> None, FontFamily -> 
             "Courier", FontSize -> 11, 
             ButtonBoxOptions -> {Appearance -> {Automatic, None}}, 
             GridBoxOptions -> {
              GridBoxAlignment -> {
                "Columns" -> {{Left}}, "ColumnsIndexed" -> {}, 
                 "Rows" -> {{Baseline}}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData["TableBase", "Printout"], 
             CellMargins -> {{2, 0}, {0, 8}}, FontSize -> 9]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData[
             "1ColumnTableMod", StyleDefinitions -> StyleData["TableBase"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.04], {
                    Scaled[0.966]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], 
                   Offset[0.126], {
                    Offset[0.77]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.4]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData[
             "1ColumnTableMod", "Printout", StyleDefinitions -> 
              StyleData["TableBase", "Printout"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.078], {
                    Scaled[0.922]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], {
                    Offset[0.56]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.56]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData[
             "2ColumnTableMod", StyleDefinitions -> StyleData["TableBase"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.05], 
                   Scaled[0.41], {
                    Scaled[0.565]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], 
                   Offset[0.14], {
                    Offset[0.77]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.4]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData[
             "2ColumnTableMod", "Printout", StyleDefinitions -> 
              StyleData["TableBase", "Printout"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.079], 
                   Scaled[0.363], {
                    Scaled[0.558]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}},
                  "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], {
                    Offset[0.56]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.56]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData[
             "3ColumnTableMod", StyleDefinitions -> StyleData["TableBase"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.04], 
                   Scaled[0.266], 
                   Scaled[0.26], {
                    Scaled[0.44]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}}, 
                 "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], 
                   Offset[0.14], {
                    Offset[0.77]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.4]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}], 
            Cell[
             StyleData[
             "3ColumnTableMod", "Printout", StyleDefinitions -> 
              StyleData["TableBase", "Printout"]], 
             GridBoxOptions -> {GridBoxItemSize -> {"Columns" -> {
                   Scaled[0.08], 
                   Scaled[0.25], 
                   Scaled[0.25], {
                    Scaled[0.42]}}, "ColumnsIndexed" -> {}, "Rows" -> {{1.}}, 
                 "RowsIndexed" -> {}}, GridBoxSpacings -> {"Columns" -> {
                   Offset[0.28], {
                    Offset[0.56]}, 
                   Offset[0.28]}, "ColumnsIndexed" -> {}, "Rows" -> {
                   Offset[0.2], {
                    Offset[0.56]}, 
                   Offset[0.2]}, "RowsIndexed" -> {}}}]}, Open]], 
         Cell[
          CellGroupData[{
            Cell[
             StyleData["TableText"], Deletable -> False, StyleMenuListing -> 
             None, FontFamily -> "Verdana", FontSize -> 0.952 Inherited], 
            Cell[
             StyleData["TableText", "Printout"], 
             CellMargins -> {{24, 0}, {0, 8}}, Hyphenation -> True, 
             HyphenationOptions -> {"HyphenationCharacter" -> "-"}, 
             LineSpacing -> {1., 2, 2.}]}, Open]], 
         Cell[
          StyleData["Continuation"], FontColor -> GrayLevel[1]]}, Closed]]}, 
     Open]]}, Visible -> False, FrontEndVersion -> 
  "12.0 for Microsoft Windows (64-bit) (September 19, 2019)", 
  StyleDefinitions -> "Default.nb"]
]
(* End of Notebook Content *)

(* Internal cache information *)
(*CellTagsOutline
CellTagsIndex->{
 "ControlSuggestions"->{
  Cell[53058, 938, 316, 8, 70, "ManipulateCaption",ExpressionUUID->"e2f090a0-4d95-4d2e-b4ea-0f62707244a6",
   CellTags->"ControlSuggestions"],
  Cell[53377, 948, 6468, 138, 70, "ManipulateCaption",ExpressionUUID->"01a3432c-0af8-4354-a3e7-d4a39d8d587c",
   CellTags->"ControlSuggestions"]},
 "Details"->{
  Cell[59848, 1088, 110, 1, 70, "DetailsSection",ExpressionUUID->"fc196182-4ed0-44c8-864c-6ef82eb7e8b2",
   CellTags->"Details"]},
 "Copyright"->{
  Cell[78303, 1605, 873, 23, 70, "Text",ExpressionUUID->"b1075779-476e-4f0d-9afd-94ad1b43d7dc",
   CellTags->"Copyright"]}
 }
*)
(*CellTagsIndex
CellTagsIndex->{
 {"ControlSuggestions", 105051, 2174},
 {"Details", 105355, 2179},
 {"Copyright", 105502, 2182}
 }
*)
(*NotebookFileOutline
Notebook[{
Cell[1563, 34, 34446, 569, 70, "DemonstrationHeader",ExpressionUUID->"c8622e65-97ca-4173-959d-4347ec69b0b6"],
Cell[36012, 605, 145, 2, 70, "DemoTitle",ExpressionUUID->"9f848c06-d2cf-40b1-bc2b-6452142f3bca"],
Cell[36160, 609, 15815, 298, 70, "Output",ExpressionUUID->"ebb911a3-74f5-4062-8e6c-6c9f6e00b246",
 CellID->345238038],
Cell[51978, 909, 1077, 27, 70, "ManipulateCaption",ExpressionUUID->"d12b4d50-1327-4509-af26-13b6ed21f5fc",
 CellID->1013067035],
Cell[53058, 938, 316, 8, 70, "ManipulateCaption",ExpressionUUID->"e2f090a0-4d95-4d2e-b4ea-0f62707244a6",
 CellTags->"ControlSuggestions"],
Cell[53377, 948, 6468, 138, 70, "ManipulateCaption",ExpressionUUID->"01a3432c-0af8-4354-a3e7-d4a39d8d587c",
 CellTags->"ControlSuggestions"],
Cell[59848, 1088, 110, 1, 70, "DetailsSection",ExpressionUUID->"fc196182-4ed0-44c8-864c-6ef82eb7e8b2",
 CellTags->"Details"],
Cell[59961, 1091, 159, 2, 70, "DetailNotes",ExpressionUUID->"cc49f8a3-f096-4abf-987d-cdcf3c69c2ad",
 CellID->1520704587],
Cell[60123, 1095, 285, 8, 70, "DetailNotes",ExpressionUUID->"0b72ba76-5bfa-49ba-a627-6928d3d50d9f",
 CellID->619226281],
Cell[60411, 1105, 947, 30, 70, "DetailNotes",ExpressionUUID->"d8a03a53-3ac3-472c-86b9-3358770ae748",
 CellID->1196967707],
Cell[61361, 1137, 102, 1, 70, "DetailNotes",ExpressionUUID->"8ed17abe-965e-446d-92cb-2112eb5f6666",
 CellID->1164058886],
Cell[61466, 1140, 1083, 34, 70, "DetailNotes",ExpressionUUID->"bbdb6f50-b7a2-4442-83c0-c5dc644e4582",
 CellID->218044408],
Cell[62552, 1176, 2828, 83, 70, "DetailNotes",ExpressionUUID->"f5a681bc-6f12-4ca1-ad7f-dea074308e3d",
 CellID->1065263474],
Cell[65383, 1261, 546, 18, 70, "DetailNotes",ExpressionUUID->"ee0e1515-4ae9-40c3-a852-5d7f7aa9fa25",
 CellID->1970551463],
Cell[65932, 1281, 102, 1, 70, "DetailNotes",ExpressionUUID->"b0432eef-5063-4599-abd5-feb4ee50d4cc",
 CellID->1563297351],
Cell[66037, 1284, 533, 16, 70, "DetailNotes",ExpressionUUID->"3de17c52-78b4-4a6c-a216-1ba28b5565d0",
 CellID->1281036049],
Cell[66573, 1302, 878, 26, 70, "DetailNotes",ExpressionUUID->"6a041c04-071a-4a57-8af1-c64806969113",
 CellID->496076751],
Cell[67454, 1330, 134, 1, 70, "DetailNotes",ExpressionUUID->"f014f6e7-c281-45d7-8cc3-d491e31285a7",
 CellID->1127956628],
Cell[67591, 1333, 269, 8, 70, "DetailNotes",ExpressionUUID->"6543a726-e098-43aa-866b-5c42b87f02a8",
 CellID->2133810852],
Cell[67863, 1343, 1245, 40, 70, "DetailNotes",ExpressionUUID->"0b721a69-c1f9-480e-ae48-0bae62e5605f",
 CellID->2057646787],
Cell[69111, 1385, 445, 14, 70, "DetailNotes",ExpressionUUID->"f7205b88-055e-415e-912d-9e6aa774839e",
 CellID->227898012],
Cell[69559, 1401, 1414, 47, 70, "DetailNotes",ExpressionUUID->"f2e868fd-c5b7-4251-bf95-93c10acfe24d",
 CellID->595758509],
Cell[70976, 1450, 443, 8, 70, "DetailNotes",ExpressionUUID->"9d0a85ae-7b45-454e-8366-a06e201c84f1",
 CellID->1413681601],
Cell[71422, 1460, 5205, 96, 70, "DemoSourceNotebookSection",ExpressionUUID->"072f9686-4e32-4ac5-9f38-1fdc85cf6d75",
 CellGroupingRules->{"SectionGrouping", 25}],
Cell[76630, 1558, 100, 0, 70, "CitationSection",ExpressionUUID->"dfabcf30-ee28-4ff0-980d-43d540c344de"],
Cell[76733, 1560, 390, 11, 70, "Author",ExpressionUUID->"f315556b-2173-4665-9e59-791e285c66bb"],
Cell[77126, 1573, 1174, 30, 70, "Citations",ExpressionUUID->"5f2ee03c-51f8-4bbd-83f4-1582d06b7bb9"],
Cell[78303, 1605, 873, 23, 70, "Text",ExpressionUUID->"b1075779-476e-4f0d-9afd-94ad1b43d7dc",
 CellTags->"Copyright"]
}
]
*)

(* End of internal cache information *)

(* NotebookSignature jTar9kQiaYKjWx7AxCWnANzL *)
