...
axs(1)=subplot(2,2,[2 4]); title('Energy and Probability Density: |{\Psi}|^2  (={\Psi}^*{\Psi})'); ylim([0,ysize^2]);  xlabel('x/L'); ylabel('Energy (J/mol)');  set(gca,'fontsize',14); hold all; set(gca,'NextPlot','replacechildren'); 
axs(3)=subplot(2,2,3); title('Real & Imaginary Components');  ylim([-ysize,ysize^2]); xlabel('x/L'); set(gca,'fontsize',14); hold all; set(gca,'NextPlot','replacechildren'); 
...
function IntegratePsi(IG,event,IT)
    ...
    QN=1:max(6,max(n(find(Cn~=0))));;
    levels=(QN).^2/(8*1.67*m*L)^2*6.62^2*6.02;
    energies=n.^2/(8*1.67*m*L)^2*6.62^2*6.02; % In J/mol 
    probs = conj(Cn).*Cn./(sum(conj(Cn).*Cn));
    Esys=sum(probs.*energies);

    axes(axs(1)); hold all
    area(q,psi2*(0.1*levels(end))+Esys,'Basevalue',Esys,'Facecolor','b'); %Shift up to E level and scale psi (won't affect integral already taken) 
...
function MakePlot();
    ...
    plot([0,1],[levels;levels],':k','linewidth',2); hold all
    plot([0,1],[Esys;Esys],'r','linewidth',3);
    plot(x,psi.*conj(psi)*(0.1*levels(end))+Esys,'k','linewidth',2); %Shift up to E level and scale psi (loses) abs mag 
